...
static void jpu_clk_enable_cv181x(struct cvi_jpu_device *jdev);
static void jpu_clk_disable_cv181x(struct cvi_jpu_device *jdev);
static void jpu_fpga_init(struct cvi_jpu_device *jdev);

const struct jpu_ops asic_jpu_ops = {
	.clk_get = jpu_clk_get,
	.clk_put = jpu_clk_put,
	.clk_enable = jpu_clk_enable_cv181x,
	.clk_disable = jpu_clk_disable_cv181x,
	.config_pll = NULL,
};
