...
static void vpu_clk_enable_cv181x(struct cvi_vpu_device *vdev, int mask);
static void vpu_clk_disable_cv181x(struct cvi_vpu_device *vdev, int mask);
static void vpu_fpga_init(struct cvi_vpu_device *vdev, int mask);

const struct vpu_ops asic_vpu_ops = {
	.clk_get = vpu_clk_get,
	.clk_put = vpu_clk_put,
	.clk_enable = vpu_clk_enable_cv181x,
	.clk_disable = vpu_clk_disable_cv181x,
	.config_ddr = NULL,
