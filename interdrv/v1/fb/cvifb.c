...
static void _fb_update_mode(struct fb_info *info)
{
	...
	ow_cfg.start.y = 0;
	ow_cfg.end.x =
		ow_cfg.start.x + (info->var.xres << cfg->gop_ctrl.b.hscl_en) - cfg->gop_ctrl.b.hscl_en;
	ow_cfg.end.y =
		ow_cfg.start.y + (info->var.yres << cfg->gop_ctrl.b.vscl_en) - cfg->gop_ctrl.b.vscl_en;
	ow_cfg.addr = par->mem_base + par->mem_offset;
	sclr_gop_ow_set_cfg(SCL_GOP_DISP, 0, &ow_cfg, true);
}
...
static void _cvifb_release_ion(struct fb_info *info)
{
	...
	if (par->dmabuf) {
		...
		cvi_ion_free(par->fd_pid, par->ion_fd);
		par->dmabuf = NULL;
	}
}
...
#endif
	sclr_gop_ow_set_cfg(SCL_GOP_DISP, 0, &cfg->ow_cfg[0], true);
...
static int _init_resources(struct platform_device *pdev)
{
	...
	/* Interrupt */
	par->irq_num = platform_get_irq_byname(pdev, irq_name[0]);
	if (par->irq_num < 0) {
		dev_err(&pdev->dev, "No IRQ resource for %s\n",	irq_name[0]);
		return -ENODEV;
	}
	fb_info(info, "irq(%d) for %s get from platform driver.\n",
			par->irq_num, irq_name[0]);
