 ...
 * Description: Cvitek clock cooling driver for cv183x
 *
...
/**
 * cv1835_cooling_get_max_state - callback function to get the max cooling state.
 ...
 */
static int cv1835_cooling_get_max_state(struct thermal_cooling_device *cdev,
					unsigned long *state)
{
	struct cv1835_cooling_device *cvcdev = cdev->devdata;
...
/**
 * cv1835_cooling_get_cur_state - function to get the current cooling state.
 * @cdev: thermal cooling device pointer.
 ...
 * Return: 0 (success)
 */
static int cv1835_cooling_get_cur_state(struct thermal_cooling_device *cdev,
					unsigned long *state)
{
	struct cv1835_cooling_device *cvcdev = cdev->devdata;
...
/**
 * cv1835_cooling_set_cur_state - function to set the current cooling state.
 * @cdev: thermal cooling device pointer.
 * @state: set this variable to the current cooling state.
 *
 * Callback for the thermal cooling device to change the cv1835 cooling
 * current cooling state.
 *
 * Return: 0 on success, an error code otherwise.
 */
static int cv1835_cooling_set_cur_state(struct thermal_cooling_device *cdev,
					unsigned long state)
{
	struct cv1835_cooling_device *cvcdev = cdev->devdata;
...
static struct thermal_cooling_device_ops const cv1835_cooling_ops = {
	.get_max_state = cv1835_cooling_get_max_state,
	.get_cur_state = cv1835_cooling_get_cur_state,
	.set_cur_state = cv1835_cooling_set_cur_state,
};

static struct cv1835_cooling_device *
cv1835_cooling_device_register(struct device *dev)
{
	struct thermal_cooling_device *cdev;
	struct cv1835_cooling_device *cvcdev = NULL;
...
static void cv1835_cooling_device_unregister(struct cv1835_cooling_device
						 *cvcdev)
{
	thermal_cooling_device_unregister(cvcdev->cdev);
}

static int cv1835_cooling_probe(struct platform_device *pdev)
{
	struct cv1835_cooling_device *cvcdev;

	cvcdev = cv1835_cooling_device_register(&pdev->dev);
	if (IS_ERR(cvcdev)) {
...
static int cv1835_cooling_remove(struct platform_device *pdev)
{
	struct cv1835_cooling_device *cvcdev = platform_get_drvdata(pdev);

	if (!IS_ERR(cvcdev))
		cv1835_cooling_device_unregister(cvcdev);

	return 0;
}

static const struct of_device_id cv1835_cooling_match[] = {
	{.compatible = "cvitek,cv1835-cooling"},
	{},
};
MODULE_DEVICE_TABLE(of, cv1835_cooling_match);

static struct platform_driver cv1835_cooling_driver = {
	.driver = {
		.name = "cv1835-cooling",
		.of_match_table = of_match_ptr(cv1835_cooling_match),
	},
	.probe = cv1835_cooling_probe,
	.remove = cv1835_cooling_remove,
};

module_platform_driver(cv1835_cooling_driver);

MODULE_AUTHOR("Cvitek Co.");
MODULE_DESCRIPTION("cv183x clock cooling driver");
MODULE_LICENSE("GPL");
