...
void isp_pre_trig(struct isp_ctx *ctx, enum cvi_isp_raw raw_num, const u8 chn_num)
{
	...
	if (ctx->isp_pipe_cfg[raw_num].is_offline_preraw) { //dram->fe
		...
		if (!ctx->is_offline_be && !ctx->is_offline_postraw) { // fly-mode
			if (raw_num == ISP_PRERAW_A)
				ISP_WR_REG(isptopb, REG_ISP_TOP_T, REG_3, 0x1410141);
			else
				ISP_WR_REG(isptopb, REG_ISP_TOP_T, REG_3, 0x1500150);
...
int ispblk_isptop_config(struct isp_ctx *ctx)
{
	...
	reg5.bits.BE2YUV_422_ENABLE	= 0; //Single yuv offline always csi to dram -> mw vi deque
				...
				int16_t *r_lut, int16_t *g_lut, int16_t *b_lut)
{
	...
	ISP_WR_BITS(clut, REG_ISP_CLUT_T, CLUT_CTRL, CLUT_ENABLE, en);

	ISP_WR_BITS(clut, REG_ISP_CLUT_T, CLUT_CTRL, PROG_EN, 1);
	...
	ISP_WR_BITS(clut, REG_ISP_CLUT_T, CLUT_CTRL, PROG_EN, 0);
}
...
void ispblk_ir_merge_config(struct isp_ctx *ctx)
{
	...
	if (enable) {
		...
		if (bit_mode == 0) {
			...
			ISP_WR_BITS(rgbtop, REG_ISP_RGB_T, REG_11, IR_IMG_WIDTH, (dmi_num + 2)/3*4);
		} else {
			dmi_num = VIP_ALIGN(ctx->img_width)>>1;
			ISP_WR_BITS(rgbtop, REG_ISP_RGB_T, REG_11, IR_IMG_WIDTH, (dmi_num + 2)/3*6);
		}
...
void ispblk_pre_be_cfg_update(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	//AF config update
	{
		...
		numx = ISP_RD_REG(sts, REG_ISP_AF_T, BLOCK_NUM_X);
		numy = ISP_RD_REG(sts, REG_ISP_AF_T, BLOCK_NUM_Y);
	...
	//AE config update
	{
		...
		num_x = ISP_RD_REG(sts, REG_ISP_AE_HIST_T, STS_AE_NUMXM1);
		num_y = ISP_RD_REG(sts, REG_ISP_AE_HIST_T, STS_AE_NUMYM1);

		sub_window_w = ctx->img_width / (num_x + 1);
		sub_window_h = ctx->img_height / (num_y + 1);
	...
	const enum cvi_isp_raw raw_num)
{
	...
	if ((cfg->x_section_size * 3 + cfg->offset_x + cfg->x_gap * 2) > img_width) {
