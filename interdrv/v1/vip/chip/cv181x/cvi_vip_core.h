...
struct cvi_isp_vdev {
	...
	u32                             pre_be_awb_frm_num[ISP_PRERAW_MAX][ISP_BE_CHN_MAX];
	...
	wait_queue_head_t		isp_err_wait_q;
	u32				isp_err_int_flag;
...
struct cvi_img_vdev {
	...
	struct cvi_vpss_grp_cfg         vpss_grp_cfg[2];
	u8                              job_grp;
...
struct cvi_sc_vdev {
	...
	struct cvi_vpss_chn_cfg         vpss_chn_cfg[2];

	enum sclr_tile_mode             tile_mode;
	void                            *shared_mem;
	atomic_t                        is_streaming;
	atomic_t                        buf_empty[ISP_PRERAW_MAX];
	bool                            bind_fb;
	bool                            is_cmdq;
};
