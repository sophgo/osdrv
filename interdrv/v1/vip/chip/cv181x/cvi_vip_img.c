...
static int cvi_img_open(struct file *file)
{
	...
	idev->irq_cnt[0] = idev->irq_cnt[1] = 0;
	...
	struct v4l2_ext_controls *vc)
{
	...
	for (i = 0; i < vc->count; ++i) {
		...
		case V4L2_CID_DV_VIP_IMG_SET_CUSTOM_CSC: {
			struct cvi_csc_cfg cfg;
			...
			idev->vpss_grp_cfg[0].csc_cfg = cfg;
			rc = 0;
		}
		break;
		...
		case V4L2_CID_DV_VIP_IMG_SET_VPSS_GRP_CFG: {
			...
			idev->vpss_grp_cfg[cfg.grp_id] = cfg;
		...
		case V4L2_CID_DV_VIP_IMG_TRIGGER_RGNEX: {
			...
			mask = sclr_get_intr_mask();
			mask.b.cmdq = true;
			mask.b.img_in_d_frame_end = false;
			mask.b.scl0_frame_end = false;
	    ...
	    struct v4l2_format *f)
{
	...
	idev->vpss_grp_cfg[0].src_size.width = mp->width;
	idev->vpss_grp_cfg[0].src_size.height = mp->width;
	idev->vpss_grp_cfg[0].pixelformat = mp->pixelformat;
	idev->vpss_grp_cfg[0].bytesperline[0] = pfmt[0].bytesperline;
	idev->vpss_grp_cfg[0].bytesperline[1] = pfmt[1].bytesperline;
	idev->vpss_grp_cfg[0].crop.left = 0;
	idev->vpss_grp_cfg[0].crop.top = 0;
	idev->vpss_grp_cfg[0].crop.width = mp->width;
	idev->vpss_grp_cfg[0].crop.height = mp->height;
	return rc;
}
	...
	if (cmdq_intr_status & 0x02) {
		...
		if (bdev->img_vdev[0].is_cmdq == true)
			cvi_img_v4l2_event_queue(&bdev->img_vdev[0],
				V4L2_EVENT_CVI_VIP_VPSS_CMDQ_DONE, 0);
	}
