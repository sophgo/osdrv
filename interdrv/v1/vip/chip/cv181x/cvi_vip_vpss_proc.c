...
int vpss_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "\n-------------------------------VPSS GRP WORK STATUS-----------------------\n");
	seq_printf(m, "%10s%10s%10s%20s%10s%20s%20s\n",
		"GrpID", "RecvCnt", "LostCnt", "StartFailCnt", "bStart", "CostTime(us)", "MaxCostTime(us)");
	for (i = 0; i < VPSS_MAX_GRP_NUM; ++i) {
		if (pvpssCtx[i].isCreated) {
			seq_printf(m, "%8s%2d%10d%10d%20d%10s%20d%20d\n",
				"#",
				i,
	...
	for (i = 0; i < CVI_VIP_IMG_MAX; ++i) {
		seq_printf(m, "%12s%2d%20d%20d%20d%20d\n%14d%20d%20d%20d\n",
			...
			bdev->img_vdev[i].irq_cnt[0], bdev->img_vdev[i].irq_cnt[1]);
	}
