 ...
 *
 * File Name: cvi_vip_isp_ext.c
 * Description: video pipeline isp auto focus sync driver
 */
...
void sync_task_init(int vi_pipe)
{
	...
	dprintk(VIP_INFO, "sync_task_init vi_pipe %d\n", vi_pipe);
}
...
void sync_task_exit(int vi_pipe)
{
	...
	dprintk(VIP_INFO, "sync_task_exit vi_pipe %d\n", vi_pipe);
}
