...
void sclr_set_scale(u8 inst)
{
	...
	tmp_fac = (g_sc_cfg[inst].sc.crop.w - 1) / (g_sc_cfg[inst].sc.dst.w - 1);
	isFacOver4 |= (tmp_fac >= 4);

	// if factor >= 4, modify scaling factor formula
	if (tmp_fac >= 4)
		tmp = ((g_sc_cfg[inst].sc.crop.w) << 13) / (g_sc_cfg[inst].sc.dst.w);
	else
	...
	tmp_fac = (g_sc_cfg[inst].sc.crop.h - 1) / (g_sc_cfg[inst].sc.dst.h - 1);
	isFacOver4 |= (tmp_fac >= 4);

	// if factor >= 4, modify scaling factor formula
	if (tmp_fac >= 4)
		tmp = ((g_sc_cfg[inst].sc.crop.h) << 13) / (g_sc_cfg[inst].sc.dst.h);
	else
	...
	// if input wd = 4096 and downscale integer multiple, disable cb mode
	if (g_sc_cfg[inst].sc.crop.w == 4096 &&
		((g_sc_cfg[inst].sc.crop.w) % (g_sc_cfg[inst].sc.dst.w) == 0)) {
		sclr_set_scale_mode(inst, g_sc_cfg[inst].sc.mir_enable, false, true);
	} else {
...
void sclr_odma_set_cfg(u8 inst, struct sclr_odma_cfg *cfg)
{
	...
	sclr_odma_set_int_line_target(inst, cfg->line_cnt);
...
void sclr_odma_set_int_line_target(u8 inst, u16 line_cnt)
{
	_reg_write(reg_base + REG_SCL_ODMA_INT_LINE_TARGET(inst), line_cnt);
}
...
{
	if (inst < SCL_MAX_INST) {
		_reg_write(reg_base + REG_SCL_GOP_CFG(inst), cfg->gop_ctrl.raw & 0xffff);
		...
		_reg_write(reg_base + REG_SCL_DISP_GOP_CFG, cfg->gop_ctrl.raw & 0xffff);
...
/**
 * sclr_gop_setup_LUT - setup gop's Look-up table from index 0 to length
 *
 * @param inst: (0~4), the instance of gop which want to be configured.
 *		0~3 is on scl, 4 is on disp.
 * @param length: update LUT-table for index 0 ~ length.
 *		There should be smaller than 256 instances.
 * @param data: values of LUT-table for index 0 ~ length.
 */
void sclr_gop_setup_LUT(u8 inst, u16 length, u16 *data)
{
	u16 i = 0;
	struct sclr_gop_cfg gop_cfg;

	gop_cfg = *sclr_gop_get_cfg(inst);
	pr_debug("before update LUT, gop_cfg ctrl:%#x\n", _reg_read(reg_base + REG_SCL_GOP_CFG(inst)));

	if (length > 256) {
		pr_err("[cvi-vip][sc] %s: length is over 256", __func__);
		return;
	}

	if (inst < SCL_MAX_INST) {
		pr_debug("%s: on sc(%d)\n", __func__, inst);
		pr_debug("LUT-length: %d\n", length);

		//Disable OW enable in gop ctrl register
		_reg_write(reg_base + REG_SCL_GOP_CFG(inst), 0x0);
		for (i = 0; i < length; ++i) {
			_reg_write(reg_base + REG_SCL_GOP_LUT0(inst),
				   (i << 16) | *(data + i));
			_reg_write(reg_base + REG_SCL_GOP_LUT1(inst), BIT(16));
			_reg_write(reg_base + REG_SCL_GOP_LUT1(inst), ~BIT(16));
			pr_debug("LUT-index %d: data(0x%x)\n", i, *(data + i));
		}

		//Enable original OW enable in gop ctrl register
		_reg_write(reg_base + REG_SCL_GOP_CFG(inst), gop_cfg.gop_ctrl.raw);
 ...
 */
//TODO: Need to disable OW and check clk status such as sclr_gop_setup_LUT.
int sclr_gop_update_LUT(u8 inst, u16 index, u16 data)
{
	if (index > 256) {
		pr_err("[cvi-vip][sc] %s: index is over 256", __func__);
		return -1;
	}
...
void sclr_disp_bt_reg03(enum sclr_vo_intf vo_intf)
{
	switch (vo_intf) {
...
void sclr_ctrl_init(bool is_resume)
{
	...
	intr_mask.b.scl0_line_target_hit = true;
	intr_mask.b.scl1_line_target_hit = true;
	intr_mask.b.scl2_line_target_hit = true;
	intr_mask.b.scl3_line_target_hit = true;
		     ...
		     u16 cmd_idx)
{
	...
	cmdQ_set_package(&cmd_start[cmd_idx++].reg,
			 ...
			 (img_cfg.burst << 8) | (img_cfg.csc_en << 7) |
...
bool sclr_left_tile(u8 inst, u16 src_l_w)
{
	...
	dprintk(VIP_DBG, "sc%d input size: w=%d h=%d\n", inst, src.w, src.h);
	dprintk(VIP_DBG, "sc%d crop size: x=%d y=%d w=%d h=%d\n", inst, crop.x, crop.y, crop.w, crop.h);
	dprintk(VIP_DBG, "sc%d output size: w=%d h=%d\n", inst, dst.w, dst.h);
	dprintk(VIP_DBG, "sc%d odma mem: x=%d y=%d w=%d h=%d pitch_y=%d pitch_c=%d\n", inst,
