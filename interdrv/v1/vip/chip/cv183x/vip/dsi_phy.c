...
void dphy_dsi_get_pixclk(u32 *clkkHz, u8 lane, u8 bits)
{
	...
	for (loop_gain = 8; loop_gain < 255; loop_gain += 8) {
		VCOCx1000 = ((u64)factor << 26) * loop_gain / reg_set;
...
void dphy_dsi_set_bt656_54mhz(u8 lane, u8 bits)
{
	_reg_write(reg_base + REG_DSI_PHY_TXPLL, 0x2118);
	_reg_write(reg_base + REG_DSI_PHY_REG_SET, 0x1da13159);
	...
	//set vip clock ctrl
	vip_sys_reg_write_mask(VIP_SYS_VIP_CLK_CTRL0, 0xFFFFFFFF, 0x0);
}
