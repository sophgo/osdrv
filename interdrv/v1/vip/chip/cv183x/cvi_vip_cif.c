...
static unsigned int max_mac_clk = 400;
...
static int cif_set_hs_mode(struct cvi_cif_dev *dev, unsigned int mode);
...
const char *_to_string_mipi_vsync_gen_mode(enum mipi_vs_gen_mode mode)
{
	switch (mode) {
	case CSI_VSYNC_BY_FS:
		return "FS";
	case CSI_VSYNC_BY_FE:
		return "FE";
	case CSI_VSYNC_BY_FS_FE:
		return "FS & FE";
	default:
		return "unknown";
	}
			      ...
			      struct combo_dev_attr_s *attr)
{
	...
	case INPUT_MODE_MIPI: {
		...
		dev_dbg(_dev, "vs_gen_mode = %s\n",
			_to_string_mipi_vsync_gen_mode(mipi->vs_gen_mode));
	}
	break;
	case INPUT_MODE_SUBLVDS:
	case INPUT_MODE_HISPI: {
			      ...
			      struct mipi_dev_attr_s *attr)
{
	...
	for (i = 0; i < CIF_LANE_NUM; i++) {
		...
		if (lane_phase[i] > 0)
			cif_set_lane_deskew(ctx, attr->lane_id[i], lane_phase[i]);
	...
	case CVI_MIPI_WDR_MODE_DOL:
		/*
		 * [TODO] csi-bdg frame error with unequal lsef size.
		 */
	default:
		return -EINVAL;
	}
	...
	csi->vs_gen_mode = attr->vs_gen_mode;
				...
				struct combo_dev_attr_s *attr)
{
	...
	param->type = CIF_TYPE_TTL;
	ttl->fmt_out = TTL_BT_FMT_OUT_YCBYCR;
				  ...
				  struct combo_dev_attr_s *attr)
{
	...
	ttl->sensor_fmt = TTL_SENSOR_12_BIT;
	ttl->clk_inv = link->clk_edge;
	ttl->vi_sel = (attr->devno == 0) ? VI_BT656_1 : VI_BT656_VI1;
...
#define MAC0_CLK_RATIO_OFFSET		10U
#define MAC1_CLK_RATIO_OFFSET		15U
#define MAC_CLK_RATIO_MASK		0x1FU
		...
		enum rx_mac_clk_e mac_clk)
{
	...
	mask = (devno == 0) ? (MAC_CLK_CTRL1_MASK << MAC0_CLK_CTRL1_OFFSET) :
				(MAC_CLK_CTRL1_MASK << MAC1_CLK_CTRL1_OFFSET);
	data = 0x2 << ((devno == 0) ? MAC0_CLK_CTRL1_OFFSET : MAC1_CLK_CTRL1_OFFSET);
	...
	if (cdev->max_mac_clk == 500) {
		...
		clk_set_parent(cdev->vip_sys2.clk_o, cdev->clk_div_1_src_vip_sys_2.clk_o);
	} else {
		dev_dbg(link->dev, "use div0 mipipll as vip_sys2 source\n");
		clk_set_parent(cdev->vip_sys2.clk_o, cdev->clk_div_0_src_vip_sys_2.clk_o);
	}

#if defined(CONFIG_COMMON_CLK_CVITEK)
	tmp = clk_val * 16 / cdev->max_mac_clk;
	/* roundup */
	if ((clk_val * 16) % cdev->max_mac_clk)
		tmp++;
	if (!devno)
		VIP_CLK_RATIO_CONFIG(CSI_MAC0, tmp);
	else
		VIP_CLK_RATIO_CONFIG(CSI_MAC1, tmp);

	clk_set_rate(clk_get_parent(cdev->vip_sys2.clk_o), cdev->max_mac_clk * 1000000UL);
	dev_dbg(link->dev, "ratio %d, set rate %dM\n", tmp, tmp * cdev->max_mac_clk / 16);
	udelay(5);
#else
	switch (mac_clk) {
	case RX_MAC_CLK_400M:
...
static int cif_set_mac_clk(struct cvi_cif_dev *dev, uint32_t devno,
		enum rx_mac_clk_e mac_clk)
{
			    ...
			    struct combo_dev_attr_s *attr)
{
	...
	struct cif_ctx *ctx = &dev->link[attr->devno].cif_ctx;
	struct cif_param *param = &dev->link[attr->devno].param;
	struct combo_dev_attr_s *rx_attr = &dev->link[attr->devno].attr;
	int rc = 0;
	...
	case INPUT_MODE_BT601_19B_VHS:
		...
		break;
	case INPUT_MODE_BT656_9B:
		//cif_set_hs_mode(dev, 1); // for single cam
		cif_set_hs_mode(dev, 6); // for dual cam
				   ...
				   struct clk_edge_s *clk_edge)
{
	...
	cif_set_clk_edge(ctx, CIF_LANE_0,
			 (clk_edge->edge == CLK_UP_EDGE)
			 ? CIF_CLK_RISING : CIF_CLK_FALLING);
	cif_set_clk_edge(ctx, CIF_LANE_1,
			 (clk_edge->edge == CLK_UP_EDGE)
			 ? CIF_CLK_RISING : CIF_CLK_FALLING);
	cif_set_clk_edge(ctx, CIF_LANE_2,
			 (clk_edge->edge == CLK_UP_EDGE)
			 ? CIF_CLK_RISING : CIF_CLK_FALLING);
	cif_set_clk_edge(ctx, CIF_LANE_3,
			 (clk_edge->edge == CLK_UP_EDGE)
			 ? CIF_CLK_RISING : CIF_CLK_FALLING);
...
static int cif_reset_mipi(struct cvi_cif_dev *dev, uint32_t devno)
{
	...
	/* mask the interrupts */
	if (link->is_on)
		cif_mask_csi_int_sts(&link->cif_ctx, 0x0F);
...
static int cif_set_wdr_manual(struct cvi_cif_dev *dev,
				...
				uint32_t devno, uint8_t on)
{
	...
	if (mclk0 > CAMPLL_FREQ_NONE && mclk0 < CAMPLL_FREQ_NUM) {
		...
		/* camera interface. */
		PINMUX_CONFIG(CAM_MCLK0, XGPIOD_4);
	}
	...
	if (mclk1 > CAMPLL_FREQ_NONE  && mclk1 < CAMPLL_FREQ_NUM) {
		...
		/* camera interface. */
		PINMUX_CONFIG(CAM_PD0, XGPIOD_1);
	}
...
static int cif_set_hs_mode(struct cvi_cif_dev *dev, unsigned int mode)
{
	...
	case 4:
	case 5:
		PINMUX_CONFIG(MIPIRX1_PAD0P, VI_DATA_0);
		PINMUX_CONFIG(MIPIRX1_PAD0N, VI_DATA_1);
		PINMUX_CONFIG(MIPIRX1_PAD1P, VI_DATA_2);
		PINMUX_CONFIG(MIPIRX1_PAD1N, VI_DATA_3);
		PINMUX_CONFIG(MIPIRX1_PAD2P, VI_DATA_4);
		PINMUX_CONFIG(MIPIRX1_PAD2N, VI_DATA_5);
		PINMUX_CONFIG(MIPIRX1_PAD3P, VI_DATA_6);
		PINMUX_CONFIG(MIPIRX1_PAD3N, VI_DATA_7);
		PINMUX_CONFIG(MIPIRX1_PAD4P, VI_DATA_8);
		PINMUX_CONFIG(MIPIRX1_PAD4N, VI_DATA_9);
		PINMUX_CONFIG(MIPIRX0_PAD4N, VI_DATA_10);
		PINMUX_CONFIG(MIPIRX0_PAD4P, VI_DATA_11);
		PINMUX_CONFIG(MIPIRX0_PAD3N, VI_DATA_12);
		PINMUX_CONFIG(MIPIRX0_PAD3P, VI_CLK);
		PINMUX_CONFIG(MIPIRX0_PAD2N, VI_DATA_13);
		PINMUX_CONFIG(MIPIRX0_PAD2P, VI_DATA_14);
		PINMUX_CONFIG(MIPIRX0_PAD1N, VI_DATA_15);
		PINMUX_CONFIG(MIPIRX0_PAD1P, VI_DATA_16);
		PINMUX_CONFIG(MIPIRX0_PAD0N, VI_DATA_17);
		PINMUX_CONFIG(MIPIRX0_PAD0P, VI_DATA_18);
		PINMUX_CONFIG(VI_DATA19, VI_DATA_19);
		PINMUX_CONFIG(VI_DATA20, VI_DATA_20);
		PINMUX_CONFIG(VI_DATA21, VI_DATA_21);
		PINMUX_CONFIG(VI_DATA22, VI_DATA_22);
		PINMUX_CONFIG(VI_DATA23, VI_DATA_23);
		PINMUX_CONFIG(VI_DATA24, VI_DATA_24);
		break;
	case 6:
		PINMUX_CONFIG(MIPIRX1_PAD0P, VI1_DATA_8);
		PINMUX_CONFIG(MIPIRX1_PAD0N, VI1_DATA_7);
		PINMUX_CONFIG(MIPIRX1_PAD1P, VI1_DATA_6);
		PINMUX_CONFIG(MIPIRX1_PAD1N, VI1_DATA_5);
		PINMUX_CONFIG(MIPIRX1_PAD2P, VI1_DATA_4);
		PINMUX_CONFIG(MIPIRX1_PAD2N, VI1_DATA_3);
		PINMUX_CONFIG(MIPIRX1_PAD3P, VI1_DATA_2);
		PINMUX_CONFIG(MIPIRX1_PAD3N, VI1_DATA_1);
		PINMUX_CONFIG(MIPIRX1_PAD4P, VI1_DATA_0);
		PINMUX_CONFIG(MIPIRX1_PAD4N, VI1_CLK);
		PINMUX_CONFIG(MIPIRX0_PAD4N, VI1_DATA_9);
		PINMUX_CONFIG(MIPIRX0_PAD4P, VI_DATA_11);
		PINMUX_CONFIG(MIPIRX0_PAD3N, VI_DATA_12);
		PINMUX_CONFIG(MIPIRX0_PAD3P, VI_CLK);
		PINMUX_CONFIG(MIPIRX0_PAD2N, VI_DATA_13);
...
{
	struct device *_dev = dev->miscdev.this_device;
	struct cif_ctx *ctx = &dev->link[devno].cif_ctx;
	struct cif_param *param = ctx->cur_config;

	if (!param) {
		dev_err(_dev, "mipirx %d attr is not ready!\n", devno);
		return -EINVAL;
	}

	if (!(param->type == CIF_TYPE_SUBLVDS || param->type == CIF_TYPE_HISPI))
		return 0;

	cif_set_lvds_en(ctx, 0);
	udelay(5);
	if (param->type == CIF_TYPE_SUBLVDS)
		cif_set_lvds_en(ctx, param->cfg.sublvds.lane_num);
	else if (param->type == CIF_TYPE_HISPI)
		cif_set_lvds_en(ctx, param->cfg.hispi.lane_num);
		       ...
		       unsigned long arg, unsigned int from_user)
{
	...
	case CVI_MIPI_SET_HS_MODE:
		if (from_user) {
			...
			devno = *(uint32_t *)arg;
		return cif_set_hs_mode(dev, devno);
	case CVI_MIPI_RESET_LVDS:
		if (from_user) {
			if (copy_from_user(&devno, (void *)arg, sizeof(devno))) {
				dev_err(_dev, "copy_from_user failed.\n");
...
static long cif_cb(struct cvi_cif_dev *dev, unsigned int cmd, unsigned long arg)
{
	return _cif_ioctl(dev, cmd, arg, 0);
}
...
static int cif_init_miscdev(struct platform_device *pdev, struct cvi_cif_dev *dev)
{
	...
	/* register cif_cb */
	rc = vip_sys_register_cif_cb(dev, cif_cb);
	if (rc)
...
static irqreturn_t cif_isr(int irq, void *_link)
{
	...
	if (link->sts_csi.errcnt_ecc > 0xFFFF ||
		...
		cif_mask_csi_int_sts(ctx, 0x0F);
		dev_err(link->dev, "mask the interrupt since err cnt is full\n");
		dev_err(link->dev, "ecc = %u, crc = %u, wc = %u, hdr = %u\n",
				link->sts_csi.errcnt_ecc,
				link->sts_csi.errcnt_crc,
				link->sts_csi.errcnt_wc,
				link->sts_csi.errcnt_hdr);
...
static int _init_resource(struct platform_device *pdev)
{
	...
	dev->clk_div_0_src_vip_sys_2.clk_o = devm_clk_get(&pdev->dev, "clk_div_0_src_vip_sys_2");
	if (!IS_ERR_OR_NULL(dev->clk_div_0_src_vip_sys_2.clk_o))
		dev_info(&pdev->dev, "clk_div_0_src_vip_sys_2 clk installed %p\n", dev->clk_div_0_src_vip_sys_2.clk_o);
	dev->clk_div_1_src_vip_sys_2.clk_o = devm_clk_get(&pdev->dev, "clk_div_1_src_vip_sys_2");
	if (!IS_ERR_OR_NULL(dev->clk_div_1_src_vip_sys_2.clk_o))
		dev_info(&pdev->dev, "clk_div_1_src_vip_sys_2 clk installed %p\n", dev->clk_div_1_src_vip_sys_2.clk_o);

	for (i = 0; i < MAX_LINK_NUM*2; ++i) {
		...
		if (i & 0x1)
			cif_set_base_addr(i>>1, reg_base[i-1], reg_base[i]);
	}
	/* Init max mac clock. */
	if (max_mac_clk <= 400)
		dev->max_mac_clk = 400;
			      ...
			      struct cvi_link *link)
{
	...
	sts->fifo_full = cif_check_csi_fifo_full(&link->cif_ctx);
			      ...
			      struct cvi_link *link)
{
	...
	seq_printf(m, "%11s%6s%6s%6s%6s%6s\n",
		   "Physical:", "D0", "D1", "D2", "D3", "D4");
	seq_printf(m, "%11s%6x%6x%6x%6x%6x\n",
		   " ",
	...
	seq_printf(m, "%11s%9s%9s%9s%9s%9d%9d%9d%9d%9s\n",
		   ...
		   state.bits.clk_hs_state, state.bits.clk_ulps_state,
		   state.bits.clk_stop_state, state.bits.clk_err_state,
		   _to_string_deskew_state(state.bits.deskew_state));
}
