 ...
 *
 * File Name: cvi_vip_mipi_tx.c
 * Description: video pipeline mipi inferface tx driver
 */
...
#include "cvi_vip_mipi_tx_proc.h"
...
static int mipi_tx_set_cmd(struct cmd_info_s *cmd_info)
{
	...
	snprintf(str, 96, "%s: ", __func__);
	for (i = 0; i < cmd_info->cmd_size && i < 16; ++i)
		snprintf(str + strlen(str), 96 - strlen(str), "%#x ", cmd_info->cmd[i]);
	pr_debug("%s\n", str);
...
static long mipi_tx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CVI_VIP_MIPI_TX_SET_CMD: {
		...
		if (cmd_info.cmd == NULL) {
			...
			cmd_info.cmd = kmalloc(cmd_info.cmd_size, GFP_KERNEL);
			if (cmd_info.cmd == NULL) {
				CVI_VIP_MIPI_TX_ERR("kmalloc failed.\n");
				rc = -ENOMEM;
				break;
			}
			if (copy_from_user(cmd_info.cmd, ((struct cmd_info_s *)arg)->cmd, cmd_info.cmd_size)) {
...
static int cvi_mipi_tx_remove(struct platform_device *pdev)
{
	...
	if (tdev->irq_vbat > 0) {
		iounmap(tdev->vbat_addr[0]);
		iounmap(tdev->vbat_addr[1]);
	}
