...
static void _hw_enque(struct cvi_disp_vdev *ddev)
{
	...
	cfg->mem.addr0 = addr[0];
	cfg->mem.addr1 = addr[1];
	cfg->mem.addr2 = addr[2];
...
static int cvi_disp_release(struct file *file)
{
	...
	if (__clk_is_enabled(ddev->clk_bt))
...
static void _disp_sel_pinmux(enum cvi_disp_intf intf_type, void *param)
{
	if (intf_type == CVI_VIP_DISP_INTF_I80) {
		...
		PINMUX_CONFIG(PAD_MIPI_TXM0, VO_DATA_2);
		PINMUX_CONFIG(PAD_MIPI_TXP0, VO_CLK);
		if (cfg->mode == BT_MODE_1120) {
			PINMUX_CONFIG(MIPIRX1_PAD0P, VO_DATA_11);
			PINMUX_CONFIG(MIPIRX1_PAD0N, VO_DATA_12);
			PINMUX_CONFIG(MIPIRX1_PAD1P, VO_DATA_13);
			PINMUX_CONFIG(MIPIRX1_PAD1N, VO_DATA_14);
			PINMUX_CONFIG(MIPIRX1_PAD2P, VO_DATA_15);
			PINMUX_CONFIG(MIPIRX1_PAD2N, VO_DATA_16);
			PINMUX_CONFIG(MIPIRX1_PAD3P, VO_DATA_17);
		}
	}
}
	...
	for (i = 0; i < vc->count; ++i) {
		...
		case V4L2_CID_DV_VIP_DISP_INTF: {
			...
			if (cfg->intf_type == CVI_VIP_DISP_INTF_DSI) {
				...
				//enable clk_bt
				clk_prepare_enable(ddev->clk_bt);
