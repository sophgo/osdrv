 ...
 *
 * File Name: cvi_vip_sc.c
 * Description: video pipeline scaler driver
 */
...
#define VPSS_SHARE_MEM_SIZE     (0xD000)
	...
	const bool is_online)
{
	...
	if (is_online) {
		list_add_tail(&b->list, &vdev->ol_rdy_queue[grp_id]);
		++vdev->ol_num_rdy[grp_id];
	} else {
...
struct cvi_sc_buf2 *cvi_sc_next_buf(struct cvi_sc_vdev *vdev, const u8 grp_id, const bool is_online)
{
	unsigned long flags;
	struct cvi_sc_buf2 *b = NULL;
	...
	spin_lock_irqsave(&vdev->rdy_lock, flags);
	if (is_online) {
		if (!list_empty(&vdev->ol_rdy_queue[grp_id]))
			b = list_first_entry(&vdev->ol_rdy_queue[grp_id], struct cvi_sc_buf2, list);
	} else {
		if (!list_empty(&vdev->rdy_queue))
			b = list_first_entry(&vdev->rdy_queue, struct cvi_sc_buf2, list);
	}
	spin_unlock_irqrestore(&vdev->rdy_lock, flags);
...
int cvi_sc_buf_num(struct cvi_sc_vdev *vdev, const u8 grp_id, const bool is_online)
{
	unsigned long flags;
	int num;
...
int cvi_sc_buf_remove(struct cvi_sc_vdev *vdev, const u8 grp_id, const bool is_online)
{
	unsigned long flags;
	struct cvi_sc_buf2 *b = NULL;
	...
	spin_lock_irqsave(&vdev->rdy_lock, flags);
	if (is_online) {
		if (vdev->ol_num_rdy[grp_id] == 0) {
			spin_unlock_irqrestore(&vdev->rdy_lock, flags);
			return -1;
		}
		if (!list_empty(&vdev->ol_rdy_queue[grp_id])) {
			b = list_first_entry(&vdev->ol_rdy_queue[grp_id],
...
void cvi_sc_buf_remove_all(struct cvi_sc_vdev *vdev, const u8 grp_id, const bool is_online)
{
	unsigned long flags;
	struct cvi_sc_buf2 *b = NULL;
	...
	spin_lock_irqsave(&vdev->rdy_lock, flags);
	if (is_online) {
		while (!list_empty(&vdev->ol_rdy_queue[grp_id])) {
			b = list_first_entry(&vdev->ol_rdy_queue[grp_id],
				struct cvi_sc_buf2, list);
			list_del_init(&b->list);
			kfree(b);
...
void cvi_sc_device_run(void *priv, bool is_tile, bool is_work_on_r_tile, u8 grp_id)
{
	...
	list_buf = cvi_sc_next_buf(sdev, grp_id, idev->is_online_from_isp);
	if (list_buf == NULL) {
	...
	if (!is_tile || !is_work_on_r_tile) {
		if (idev->is_online_from_isp)
			cvi_sc_update(sdev, &sdev->ol_vpss_chn_cfg[grp_id]);
		else
			cvi_sc_update(sdev, &sdev->vpss_chn_cfg);
	}
}
...
static int cvi_sc_open(struct file *file)
{
	...
	if (v4l2_fh_is_singular_file(file)) {
		...
		sclr_set_cfg(sdev->dev_idx, false, false, false, true, false);
...
static int cvi_sc_release(struct file *file)
{
	...
	cvi_sc_buf_remove_all(sdev, 0, true);
	cvi_sc_buf_remove_all(sdev, 1, true);
	cvi_sc_buf_remove_all(sdev, 0, false);
	dprintk(VIP_INFO, "-\n");
	return 0;
}
	...
	struct v4l2_ext_controls *vc)
{
	...
	for (i = 0; i < vc->count; ++i) {
		...
		case V4L2_CID_DV_VIP_SC_SET_RGN_LUT: {
			void *addr;
			struct cvi_rgn_lut_cfg cfg;

			if (copy_from_user(&cfg, ext_ctrls[i].ptr, sizeof(cfg))) {
				dprintk(VIP_ERR, "ioctl-%#x, rgn_cfg copy_from_user failed.\n", ext_ctrls[i].id);
				rc = -ENOMEM;
				break;
			}

			// lut palette is ARGB4444 format, bpp is 16
			addr = kmalloc(cfg.lut_length * 2, GFP_ATOMIC);
			if (addr == NULL) {
				dprintk(VIP_ERR, "sc_lut kmalloc size(%d) failed.\n", cfg.lut_length * 2);
				break;
			}

			if (copy_from_user(addr, (void *)(cfg.lut_addr), cfg.lut_length * 2)) {
				dprintk(VIP_ERR, "ioctl-%#x, lut copy_from_user failed.\n", ext_ctrls[i].id);
				rc = -ENOMEM;
				kfree(addr);
				break;
			}

			// temporarily enable clk for init GOP LUT.
			if (clk_enable(sdev->clk)) {
				dprintk(VIP_ERR, "[ERR][LUT] VIP_CLK enable fail\n");
				kfree((void *)addr);
				break;
			}

			sclr_gop_setup_LUT(sdev->dev_idx, cfg.lut_length, (u16 *)cfg.lut_addr);
		...
		case V4L2_CID_DV_VIP_SC_SET_VPSS_CHN_CFG: {
			...
			if (cfg.is_online)
				sdev->ol_vpss_chn_cfg[cfg.grp_id] = cfg;
			else
				sdev->vpss_chn_cfg = cfg;
			rc = 0;
		}
		break;
		...
		case V4L2_CID_DV_VIP_QBUF: {
			...
			if (buf.is_online) {
				if (buf.index > 1)
					buf.index = 0;
				if (cvi_sc_buf_num(sdev, buf.index, buf.is_online) > 1) {
					dprintk(VIP_ERR, "sc(%d): previous buf_idx(%d) isn't finished yet, ol(%d)\n",
						sdev->dev_idx, buf.index, buf.is_online);
					break;
				}
			} else {
				buf.index = 0;
				if (cvi_sc_buf_num(sdev, buf.index, buf.is_online)) {
					dprintk(VIP_ERR, "sc(%d): previous buf isn't finished yet, ol(%d)\n",
						sdev->dev_idx, buf.is_online);
					break;
				}
			}

			list_buf.buf = buf;
			if (cvi_sc_buf_queue(sdev, &list_buf, buf.is_online))
				break;
			if (buf.is_online) {
				dprintk(VIP_INFO, "sc(%d): num_rdy(%d) buf_idx(%d) num_buf(%d)\n",
					sdev->dev_idx, sdev->ol_num_rdy[buf.index], buf.index, buf.length);
				dprintk(VIP_INFO, "sc(%d): addr(%#llx %#llx %#llx)\n",
					buf.planes[0].addr, buf.planes[1].addr, buf.planes[2].addr);
			} else {
				dprintk(VIP_INFO, "sc(%d): num_rdy(%d) buf_idx(%d) num_buf(%d)\n",
					sdev->dev_idx, sdev->num_rdy, buf.index, buf.length);
				dprintk(VIP_INFO, "sc(%d): addr(%#llx %#llx %#llx)\n",
					buf.planes[0].addr, buf.planes[1].addr, buf.planes[2].addr);
			}
			rc = 0;
		...
		case V4L2_CID_DV_VIP_SC_RELEASE_BUF: {
			struct cvi_sc_buf_cfg cfg;
			...
			cvi_sc_buf_remove_all(sdev, cfg.grp_id, cfg.is_online_from_isp);
			rc = 0;
		}
		break;
...
int sc_create_instance(struct platform_device *pdev)
{
	...
	for (i = 0; i < CVI_VIP_SC_MAX; ++i) {
		...
		INIT_LIST_HEAD(&sdev->rdy_queue);
#endif
		sdev->num_rdy = sdev->ol_num_rdy[0] = sdev->ol_num_rdy[1] = 0;
		INIT_LIST_HEAD(&sdev->rdy_queue);
		INIT_LIST_HEAD(&sdev->ol_rdy_queue[0]);
		INIT_LIST_HEAD(&sdev->ol_rdy_queue[1]);
...
{
	u8 i = 0;
	u8 sc_early_ids[CVI_VIP_IMG_MAX] = {0, 0};
	bool bNotifyEarlyInt[CVI_VIP_IMG_MAX] = {false, false};
	...
	for (i = 0; i < CVI_VIP_SC_MAX; ++i) {
		sdev = &bdev->sc_vdev[i];
		idev = &bdev->img_vdev[sdev->img_src];

		if (idev->is_online_from_isp)
			chn_cfg = &sdev->ol_vpss_chn_cfg[idev->job_grp];
		else
			chn_cfg = &sdev->vpss_chn_cfg;
		// check if any early int comes
		if ((i == 0 && intr_status.b.scl0_line_target_hit == 1) ||
		    (i == 1 && intr_status.b.scl1_line_target_hit == 1) ||
		    (i == 2 && intr_status.b.scl2_line_target_hit == 1) ||
		    (i == 3 && intr_status.b.scl3_line_target_hit == 1)) {
			if (chn_cfg->low_delay_cfg.enable)
				sc_early_ids[sdev->img_src] |= (1 << i);
		}

		if ((i == 0 &&
		    (intr_status.b.scl0_frame_end == 0 || sdev->is_cmdq == true)) ||
		    (i == 1 && intr_status.b.scl1_frame_end == 0) ||
		    (i == 2 && intr_status.b.scl2_frame_end == 0) ||
		    (i == 3 && intr_status.b.scl3_frame_end == 0)) {
			// if there comes early int and sc frame_end not ready,
			// need to notify user early int
			if (!idev->is_tile || idev->is_work_on_r_tile) {
				if (atomic_read(&sdev->job_state) != CVI_VIP_IDLE)
					bNotifyEarlyInt[sdev->img_src] = true;
			}
			continue;
		}
		...
		if (cvi_sc_buf_remove(sdev, sdev->job_grp, bdev->img_vdev[sdev->img_src].is_online_from_isp))
...
sc_job_finish:
	...
	for (i = 0; i < CVI_VIP_IMG_MAX; ++i) {
		...
		// if there comes early int and img frame_end not ready,
		// need to notify user early int
		if (idev->job_flags & TRANS_RUNNING)
			bNotifyEarlyInt[i] = true;

		if (bNotifyEarlyInt[i] && sc_early_ids[i] != 0)
			// notify user early int
			cvi_img_v4l2_event_queue(idev, V4L2_EVENT_CVI_VIP_VPSS_EARLY_INT,
				idev->job_grp, sc_early_ids[i]);
	}
}
...
void cvi_sc_update(struct cvi_sc_vdev *sdev, const struct cvi_vpss_chn_cfg *chn_cfg)
{
	...
	if (chn_cfg->low_delay_cfg.enable)
		cfg->line_cnt = chn_cfg->low_delay_cfg.lineCnt;
