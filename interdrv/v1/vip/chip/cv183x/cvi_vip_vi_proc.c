 ...
 *
 * File Name: cvi_vip_vi_proc.c
 * Description: video pipeline isp engine info
 */
...
static int _vi_proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "\n-------------------------------VI MODE------------------------------------------\n");
	seq_puts(m, "\tDevID\tPreraw\t\tPostraw\t\tScaler\n");
	for (i = 0; i < VI_MAX_DEV_NUM; i++) {
		if (pviProcCtx->isDevEnable[i]) {
			seq_printf(m, "\t%3d\t%7s\t\t%7s\t\t%7s\n", i,
				vdev->ctx.isp_pipe_cfg[i].is_offline_preraw ? "offline" : "online",
				vdev->ctx.is_offline_postraw ? "offline" : "online",
				vdev->ctx.is_offline_scaler ? "offline" : "online");
		}
	}
	...
	for (i = 0; i < VI_MAX_DEV_NUM; i++) {
		if (pviProcCtx->isDevEnable[i]) {
			...
			seq_printf(m, "\t%4s\t%4s\t%3s\n", o,
				(pviProcCtx->devAttr[i].enWorkMode == VI_WORK_MODE_1Multiplex) ? "1MUX" : "Other",
	...
	seq_puts(m, "\n-------------------------------VI CHN ATTR1-------------------------------------\n");
	seq_puts(m, "\tDevID\tChnID\tWidth\tHeight\tMirror\tFlip\tSrcFRate\tDstFRate\tPixFmt\tVideoFmt\n");
	for (i = 0; i < VI_MAX_DEV_NUM; i++) {
		if (pviProcCtx->isDevEnable[i]) {
			for (chn = 0, j = 0; j < i; j++) {
			...
			for (j = 0; j < pviProcCtx->devAttr[i].chn_num; j++, chn++) {
				...
				seq_printf(m, "\t%3d\t%3d\t%4d\t%4d\t%3s\t%2s\t%4d\t\t%4d\t\t%3s\t%6s\n", i, j,
