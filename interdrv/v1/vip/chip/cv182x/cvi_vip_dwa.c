...
static void cvi_dwa_device_run(void *priv)
{
	...
	switch (ctx->op) {
	case CVI_DWA_OP_ROT_90:
		cfg.dst_mode = LDC_DST_ROT_270;
		break;
	case CVI_DWA_OP_ROT_270:
	...
	cfg.map_base = ctx->mesh_id_addr;
	cfg.bgcolor = ctx->bgcolor;
	cfg.src_width  = ALIGN(src_fmt->w, 64);
	cfg.src_height = ALIGN(src_fmt->h, 64);
	cfg.ras_width = cfg.src_width;
	cfg.ras_height = cfg.src_height;
	if (cfg.map_base == 0) {
		cfg.map_bypass = true;
		cfg.src_xstart = 0;
		cfg.src_xend = src_fmt->w;
	} else {
		cfg.map_bypass = false;
		if (ctx->op == CVI_DWA_OP_ROT_270) {
			cfg.src_xstart = 0;
			cfg.src_xend = src_fmt->w;
		} else {
			cfg.src_xstart = cfg.ras_width - src_fmt->w;
			cfg.src_xend = cfg.ras_width;
		}
	}

	// TODO: define start/end by crop

	cfg.src_y_base = src_buf->vb2_buf.planes[0].m.userptr;
	cfg.dst_y_base = dst_buf->vb2_buf.planes[0].m.userptr;
	if (num_of_plane == 2) {
		cfg.src_c_base = src_buf->vb2_buf.planes[1].m.userptr;
		cfg.dst_c_base = dst_buf->vb2_buf.planes[1].m.userptr;
	}

	dprintk(VIP_DBG, "update size src(%d %d)\n",
		cfg.src_width, cfg.src_height);
	dprintk(VIP_DBG, "update src-buf: 0x%lx-0x%lx\n",
		src_buf->vb2_buf.planes[0].m.userptr,
		src_buf->vb2_buf.planes[1].m.userptr);
	dprintk(VIP_DBG, "update dst-buf: 0x%lx-0x%lx\n",
...
static int cvi_dwa_open(struct file *file)
{
	...
	ctx->fmt = cvi_vip_get_format(V4L2_PIX_FMT_NV21M);
	...
	struct v4l2_ext_controls *vc)
{
	...
	for (i = 0; i < vc->count; ++i) {
		...
		case V4L2_CID_DV_VIP_DWA_SET_OP:
			ctx->op = ext_ctrls[i].value;
			rc = 0;
			break;
	    ...
	    struct v4l2_format *f)
{
	...
	if ((mp->pixelformat != V4L2_PIX_FMT_NV12M) && (mp->pixelformat != V4L2_PIX_FMT_NV21M)
	 && (mp->pixelformat != V4L2_PIX_FMT_GREY)) {
		dprintk(VIP_ERR, "Invalid format(%s), NV12 or NV21 or Y-only .\n",
