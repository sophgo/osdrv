...
static int rgn_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < RGN_MAX_NUM; ++i) {
		for (j = 0; j < prgnCtx[i].attachChnNum; ++j) {
			if (prgnCtx[i].stRegion.enType == OVERLAY_RGN && prgnCtx[i].bCreated && prgnCtx[i].bUsed) {
				seq_printf(m, "%7s%3d%10d%10s%10d%10d%10s%10d%10d%10d\n",
					...
					prgnCtx[i].Handle,
					prgnCtx[i].stRegion.enType,
					MOD_STRING[prgnCtx[i].stChns[j].enModId],
					prgnCtx[i].stChns[j].s32DevId,
					prgnCtx[i].stChns[j].s32ChnId,
					(prgnCtx[i].stChnsAttr[j].bShow) ? "Y" : "N",
					prgnCtx[i].stChnsAttr[j].unChnAttr.stOverlayChn.stPoint.s32X,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stOverlayChn.stPoint.s32Y,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stOverlayChn.u32Layer);
			}
		}
	}
	...
	for (i = 0; i < RGN_MAX_NUM; ++i) {
		for (j = 0; j < prgnCtx[i].attachChnNum; ++j) {
			if (prgnCtx[i].stRegion.enType == COVER_RGN && prgnCtx[i].bCreated && prgnCtx[i].bUsed
				&& prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.enCoverType == AREA_RECT) {
				seq_printf(m, "%7s%3d%10d%10s%10d%10d%10s\n%10d%10d%10d%10d%10X%10d%10s\n",
					...
					prgnCtx[i].stChns[j].s32ChnId,
					(prgnCtx[i].stChnsAttr[j].bShow) ? "Y" : "N",
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.stRect.s32X,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.stRect.s32Y,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.stRect.u32Width,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.stRect.u32Height,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.u32Color,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.u32Layer,
					(prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverChn.enCoordinate == RGN_ABS_COOR) ?
						"ABS" : "RATIO");
			}
		}
	}
	...
	for (i = 0; i < RGN_MAX_NUM; ++i) {
		for (j = 0; j < prgnCtx[i].attachChnNum; ++j) {
			if (prgnCtx[i].stRegion.enType == COVEREX_RGN && prgnCtx[i].bCreated && prgnCtx[i].bUsed
				&& prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.enCoverType == AREA_RECT) {
				seq_printf(m, "%7s%3d%10d%10s%10d%10d%10s\n%10d%10d%10d%10d%10X%10d\n",
					...
					prgnCtx[i].stChns[j].s32ChnId,
					(prgnCtx[i].stChnsAttr[j].bShow) ? "Y" : "N",
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.stRect.s32X,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.stRect.s32Y,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.stRect.u32Width,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.stRect.u32Height,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.u32Color,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stCoverExChn.u32Layer);
			}
		}
	}
		...
		for (j = 0; j < prgnCtx[i].attachChnNum; ++j) {
			if (prgnCtx[i].stRegion.enType == OVERLAYEX_RGN && prgnCtx[i].bCreated && prgnCtx[i].bUsed) {
				seq_printf(m, "%7s%3d%10d%10s%10d%10d%10s%10d%10d%10d\n",
					...
					prgnCtx[i].stChns[j].s32ChnId,
					(prgnCtx[i].stChnsAttr[j].bShow) ? "Y" : "N",
					prgnCtx[i].stChnsAttr[j].unChnAttr.stOverlayExChn.stPoint.s32X,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stOverlayExChn.stPoint.s32Y,
					prgnCtx[i].stChnsAttr[j].unChnAttr.stOverlayExChn.u32Layer);
			}
		}
	}
