			       ...
			       struct param_sublvds *param)
{
	...
	if (!ctx->mac_num) {
		...
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      LVDS_F0, SENSOR0_MODE,
				      1);
		/* DPHY bit mode select */
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      LVDS_F0, SLVDS0_BIT_MODE,
				      param->fmt);
		/* DPHY endian mode select */
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      LVDS_F0, SLVDS0_INV_EN,
				      param->wrap_endian == CIF_SLVDS_ENDIAN_MSB);
	} else {
		/* DPHY sensor mode select */
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      LVDS_F8, SENSOR1_MODE,
				      1);
		/* DPHY bit mode select */
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      LVDS_F8, SLVDS1_BIT_MODE,
				      param->fmt);
		/* DPHY endian mode select */
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      LVDS_F8, SLVDS1_INV_EN,
				      param->wrap_endian == CIF_SLVDS_ENDIAN_MSB);
	}
}
			...
			enum ttl_vi_from_e vi, enum ttl_vi_func_e func, uint32_t pad)
{
	...
	if (vi == FROM_VI2) {
		...
		value = pad << offset;
		mask = 0x7 << offset;
		reg = CIF_RD_REG(mac_top, REG_SENSOR_MAC_1C4D_T, REG_74);
		reg &= ~mask;
		reg |= value;
		CIF_WR_REG(mac_top, REG_SENSOR_MAC_1C4D_T,
				    REG_74,
				    reg);
		return;
	}
	...
	if (func <= VI_FUNC_HDE) {
		reg = CIF_RD_REG(mac_top, REG_SENSOR_MAC_1C4D_T, REG_60);
		reg &= ~mask;
		reg |= value;
		CIF_WR_REG(mac_top, REG_SENSOR_MAC_1C4D_T,
				    REG_60,
		...
		reg = CIF_RD_REG(mac_top, REG_SENSOR_MAC_1C4D_T, REG_64);
		reg &= ~mask;
		reg |= value;
		CIF_WR_REG(mac_top, REG_SENSOR_MAC_1C4D_T,
				    REG_64,
		...
		reg = CIF_RD_REG(mac_top, REG_SENSOR_MAC_1C4D_T, REG_68);
		reg &= ~mask;
		reg |= value;
		CIF_WR_REG(mac_top, REG_SENSOR_MAC_1C4D_T,
				    REG_68,
		...
		reg = CIF_RD_REG(mac_top, REG_SENSOR_MAC_1C4D_T, REG_6C);
		reg &= ~mask;
		reg |= value;
		CIF_WR_REG(mac_top, REG_SENSOR_MAC_1C4D_T,
				    REG_6C,
		...
		reg = CIF_RD_REG(mac_top, REG_SENSOR_MAC_1C4D_T, REG_70);
		reg &= ~mask;
		reg |= value;
		CIF_WR_REG(mac_top, REG_SENSOR_MAC_1C4D_T,
				    REG_70,
				 ...
				 uint32_t on)
{
	...
	/* 1822 not support invert the HDR */
//	CIF_WR_BITS(mac_top, REG_SENSOR_MAC_1C4D_T,
//			     REG_00, CSI_HDR_INV,
//			     1);
	CIF_WR_BITS(mac_csi, REG_CSI_CTRL_TOP_T,
			      ...
			      struct cif_param *param, uint32_t on)
{
	...
	case CIF_TYPE_CSI:
		...
		if (!ctx->mac_num) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				    REG_24, DESKEW_LANE_EN, 0);
		} else {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				    REG_20, DESKEW_LANE_EN_P1, 0);
		}
		if (on) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			...
			if (!ctx->mac_num) {
				CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
					    REG_24, DESKEW_LANE_EN,
					    (1 << cfg->csi.lane_num) - 1);
			} else {
				CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
					    REG_20, DESKEW_LANE_EN_P1,
					    (1 << cfg->csi.lane_num) - 1);
			}
		}
		break;
	case CIF_TYPE_SUBLVDS:
		...
		if (!ctx->mac_num) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				    LVDS_F0, SLVDS0_LANE_EN, 0);
		} else {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				    LVDS_F8, SLVDS1_LANE_EN, 0);
		}
		if (on) {
			udelay(20);
			/* lane enable */
			if (!ctx->mac_num) {
				CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
					    LVDS_F0, SLVDS0_LANE_EN,
					    (1 << cfg->sublvds.lane_num) - 1);
			} else {
				CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
					    LVDS_F8, SLVDS1_LANE_EN,
					    (1 << cfg->sublvds.lane_num) - 1);
			}
		}
		break;
...
void cif_set_csi_format_force(struct cif_ctx *ctx, uint32_t on, enum csi_fmt_e fmt)
{
	uintptr_t mac_csi = ctx->mac_phys_regs[CIF_MAC_BLK_ID_CSI];

	CIF_WR_BITS(mac_csi, REG_CSI_CTRL_TOP_T,
			REG_00, CSI_FORMAT_FRC, 0);

	if (on) {
		CIF_WR_BITS(mac_csi, REG_CSI_CTRL_TOP_T,
				REG_00, CSI_FORMAT_FRC, 1);
		CIF_WR_BITS(mac_csi, REG_CSI_CTRL_TOP_T,
				REG_00, CSI_FORMAT_SET, 1 << fmt);
	}
}
	...
	case CIF_LANE_CLK:
		if (!ctx->mac_num) {
			...
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      ...
				      CIF_RD_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
						REG_04, MIPIRX_SEL_CLK_CHANNEL) | 1 << select);
			/* PHYD clock select */
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_CK_SEL,
				      select);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_CK_PNSWAP,
				      pn_swap);
		} else {
			/* Enable dual mode */
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, DUAL_SENSOR_MODE,
				      1);
			/* PHYA clock select */
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_04, MIPIRX_SEL_CLK_CHANNEL,
			...
			/* PHYD clock select */
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, CSI_LANE_CK_SEL_P1,
				      select % 3);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, CSI_LANE_CK_PNSWAP_P1,
				      pn_swap);
		}
		break;
	case CIF_LANE_0:
		if (!ctx->mac_num) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D0_SEL,
				      select);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D0_PNSWAP,
				      pn_swap);
		} else {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, CSI_LANE_D0_SEL_P1,
				      select % 3);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, CSI_LANE_D0_PNSWAP_P1,
				      pn_swap);
		}
		break;
	case CIF_LANE_1:
		if (!ctx->mac_num) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D1_SEL,
				      select);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D1_PNSWAP,
				      pn_swap);
		} else {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, CSI_LANE_D1_SEL_P1,
				      select % 3);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_20, CSI_LANE_D1_PNSWAP_P1,
				      pn_swap);
		}
		break;
	case CIF_LANE_2:
		if (!ctx->mac_num) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D2_SEL,
				      select);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D2_PNSWAP,
				      pn_swap);
		}
		break;
	case CIF_LANE_3:
		if (!ctx->mac_num) {
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D3_SEL,
				      select);
			CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
				      REG_24, CSI_LANE_D3_PNSWAP,
				      pn_swap);
		}
		break;
	default:
		      ...
		      enum phy_lane_id_e lane, enum cif_clk_edge_e edge)
{
	...
	case CIF_PHY_LANE_0:
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			      REG_50, AD_D0_CLK_INV,
			      edge);
		break;
	case CIF_PHY_LANE_1:
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			      REG_50, AD_D1_CLK_INV,
			      edge);
		break;
	case CIF_PHY_LANE_2:
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			      REG_50, AD_D2_CLK_INV,
			      edge);
		break;
	case CIF_PHY_LANE_3:
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			      REG_50, AD_D3_CLK_INV,
			      edge);
		break;
	case CIF_PHY_LANE_4:
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			      REG_50, AD_D4_CLK_INV,
			      edge);
		break;
	case CIF_PHY_LANE_5:
		CIF_WR_BITS(wrap_top, REG_CSI_DPHY_4LANE_WRAP_T,
			      REG_50, AD_D5_CLK_INV,
			      edge);
		break;
	default:
		break;
