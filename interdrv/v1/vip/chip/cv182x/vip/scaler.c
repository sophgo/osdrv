 ...
 *
 * File Name: scaler.c
 * Description: scaler's register access control
 */
...
#if 0
		void *sc_clk_reg = ioremap(0x3002008, 4);
		u32 sc_pll = ioread32(vip_clk_reg);
...
int sclr_dsi_config(u8 lane_num, enum sclr_dsi_fmt fmt, u16 width)
{
	...
	val = 0x80 << 16 | UPPER(width * bit_depth[fmt], 3);
...
void sclr_engine_cmdq_rgnex(struct sclr_rgnex_cfg *cfgs, u8 cnt, uintptr_t cmdq_addr)
{
	...
	for (i = 0; i < cnt; ++i) {
		...
		if (cfg->fmt == SCL_FMT_YUV420 || cfg->fmt == SCL_FMT_NV12 || cfg->fmt == SCL_FMT_NV21) {
			...
			pre_fetch_raw = 2 * (cfg->rgnex_rect.w + 1) / 16 - 1;
			fifo_rd_th_y = MIN(pre_fetch_raw, 32);
			fifo_pr_th_y = fifo_rd_th_y / 2;
			fifo_rd_th_c = fifo_rd_th_y;
			fifo_pr_th_c = fifo_pr_th_y;
			cmdQ_set_package(&cmd_start[cmd_idx++].reg,
				 REG_SCL_CMDQ_TOP_BASE + REG_SCL_IMG_FIFO_THR(img_inst),
				 ((fifo_pr_th_c << 24) & 0xff000000) |
				 ((fifo_rd_th_c << 16) & 0x00ff0000) |
				 ((fifo_pr_th_y << 8) & 0x0000ff00) |
				 (fifo_rd_th_y & 0x000000ff));
		}
...
void vip_axi_realtime_fab_priority(void)
{
	...
	vip_pr(CVI_INFO, "vip_subsys patch applied.\n");
...
void _ddr_ctrl_patch(bool enable)
{
	...
	if (ddr_base) {
		if (enable) {
			vip_pr(CVI_INFO, "ddr patch applied.\n");
			iowrite32(0x01110e00, ddr_base);
			iowrite32(0x0, ddr_base + 0x04);
		} else {
			vip_pr(CVI_INFO, "ddr patch exit.\n");
