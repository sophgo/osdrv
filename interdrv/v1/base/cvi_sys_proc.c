 ...
 *
 * File Name: cvi_sys_proc.c
 * Description: sys proc driver code
 ...
 *	sys proc functions
 *************************************************************************/
static bool _is_fisrt_level_bind_node(struct BIND_NODE_S *node)
{
	int i, j;
	struct BIND_NODE_S *bindNodes;

	bindNodes = (struct BIND_NODE_S *)(shared_mem + BASE_BIND_INFO_OFFSET);
...
static struct BIND_NODE_S *_find_next_bind_node(const struct MMF_CHN_S *pstSrcChn)
{
	int i;
	struct BIND_NODE_S *bindNodes;
...
{
	int i, j, k;
	struct MMF_VERSION_S *mmfVersion;
	struct BIND_NODE_S *bindNodes, *nextBindNode;
	struct MMF_CHN_S *first, *second, *third;

	mmfVersion = (struct MMF_VERSION_S *)(shared_mem + BASE_VERSION_INFO_OFFSET);
	bindNodes = (struct BIND_NODE_S *)(shared_mem + BASE_BIND_INFO_OFFSET);
...
int sys_proc_init(struct proc_dir_entry *_proc_dir, void *shm)
{
	...
	if (proc_create_data(SYS_PROC_NAME, SYS_PROC_PERMS, _proc_dir, &_sys_proc_fops, NULL) == NULL) {
		pr_err("sys proc creation failed\n");
		rc = -1;
	}
