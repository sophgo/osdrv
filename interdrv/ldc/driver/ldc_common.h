...
#else
#define LDC_IDLE_WAIT_TIMEOUT_MS    (1000*20)
#define LDC_SYNC_IO_WAIT_TIMEOUT_MS (1000*100)
#define LDC_EOF_WAIT_TIMEOUT_MS     (1000*5)
#endif
...
struct ldc_task {
	struct osal_list_head node;// add to ldc_job task_list
	...
	rotation_e rotation;
	osal_atomic state;//ldc_task_state
	int tsk_id;
	//char coreid;
};
...
struct ldc_job {
	...
	osal_atomic job_state; //ldc_job_state
	struct osal_list_head node;//add to ldc_vdev job_list
	struct osal_list_head task_list;
	osal_atomic task_num;
...
struct ldc_vb_doneq {
	osal_semaphore sem;
	struct osal_list_head doneq;
	osal_spinlock lock;
};

struct ldc_vb_done {
	struct osal_list_head node;//add to struct ldc_vb_doneq doneq
