...
static int cvi_wiegand_rx_consult(struct cvi_wiegand_device *ndev, struct wgn_rx_timeout timeout)
{
	if (timeout.timeout_flag) {
		if (wait_event_interruptible_timeout(wq, flag == 'y', msecs_to_jiffies(timeout.timeout_ms)) == 0)
			return 1;
	} else {
		wait_event_interruptible(wq, flag == 'y');
	}
...
static int cvi_wiegand_rx_start(struct cvi_wiegand_device *ndev, unsigned long arg)
{
	uint32_t value;
...
static long cvi_wiegand_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_WGN_TX:
		...
		break;
	case IOCTL_WGN_RX_START:
		ret = cvi_wiegand_rx_start(ndev, arg);
		break;
	case IOCTL_WGN_RX:
		if (copy_from_user(&timeout, (void *)arg, sizeof(struct wgn_rx_timeout))) {
			pr_err("copy_from_user failed.\n");
			break;
		}
		ret = cvi_wiegand_rx_consult(ndev, timeout);
		break;
