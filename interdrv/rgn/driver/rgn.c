...
static osal_mutex hdlslock, g_rgnlock, g_rgnhashlock;
static osal_hash *rgn_hash;
...
{
	bool is_found = false;
	struct rgn_ctx *obj = NULL;

#if 0 //Linux Hashmap for print every element
	unsigned int  bkt;

	TRACE_RGN(RGN_INFO, "_rgn_hash_find find handle(%d), del(%d)\n", handle, del);
	hash_for_each(rgn_hash, bkt, obj, node) {
		TRACE_RGN(RGN_INFO, "rgn_hash: handle(%d), created:(%d), region.type:(%d)\n"
	...
	if (del) {
		obj = osal_hash_remove(rgn_hash, handle);
	} else {
		obj = osal_hash_get(rgn_hash, handle);
	}
...
static int check_rgn_handle(struct rgn_ctx **ctx, rgn_handle handle)
{
	if (!_rgn_hash_find(handle, ctx, false)) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) is non-existent.\n", handle);
...
int32_t _rgn_init(void)
{
	...
	if (!rgn_hash) {
		TRACE_RGN(RGN_ERR, "osal_hash_create fail!\n");
	}

	osal_memset(rgn_prc_ctx, 0, sizeof(rgn_prc_ctx));
	rgn_num = 0;
	osal_mutex_init(&hdlslock);
	osal_mutex_init(&g_rgnlock);
	osal_mutex_init(&g_rgnhashlock);
	TRACE_RGN(RGN_INFO, "-\n");
	return 0;
}

static int _hash_cb_rgn_destory(long long unsigned int key, void *value, void *context)
{
	struct rgn_ctx *obj = (struct rgn_ctx *)value;
...
int32_t _rgn_exit(void)
{
	...
	osal_hash_destroy(rgn_hash);
	rgn_hash = NULL;
	osal_mutex_unlock(&g_rgnhashlock);

	osal_memset(rgn_prc_ctx, 0, sizeof(rgn_prc_ctx));
	rgn_num = 0;
	osal_mutex_destroy(&hdlslock);
	osal_mutex_destroy(&g_rgnlock);
	osal_mutex_destroy(&g_rgnhashlock);
	TRACE_RGN(RGN_INFO, "-\n");
	return 0;
}
...
unsigned int _rgn_proc_get_idx(rgn_handle hhandle)
{
	...
	if (i == RGN_MAX_NUM)
		TRACE_RGN(RGN_ERR, "cannot find corresponding rgn in rgn_prc_ctx, handle: %d\n", hhandle);
...
static inline int _rgn_get_bytesperline(pixel_format_e pixel_format, unsigned int width, unsigned int *bytesperline)
{
	...
	case PIXEL_FORMAT_FONT:
		*bytesperline = (width + 7) >> 3;
		break;
	default:
		TRACE_RGN(RGN_ERR, "not supported pxl-fmt(%d).\n", pixel_format);
...
int _rgn_insert(rgn_handle hdls[], unsigned char size, rgn_handle hdl)
{
	...
	if (ctx->canvas_info[ctx->canvas_idx].compressed
		...
		TRACE_RGN(RGN_ERR, "cannot insert hdl(%d), only allow one ow when odec on\n",
			hdl);
		return ERR_RGN_NOT_PERM;
	}
	if (_rgn_get_rect(ctx, &rect_new) != 0) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) can't get rect.\n", hdl);
		return -1;
	}
	if (hdls[size - 1] != RGN_INVALID_HANDLE) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) rgn's count is full.\n", hdl);
		return -1;
	}
...
int _rgn_ex_insert(rgn_handle hdls[], unsigned char size, rgn_handle hdl)
{
	...
	if (hdls[size - 1] != RGN_INVALID_HANDLE) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) rgn_ex's count is full.\n", hdl);
		return -1;
	}

	if (_rgn_get_layer(ctx, &layer_new) != 0) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) can't get layer.\n", hdl);
		return -1;
	}
...
static int _rgn_update_cover_canvas(struct rgn_ctx *ctx, const rgn_chn_attr_s *pchn_attr)
{
	...
	if (buf_len > ctx->ion_len) {
		...
		if (base_ion_alloc(&ctx->canvas_info[0].phy_addr, (void **)&ctx->canvas_info[0].virt_addr,
						  (uint8_t *)"rgn_canvas", ctx->ion_len, false) != 0) {
			TRACE_RGN(RGN_ERR, "Region(%d) Can't acquire ion for Canvas.\n", ctx->handle);
			...
			rgn_type_e type, unsigned char cmpr)
{
	...
	if (pchn->mod_id == ID_VPSS) {
		...
		if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_SET_RGN_HDLS, &rgn_hdls_arg) != 0) {
			TRACE_RGN(RGN_ERR, "VPSS_CB_SET_RGN_HDLS is failed\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
		...
		if (type == OVERLAY_RGN || type == COVER_RGN) {
			...
			if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_SET_RGN_CFG, &rgn_cfg_arg) != 0) {
				TRACE_RGN(RGN_ERR, "VPSS_CB_SET_RGN_CFG is failed\n");
				return ERR_RGN_ILLEGAL_PARAM;
			}
			...
			if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_SET_COVEREX_CFG, &rgn_coverex_cfg_arg) != 0) {
				TRACE_RGN(RGN_ERR, "VPSS_CB_SET_COVEREX_CFG is failed\n");
				return ERR_RGN_ILLEGAL_PARAM;
			}
			...
			if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_SET_RGNEX_CFG, &rgnex_cfg_arg) != 0) {
				TRACE_RGN(RGN_ERR, "VPSS_CB_SET_RGNEX_CFG is failed\n");
				return ERR_RGN_ILLEGAL_PARAM;
			}
			...
			if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_SET_MOSAIC_CFG, &rgn_mosaic_cfg_arg) != 0) {
				TRACE_RGN(RGN_ERR, "VPSS_CB_SET_MOSAIC_CFG is failed\n");
				return ERR_RGN_ILLEGAL_PARAM;
			}
		}
	} else {
		TRACE_RGN(RGN_ERR, "Unsupported mod_id(%d)\n", pchn->mod_id);
		return ERR_RGN_ILLEGAL_PARAM;
	}
...
static int _rgn_set_hw_cfg(rgn_handle hdls[], unsigned char size, mmf_chn_s *pchn, rgn_type_e type, unsigned char cmpr)
{
	...
	for (i = 0; i < size; ++i) {
		...
		case PIXEL_FORMAT_FONT:
			...
			cfg.invert_param[j].invert_color =
				ctx->chn_attr.unchn_attr.overlay_chn.invert_color;
			++j;
			break;
		case PIXEL_FORMAT_ARGB_1555:
		default:
		...
		if (ctx->canvas_info[ctx->canvas_idx].compressed) {
			...
			cfg.odec.canvas_mutex_lock = (uintptr_t)&ctx->rgn_canvas_q_lock;
	...
	if (ret != 0) {
		TRACE_RGN(RGN_ERR, "_rgn_update_hw_cfg_to_module failed\n");
	}
...
static int _rgn_mosaic_set_hw_cfg(rgn_handle hdls[], unsigned char size, mmf_chn_s *pchn, rgn_type_e type)
{
	...
	if (buf_size > ion_buf->buf_len) {
		...
		if (base_ion_alloc((uint64_t *)&phy_addr, &virt_addr, (uint8_t *)"mosaic_canvas", buf_size, false) != 0) {
			TRACE_RGN(RGN_ERR, "Region(%d) Can't acquire ion for Canvas.\n", ctx->handle);
			return ERR_RGN_NOBUF;
		}
	...
	osal_memset(virt_addr, 0, buf_size);
	...
	for (i = 0; i < rgn_idx; ++i) {
		...
		for (j = 0; j < (rect[i].height / grid_size); ++j) {
			for (k = 0; k < (rect[i].width / grid_size); ++k) {
				...
				tmp = (unsigned char)osal_get_random_int();
...
static int _rgn_update_hw(struct rgn_ctx *ctx, enum rgn_op op)
{
	...
	osal_mutex_lock(&hdlslock);
	ret = _rgn_get_chn_info(ctx, hdls, &size);
	if (ret != 0) {
		osal_mutex_unlock(&hdlslock);
		return ret;
	}
	...
	if (op == RGN_OP_INSERT) {
		...
		if (ret != 0) {
			osal_mutex_unlock(&hdlslock);
			TRACE_RGN(RGN_ERR, "rgn_handle(%d) not at CHN(%s-%d-%d).\n", ctx->handle
	...
	if (ctx->region.type == OVERLAY_RGN || ctx->region.type == COVER_RGN) {
		...
		if (ret != 0) {
			TRACE_RGN(RGN_ERR, "_rgn_set_hw_cfg failed\n");
			osal_mutex_unlock(&hdlslock);
			return ret;
		}
	} else if (ctx->region.type == COVEREX_RGN) {
		...
		if (ret != 0) {
			TRACE_RGN(RGN_ERR, "_rgn_coverex_set_hw_cfg failed\n");
			osal_mutex_unlock(&hdlslock);
			return ret;
		}
	} else if (ctx->region.type == OVERLAYEX_RGN) {
		...
		if (ret != 0) {
			TRACE_RGN(RGN_ERR, "_rgn_ex_set_hw_cfg failed\n");
			osal_mutex_unlock(&hdlslock);
			return ret;
		}
	} else if (ctx->region.type == MOSAIC_RGN) {
		...
		if (ret != 0) {
			TRACE_RGN(RGN_ERR, "_rgn_mosaic_set_hw_cfg failed\n");
			osal_mutex_unlock(&hdlslock);
			return ret;
		}
	} else {
	...
	if (ctx->canvas_info[ctx->canvas_idx].phy_addr && ctx->canvas_info[ctx->canvas_idx].virt_addr)
...
int _rgn_check_chn_attr(rgn_handle handle, const mmf_chn_s *pchn, const rgn_chn_attr_s *pchn_attr)
{
	...
	struct rgn_ctx *ctx;
	pixel_format_e pixel_format = PIXEL_FORMAT_ARGB_8888;
	overlay_invert_color_s invert_color = {0};

	check_rgn_handle(&ctx, handle);

	if ((pchn_attr->type == OVERLAYEX_RGN || pchn_attr->type == MOSAIC_RGN) &&
			(pchn->mod_id != ID_VPSS)) {
		TRACE_RGN(RGN_ERR, "overlayex/mosaic only rgn support!\n");
		return ERR_RGN_NOT_SUPPORT;
	}
	...
	if (pchn->mod_id == ID_VPSS) {
		...
		if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_GET_CHN_SIZE, &rgn_chn_arg)) {
			TRACE_RGN(RGN_ERR, "VPSS_CB_GET_CHN_SIZE failed!\n");
	...
	if (pchn_attr->type == COVEREX_RGN) {
		if (pchn_attr->unchn_attr.cover_ex_chn.cover_type != AREA_RECT) {
			TRACE_RGN(RGN_ERR, "AREA_RECT only now.\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
		...
		if (pchn_attr->unchn_attr.cover_chn.coordinate != RGN_ABS_COOR) {
			TRACE_RGN(RGN_ERR, "abs-coordinate only now.\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
		...
		invert_color = pchn_attr->unchn_attr.overlay_chn.invert_color;
		...
		if (pchn_attr->unchn_attr.mosaic_chn.blk_size != MOSAIC_BLK_SIZE_8
			...
			TRACE_RGN(RGN_ERR, "Mosaic only support block size 8/16.\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
	...
	if (rect_rgn.x < 0 || rect_rgn.y < 0) {
		TRACE_RGN(RGN_ERR, "The start point can't be less than 0, start point(%d %d).\n",
			rect_rgn.x, rect_rgn.y);
		return ERR_RGN_ILLEGAL_PARAM;
	}
	if ((rect_rgn.width + rect_rgn.x) > rect_chn.width
		|| (rect_rgn.height + rect_rgn.y) > rect_chn.height) {
		TRACE_RGN(RGN_ERR, "size(%d %d %d %d) larger than chnsize(%d %d).\n",
			rect_rgn.x, rect_rgn.width, rect_rgn.y, rect_rgn.height,
			rect_chn.width, rect_chn.height);
		return ERR_RGN_ILLEGAL_PARAM;
	}

	if (pixel_format == PIXEL_FORMAT_FONT) {
		if (pchn_attr->type != OVERLAY_RGN) {
			TRACE_RGN(RGN_ERR, "font-box only support overlay.\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
		if ((rect_rgn.width % invert_color.inv_col_area.width) ||
			((rect_rgn.width / invert_color.inv_col_area.width) > 32)) {
			TRACE_RGN(RGN_ERR, "rgn.width must be a multiple of inv_col_area.width (1-32).\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
		if (ctx->region.unattr.overlay.compress_info.osd_compress_mode) {
			TRACE_RGN(RGN_ERR, "font-box only support no compression.\n");
			return ERR_RGN_ILLEGAL_PARAM;
		}
		if (!invert_color.inv_col_en)
			TRACE_RGN(RGN_WARN, "Note whether you want to turn on the invert color feature.\n");
	}
...
int rgn_create(rgn_handle handle, const rgn_attr_s *pregion)
{
	...
	if (_rgn_hash_find(handle, &ctx, false)) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) is already existing.\n", handle);
		osal_mutex_unlock(&g_rgnlock);
		return ERR_RGN_EXIST;
	}

	if (rgn_num >= RGN_MAX_NUM) {
		TRACE_RGN(RGN_ERR, "over max rgn number %d\n", RGN_MAX_NUM);
		osal_mutex_unlock(&g_rgnlock);
		return ERR_RGN_NOT_PERM;
	}

	if (pregion->type == OVERLAYEX_RGN) {
		TRACE_RGN(RGN_ERR, "rgn extension only support on rgn rgnex mode.\n");
		osal_mutex_unlock(&g_rgnlock);
		return ERR_RGN_NOT_SUPPORT;
	}
	...
	if (pregion->type == OVERLAY_RGN || pregion->type == OVERLAYEX_RGN) {
		...
		if (canvas_num == 0 || canvas_num > RGN_MAX_BUF_NUM) {
			TRACE_RGN(RGN_ERR, "invalid canvas_num(%d).\n", canvas_num);
			check_fail = 1;
		}
		...
		TRACE_RGN(RGN_ERR, "type(%d) not supported yet.\n", pregion->type);
		osal_mutex_unlock(&g_rgnlock);
		return ERR_RGN_NOT_SUPPORT;
	}

	ctx = osal_kzalloc(sizeof(*ctx), OSAL_GFP_ATOMIC);
	if (ctx == NULL) {
		TRACE_RGN(RGN_ERR, "malloc failed.\n");
		ret = ERR_RGN_NOMEM;
		goto RGN_CTX_MALLOC_FAIL;
	}
	ctx->handle = handle;
	ctx->region = *pregion;
	osal_mutex_init(&ctx->rgn_canvas_q_lock);

	osal_mutex_lock(&g_rgnhashlock);
	osal_hash_add(rgn_hash, handle, ctx);
	osal_mutex_unlock(&g_rgnhashlock);
	rgn_num++;
	...
	hash_for_each(rgn_hash, bkt, cur, node) {
		TRACE_RGN(RGN_INFO, "handle:%d, created:%d, region.type:%d\n"
...
int rgn_destory(rgn_handle handle)
{
	...
	TRACE_RGN(RGN_INFO, "rgn_handle(%d) canvas fmt(%d) size(%d * %d) stride(%d).\n"
	...
	if (!_rgn_hash_find(handle, &ctx, true)) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) remove failed.\n", handle);
		osal_mutex_unlock(&g_rgnlock);
		return ERR_RGN_UNEXIST;
	}
	...
	osal_memset(&rgn_prc_ctx[proc_idx], 0, sizeof(rgn_prc_ctx[proc_idx]));
	rgn_num--;

	osal_mutex_unlock(&g_rgnlock);

	osal_mutex_destroy(&ctx->rgn_canvas_q_lock);
...
int rgn_set_bit_map(rgn_handle handle, const bitmap_s *pbitmap)
{
	...
	if (ctx->canvas_info[0].compressed) {
		TRACE_RGN(RGN_ERR, "This function only suppors OSD_COMPRESS_MODE_NONE.\n");
		return ERR_RGN_NOT_SUPPORT;
	}

	if (ctx->region.type != OVERLAY_RGN && ctx->region.type != OVERLAYEX_RGN) {
		TRACE_RGN(RGN_ERR, "Only Overlay/OverlayEx support. type(%d).\n", ctx->region.type);
		return ERR_RGN_NOT_SUPPORT;
	}

	if (ctx->canvas_get) {
		TRACE_RGN(RGN_ERR, "Need CVI_RGN_UpdateCanvas() first.\n");
		return ERR_RGN_NOT_PERM;
	}

	if (ctx->chn.mod_id == ID_BASE) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) not attached to any chn yet.\n", handle);
	...
	if ((pbitmap->width > rgn_size.width)
	 || (pbitmap->height > rgn_size.height)) {
		TRACE_RGN(RGN_ERR, "size of bitmap (%d * %d) > region (%d * %d).\n"
	...
	if (pbitmap->pixel_format != pixel_format) {
		TRACE_RGN(RGN_ERR, "pxl-fmt of bitmap(%d) != region(%d).\n"
			, pbitmap->pixel_format, pixel_format);
		return ERR_RGN_ILLEGAL_PARAM;
	}

	TRACE_RGN(RGN_INFO, "bitmap size(%d * %d) fmt(%d), region size(%d * %d) fmt(%d).\n"
...
int rgn_attach_to_chn(rgn_handle handle, const mmf_chn_s *pchn, const rgn_chn_attr_s *pchn_attr)
{
	...
	if (ctx->region.type != pchn_attr->type) {
		TRACE_RGN(RGN_ERR, "type(%d) is different with chn type(%d).\n"
			, ctx->region.type, pchn_attr->type);
		return ERR_RGN_ILLEGAL_PARAM;
	}

	if (pchn->mod_id != ID_VPSS) {
		TRACE_RGN(RGN_ERR, "rgn can only be attached to rgn.\n");
		return ERR_RGN_NOT_SUPPORT;
	}

	if (ctx->chn.mod_id != ID_BASE) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) has been attached to CHN(%s-%d-%d).\n", handle
			, sys_get_modname(ctx->chn.mod_id), ctx->chn.dev_id, ctx->chn.chn_id);
		return ERR_RGN_NOT_CONFIG;
	}
	...
	if (pchn_attr->type == COVER_RGN) {
		...
		if (ret != 0) {
			TRACE_RGN(RGN_ERR, "rgn_handle(%d) fill cover failed.\n", handle);
			osal_mutex_unlock(&g_rgnlock);
			return ret;
		}
	}
	...
	if (ctx->region.type == OVERLAY_RGN || ctx->region.type == OVERLAYEX_RGN) {
		...
		if (ctx->canvas_info[0].compressed)
			TRACE_RGN(RGN_INFO, "rgn_handle(%d) compressed canvas size(%d).\n"
		...
		for (i = 0; i < canvas_num; ++i) {
			if (base_ion_alloc(&ctx->canvas_info[i].phy_addr,
							...
							(uint8_t *)"rgn_canvas", ctx->ion_len, true) != 0) {
				TRACE_RGN(RGN_ERR, "Region(%d) Can't acquire ion for Canvas-%d.\n", handle, i);
				ret = ERR_RGN_NOBUF;
...
int rgn_set_display_attr(rgn_handle handle, const mmf_chn_s *pchn, const rgn_chn_attr_s *pchn_attr)
{
	...
	if (ctx->chn.mod_id == ID_BASE
		...
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) is not attached on ModId(%d) dev_id(%d) chn_id(%d)\n",
			handle, pchn->mod_id, pchn->dev_id, pchn->chn_id);
		return ERR_RGN_NOT_CONFIG;
	}
	if (ctx->chn_attr.type != pchn_attr->type) {
		TRACE_RGN(RGN_ERR, "rgn_handle(%d) type not allowed to changed.\n", handle);
		return ERR_RGN_NOT_PERM;
	}
	...
	osal_memcpy(&chn_attr, &ctx->chn_attr, sizeof(rgn_chn_attr_s));
	ctx->chn_attr = rgn_prc_ctx[proc_idx].chn_attr = *pchn_attr;
...
int rgn_update_canvas(rgn_handle handle)
{
	...
	if (!ctx->canvas_get) {
		TRACE_RGN(RGN_ERR, "CVI_RGN_GetCanvasInfo() first.\n");
		return ERR_RGN_NOT_SUPPORT;
	}
...
int rgn_invert_color(rgn_handle handle, mmf_chn_s *pchn, unsigned int *pcolor)
{
	...
	// 	TRACE_RGN(RGN_ERR, "rgn_handle(%d) not attached to chn(%d-%d-%d) yet.\n",
	// 		handle,  pchn->mod_id, pchn->dev_id, pchn->chn_id);
	// 	return ERR_RGN_ILLEGAL_PARAM;
	// }

	// if (ctx->region.type != OVERLAY_RGN && ctx->region.type != OVERLAYEX_RGN) {
	// 	TRACE_RGN(RGN_ERR, "CVI_RGN_Invert_Color only support Overlay/OverlayEx (%d).\n"
	// 		, ctx->region.type);
	// 	return ERR_RGN_NOT_SUPPORT;
	// }

	// if (pchn->mod_id != ID_VPSS) {
	// 	TRACE_RGN(RGN_ERR, "CVI_RGN_Invert_Color GRN did not attach to rgn.\n");
	// 	return ERR_RGN_NOT_SUPPORT;
	// }
	...
	// 	TRACE_RGN(RGN_ERR, "CVI_RGN_Invert_Color inv_col_en hasn't been set\n");
	// 	return ERR_RGN_SYS_NOTREADY;
	// }
...
unsigned short RGN_MAKECOLOR_U16_A(unsigned char a, unsigned char r, unsigned char g,
				   unsigned char b, rgn_component_info_s input_fmt)
{
			...
			rgn_rgbquad_s *input_pixel_table)
{
	...
	if (ppalette->lut_length > 256) {
		TRACE_RGN(RGN_ERR, "RGN_LUT_index(%d) is over maximum(256).\n", ppalette->lut_length);
		return -1;
	}
	...
	for (idx = 0 ; idx < ppalette->lut_length ; idx++) {
		...
		TRACE_RGN(RGN_INFO, "Input Table index(%d) (0x%x).\n", idx, u32pixel);

		pixel = RGN_MAKECOLOR_U16_A(a, r, g, b, convert_info[ppalette->pixel_format]);
		TRACE_RGN(RGN_INFO, "Output data = (0x%x).\n", pixel);
	...
	if (_rgn_call_cb(E_MODULE_VPSS, VPSS_CB_SET_RGN_LUT_CFG, &rgn_lut_arg) != 0) {
		TRACE_RGN(RGN_ERR, "VPSS_CB_SET_LUT_CFG is failed\n");
		return ERR_RGN_ILLEGAL_PARAM;
	}
...
long _rgn_s_ctrl(struct rgn_dev *rdev, struct rgn_ext_control *p)
{
	...
	case RGN_IOCTL_SDK_CTRL: {
		switch (sdk_id) {
		case RGN_SDK_CREATE: {
			if (osal_copy_from_user(&region, p->ptr1, sizeof(rgn_attr_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region create, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_SET_ATTR: {
			if (osal_copy_from_user(&region, p->ptr1, sizeof(rgn_attr_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region set attribute, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_SET_BIT_MAP: {
			if (osal_copy_from_user(&bitmap, p->ptr1, sizeof(bitmap_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region set bitmap, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_ATTACH_TO_CHN: {
			if ((osal_copy_from_user(&chn, p->ptr1, sizeof(mmf_chn_s)) != 0) ||
				(osal_copy_from_user(&chn_attr, p->ptr2, sizeof(rgn_chn_attr_s)) != 0)) {
				TRACE_RGN(RGN_ERR, "Region attach to chn, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_DETACH_FROM_CHN: {
			if (osal_copy_from_user(&chn, p->ptr1, sizeof(mmf_chn_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region detach from chn, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_SET_DISPLAY_ATTR: {
			if ((osal_copy_from_user(&chn, p->ptr1, sizeof(mmf_chn_s)) != 0) ||
				(osal_copy_from_user(&chn_attr, p->ptr2, sizeof(rgn_chn_attr_s)) != 0)) {
				TRACE_RGN(RGN_ERR, "Region set display attribute, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_INVERT_COLOR: {
			if ((osal_copy_from_user(&chn, p->ptr1, sizeof(mmf_chn_s)) != 0) ||
				(osal_copy_from_user(&color, p->ptr2, sizeof(u32)) != 0)) {
				TRACE_RGN(RGN_ERR, "Region invert color, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
		...
		case RGN_SDK_SET_CHN_PALETTE: {
			...
			if ((osal_copy_from_user(&chn, p->ptr1, sizeof(mmf_chn_s)) != 0) ||
			    (osal_copy_from_user(&palette, p->ptr2, sizeof(rgn_palette)) != 0)) {
				TRACE_RGN(RGN_ERR, "Region set chn palette, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}

			input_pixel_table = osal_kmalloc(palette.lut_length * sizeof(rgn_rgbquad_s), OSAL_GFP_KERNEL);
			if (input_pixel_table == NULL) {
				TRACE_RGN(RGN_ERR, "kmalloc failed.\n");
				return ERR_RGN_NOMEM;
			}
			if (osal_copy_from_user(input_pixel_table, (void *)(palette.ppalette_table),
				palette.lut_length * sizeof(rgn_rgbquad_s))) {
				TRACE_RGN(RGN_ERR, "lut osal_copy_from_user failed.\n");
				ret = ERR_RGN_NOMEM;
				osal_kfree(input_pixel_table);
				break;
			}
		...
		case RGN_SDK_SET_CMPR_SIZE: {
			...
			if ((osal_copy_from_user(&cmpr_sz, p->ptr1, sizeof(s32)) != 0)) {
				TRACE_RGN(RGN_ERR, "Region set compress size, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
...
long _rgn_g_ctrl(struct rgn_dev *rdev, struct rgn_ext_control *p)
{
	...
	case RGN_IOCTL_SDK_CTRL: {
		...
		case RGN_SDK_GET_ATTR: {
			...
			if (osal_copy_to_user(p->ptr1, &region, sizeof(rgn_attr_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region get attribute, osal_copy_to_user failed.\n");
				ret = OSAL_SUCCESS;
			}
		}
		break;
		...
		case RGN_SDK_GET_DISPLAY_ATTR: {
			if (osal_copy_from_user(&chn, p->ptr1, sizeof(mmf_chn_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region get display attribute, osal_copy_from_user failed.\n");
				ret = OSAL_SUCCESS;
				break;
			}
			...
			if (osal_copy_to_user(p->ptr2, &chn_attr, sizeof(rgn_chn_attr_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region get display attribute, osal_copy_to_user failed.\n");
				ret = OSAL_SUCCESS;
			}
		}
		break;
		...
		case RGN_SDK_GET_CANVAS_INFO: {
			...
			if (osal_copy_to_user(p->ptr1, &canvas_info, sizeof(rgn_canvas_info_s)) != 0) {
				TRACE_RGN(RGN_ERR, "Region get canvas info, osal_copy_to_user failed.\n");
				ret = OSAL_SUCCESS;
			}
		}
		break;
		...
		case RGN_SDK_GET_ION_LEN: {
			...
			if (ret != 0)
				break;
			if (osal_copy_to_user(p->ptr1, &ctx->ion_len, sizeof(u32)) != 0) {
				TRACE_RGN(RGN_ERR, "Region get ion length, osal_copy_to_user failed.\n");
				ret = OSAL_SUCCESS;
			}
