...
static void _cviEncRc_setBitrateParam(stRcInfo *pRcInfo, EncOpenParam *pOpenParam)
{
    EncWave5Param *param = &pOpenParam->EncStdParam.waveParam;
...
static void _cviEncRc_setFramerateParam(stRcInfo *pRcInfo, EncOpenParam *pOpenParam)
{
    pRcInfo->framerate = pOpenParam->frameRateInfo;
}

int cviEncRc_SetParam(stRcInfo *pRcInfo, EncOpenParam *pOpenParam, eRcParam eParam)
{
    int ret = 0;
    ...
    switch (eParam) {
    case E_BITRATE:
        _cviEncRc_setBitrateParam(pRcInfo, pOpenParam);
        ret = 1;
        break;
    case E_FRAMERATE:
        _cviEncRc_setFramerateParam(pRcInfo, pOpenParam);
        ret = 1;
        break;
    default:
        break;
...
void cviEncRc_Open(stRcInfo *pRcInfo, EncOpenParam *pOpenParam)
{
    EncWave5Param *param = &pOpenParam->EncStdParam.waveParam;

    pRcInfo->rcEnable = pOpenParam->rcEnable;
    pRcInfo->cviRcEn = pOpenParam->cviRcEn;
    ...
    // bitrate
    cviEncRc_SetParam(pRcInfo, pOpenParam, E_BITRATE);
    cviEncRc_SetParam(pRcInfo, pOpenParam, E_FRAMERATE);
...
BOOL cviEnc_Avbr_PicCtrl(stRcInfo *pRcInfo, EncOpenParam *pOpenParam, int frameIdx)
{
...
BOOL cviEncRc_Avbr_CheckFrameSkip(stRcInfo *pRcInfo, EncOpenParam *pOpenParam, int isIFrame)
{
...
int cviEncRc_Avbr_GetQpDelta(stRcInfo *pRcInfo, EncOpenParam *pOpenParam)
{
    EncWave5Param *param = &pOpenParam->EncStdParam.waveParam;
    int qDelta = 0;
    if ((!pRcInfo->rcEnable) || (pRcInfo->rcMode != RC_MODE_AVBR)) {
