...
static int jpu_update_usage_info(jpu_statistic_info_t *jpu_usage_info)
{
    ...
    for (i = 0; i < get_max_num_jpu_core(); i++) {
        int jpu_woking_time_ms = 0;
        int jpu_instant_usage = 0;
        int j;


        jpu_woking_time_ms = (uint64_t)jpu_usage_info->jpu_stat_cycles[i]  * 1000 / JPU_STAT_CYCLES;
        jpu_woking_time_ms = (jpu_woking_time_ms > JPU_INFO_STAT_INTERVAL) ? JPU_INFO_STAT_INTERVAL: jpu_woking_time_ms;
        jpu_usage_info->jpu_stat_cycles[i] = 0;

        jpu_usage_info->jpu_working_time_in_ms[i] += jpu_woking_time_ms;
        jpu_usage_info->jpu_total_time_in_ms[i] += JPU_INFO_STAT_INTERVAL;

        jpu_usage_info->jpu_working_array[i][jpu_usage_info->jpu_status_index[i]] = jpu_woking_time_ms;
        ...
        for (j = 0; j < MAX_JPU_STAT_WIN_SIZE; j++)
            jpu_instant_usage += jpu_usage_info->jpu_working_array[i][j];

        jpu_usage_info->jpu_instant_usage[i] = (jpu_instant_usage) / MAX_JPU_STAT_WIN_SIZE;
        jpu_usage_info->jpu_instant_usage[i] = (jpu_usage_info->jpu_instant_usage[i] > 100) ? 100 : jpu_usage_info->jpu_instant_usage[i];
    }
...
static int jpuinfo_show(struct seq_file *m, void *v)
{
    ...
    if(s_jpu_open_ref_count > 0) {
        for (core_idx = 0; core_idx < get_max_num_jpu_core(); core_idx++) {
            seq_printf(m, "{\"instance_id\":%d, \"status\":%s, \"res\":%dx%d, "
...
static int jpuinfo_open(struct inode *inode, struct file *file) {
    return single_open(file, jpuinfo_show, NULL);
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static const struct proc_ops jpeg_proc_fops = {
    .proc_open = jpuinfo_open,
    .proc_read = seq_read,
    .proc_lseek = seq_lseek,
    .proc_release = single_release,
};
#else
static const struct file_operations jpeg_proc_fops = {
    .owner = THIS_MODULE,
    .open = jpuinfo_open,
    .read = seq_read,
    .llseek = seq_lseek,
