...
static void vi_gdc_callback(void *pParam, VB_BLK blk)
{
...
/* when rgbmap use frame buffer mode and grid bit is 5, mmap hardware IP
 * readout data is not equal to the rgbmap, will read more garbage data
 * on the right side, so fill 0x80 instead of the garbage.
 */
void isp_fill_rgbmap(struct isp_ctx *ctx, enum cvi_isp_raw raw_num)
{
	uintptr_t dmab;
	uint32_t rgbmap_le, rgbmap_se;
	uint32_t stride, total;
	uint8_t i = 0;
	void *vaddr = NULL;
	...
	if (ctx->isp_pipe_cfg[raw_num].rgbmap_i.w_bit != 5)
		return;
	...
	if (raw_num == ISP_PRERAW0) {
		...
		//No se in ISP_PRERAW2
	}

	dmab   = ctx->phys_regs[rgbmap_le];
	stride = ISP_RD_BITS(dmab, REG_ISP_DMA_CTL_T, DMA_STRIDE, STRIDE);
	total  = UPPER(ctx->isp_pipe_cfg[raw_num].crop.h, ctx->isp_pipe_cfg[raw_num].rgbmap_i.h_bit) * stride;

	if (total) {
		for (i = 0; i < RGBMAP_BUF_IDX; i++) {
			vaddr = phys_to_virt(isp_bufpool[raw_num].rgbmap_le[i]);
			memset(vaddr, 0x80, total);
		}
...
void _vi_yuv_dma_setup(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	for (i = chn_str; i < total_chn; i++) {
		switch (raw_num) {
		case ISP_PRERAW0:
			dma = ISP_BLK_ID_DMA_CTL6 + i - chn_str;
			break;
		case ISP_PRERAW1:
			dma = ISP_BLK_ID_DMA_CTL12 + i - chn_str;
			break;
		case ISP_PRERAW2:
			dma = ISP_BLK_ID_DMA_CTL18 + i - chn_str;
			break;
		default:
			break;
		}
		...
		for (j = 0; j < OFFLINE_YUV_BUF_NUM; j++) {
			...
			pool->yuv_yuyv[b->chn_num][j] = b->addr = _mempool_pop(bufsize_yuyv);
...
static void _isp_preraw_be_dma_dump(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
	...
	for (raw = ISP_PRERAW0; raw < ISP_PRERAW_MAX; raw++) {
...
void _isp_preraw_be_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
	...
	if (ictx->is_offline_be && !ictx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw) {
		//apply pre_fe_le buffer
		ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL4, isp_bufpool[ISP_PRERAW0].bayer_le[0]);

		if (ictx->is_hdr_on) {
			//apply pre_fe_se buffer
			ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL5, isp_bufpool[ISP_PRERAW0].bayer_se[0]);
		}
	}

	if (_is_fe_be_online(ictx)) { //fe->be->dram->post
		if (ictx->is_slice_buf_on) {
			DMA_SETUP_2(ISP_BLK_ID_DMA_CTL22, ISP_PRERAW0);
			isp_bufpool[ISP_PRERAW0].prebe_le[0] = bufaddr;

			if (ictx->isp_pipe_cfg[ISP_PRERAW0].is_hdr_on) {
				DMA_SETUP_2(ISP_BLK_ID_DMA_CTL23, ISP_PRERAW0);
				isp_bufpool[ISP_PRERAW0].prebe_se[0] = bufaddr;
			}
		} else {
			for (buf_num = 0; buf_num < OFFLINE_PRE_BE_BUF_NUM; buf_num++) {
				...
				memset(b, 0, sizeof(*b));
				b->addr = bufaddr;
				b->raw_num = ISP_PRERAW0;
				b->ir_idx = buf_num;
				isp_bufpool[ISP_PRERAW0].prebe_le[buf_num] = b->addr;
				isp_buf_queue(&pre_be_out_q, b);
			}

			ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL22, isp_bufpool[ISP_PRERAW0].prebe_le[0]);
			...
			if (ictx->isp_pipe_cfg[ISP_PRERAW0].is_hdr_on) {
				for (buf_num = 0; buf_num < OFFLINE_PRE_BE_BUF_NUM; buf_num++) {
					...
					b->raw_num = ISP_PRERAW0;
					b->ir_idx = buf_num;
					isp_bufpool[ISP_PRERAW0].prebe_se[buf_num] = b->addr;
					isp_buf_queue(&pre_be_out_se_q, b);
				}

				ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL23, isp_bufpool[ISP_PRERAW0].prebe_se[0]);
			}
		}
	}
...
void _isp_rawtop_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
	...
	if (_is_fe_be_online(ictx)) { //fe->be->dram->post
		...
		ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL28, isp_bufpool[ISP_PRERAW0].prebe_le[0]);

		if (ictx->is_hdr_on) {
			//apply pre_be se buffer from DMA_CTL23
			ispblk_dma_setaddr(ictx, ISP_BLK_ID_DMA_CTL29, isp_bufpool[ISP_PRERAW0].prebe_se[0]);
		}
	}
...
void _isp_yuvtop_dma_setup(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
	...
	if (cfg_dma) {
		if (ictx->isp_pipe_cfg[ISP_PRERAW0].is_offline_scaler ||
			...
			if (_is_be_post_online(ictx) && !ictx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw) {
...
void _vi_dma_set_sw_mode(struct isp_ctx *ctx)
{
	...
	ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL28, false); //raw crop LE
	ispblk_dma_set_sw_mode(ctx, ISP_BLK_ID_DMA_CTL29, false); //raw crop SE
...
void _vi_yuv_get_dma_size(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	for (i = chn_str; i < total_chn; i++) {
		...
		for (j = 0; i < OFFLINE_YUV_BUF_NUM; j++) {
			bufsize_yuyv = ispblk_dma_yuv_bypass_config(ctx, dma, 0, raw_num);
...
void _vi_pre_be_get_dma_size(struct isp_ctx *ictx, enum cvi_isp_raw raw_max)
{
	...
	if (_is_fe_be_online(ictx)) { //fe->be->dram->post
		if (ictx->is_slice_buf_on) {
			bufsize = ispblk_dma_buf_get_size2(ictx, ISP_BLK_ID_DMA_CTL22, ISP_PRERAW0);
			_mempool_pop(bufsize);

			if (ictx->isp_pipe_cfg[ISP_PRERAW0].is_hdr_on) {
				bufsize = ispblk_dma_buf_get_size2(ictx, ISP_BLK_ID_DMA_CTL23, ISP_PRERAW0);
				_mempool_pop(bufsize);
			}
		} else {
			for (buf_num = 0; buf_num < OFFLINE_PRE_BE_BUF_NUM; buf_num++) {
...
static void _vi_preraw_be_init(struct cvi_vi_dev *vdev)
{
	...
	if (cfg_be) { //RGB sensor
		...
		// preraw_be_top
		ispblk_preraw_be_config(ctx, ISP_PRERAW0);
		// preraw_be wdma ctrl
		ispblk_pre_wdma_ctrl_config(ctx, ISP_PRERAW0);
		...
		if (_is_fe_be_online(ctx))
			ispblk_slice_buf_config(&vdev->ctx, ISP_PRERAW0, vdev->ctx.is_slice_buf_on);
		else
			ispblk_slice_buf_config(&vdev->ctx, ISP_PRERAW0, false);
	}
}
...
static void _isp_rawtop_init(struct cvi_vi_dev *vdev)
{
	...
	// raw_top
	ispblk_rawtop_config(ictx, ISP_PRERAW0);
	// raw_rdma ctrl
	ispblk_raw_rdma_ctrl_config(ictx, ISP_PRERAW0);
...
static void _isp_rgbtop_init(struct cvi_vi_dev *vdev)
{
	...
	ispblk_rgbtop_config(ictx, ISP_PRERAW0);
...
static void _isp_yuvtop_init(struct cvi_vi_dev *vdev)
{
	...
	ispblk_yuvtop_config(ictx, ISP_PRERAW0);
	...
	const u16 _i2c_num)
{
	...
	for (j = 0; j < _i2c_num; j++) {
		...
		vi_pr(VI_DBG, "raw_num=%d, i2c_num=%d, j=%d, magic_num=%d, fe_frm_num=%d\n",
		...
		if (node->n.magic_num == fe_frm_num ||
			...
			for (i = 0; i < node->n.regs_num; i++) {
				...
				if (i2c_data->update && (i2c_data->dly_frm_num == 0)) {
					vi_pr(VI_DBG, "i2c_addr=0x%x write:0x%x\n",
	...
	const enum cvi_isp_raw raw_num)
{
	...
	struct _isp_snr_i2c_node *i2c_n[VI_MAX_SNS_CFG_NUM];
	unsigned long flags;
	u16 i2c_num = 0;
...
static void _usr_pic_timer_handler(unsigned long data)
{
	...
	if (!ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw) {
#if (KERNEL_VERSION(4, 15, 0) <= LINUX_VERSION_CODE)
	...
	if (atomic_read(&vdev->pre_be_state[ISP_BE_CH0]) == ISP_PRE_BE_IDLE &&
		...
		if (_is_fe_be_online(ctx) && ctx->isp_pipe_cfg[ISP_PRERAW0].is_hdr_on) {
...
static void _isp_yuv_bypass_buf_enq(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num, const u8 buf_chn)
{
	...
	if (raw_num == ISP_PRERAW0) {
		...
		case 3:
			...
			break;
		default:
			vi_pr(VI_ERR, "PRERAW_%d Wrong chn_num(%d)\n", raw_num, hw_chn_num);
			return;
		}
	} else if (raw_num == ISP_PRERAW1) {
		switch (hw_chn_num) {
 ...
 */
	...
	if (!ictx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) {
		//Postraw out size
		ictx->img_width = ictx->isp_pipe_cfg[ISP_PRERAW0].crop.w;
		ictx->img_height = ictx->isp_pipe_cfg[ISP_PRERAW0].crop.h;
	}
...
void _vi_scene_ctrl(struct cvi_vi_dev *vdev, enum cvi_isp_raw *raw_max)
{
	...
	if (gViCtx->total_dev_num >= 2) { //Multi sensor scenario
		if (!ctx->isp_pipe_cfg[ISP_PRERAW0].is_yuv_bypass_path &&
			ctx->isp_pipe_cfg[ISP_PRERAW1].is_yuv_bypass_path) { //rgb+yuv
			...
			ctx->is_fbc_on = false;
			// if (ctx->isp_pipe_cfg[ISP_PRERAW2].is_yuv_bypass_path &&
		...
		*raw_max = ISP_PRERAW1;
		...
		if (ctx->isp_pipe_cfg[ISP_PRERAW0].is_yuv_bypass_path) {
			ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_scaler = true;
			ctx->isp_pipe_cfg[ISP_PRERAW0].is_422_to_420  = false;
			ctx->is_slice_buf_on = false;
		}
...
void _vi_bw_cal_set(struct cvi_vi_dev *vdev)
{
	struct isp_ctx *ctx = &vdev->ctx;
	struct cvi_vi_ctx *pviProcCtx = NULL;
...
int vi_start_streaming(struct cvi_vi_dev *vdev)
{
	...
	for (raw_num = ISP_PRERAW0; raw_num < ISP_PRERAW_MAX; raw_num++) {
		...
		if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw)
...
int vi_stop_streaming(struct cvi_vi_dev *vdev)
{
	...
	while (--count > 0) {
		if (atomic_read(&vdev->postraw_state) == ISP_POSTRAW_IDLE &&
			atomic_read(&vdev->pre_fe_state[ISP_PRERAW0][ISP_FE_CH0]) == ISP_PRERAW_IDLE &&
			atomic_read(&vdev->pre_fe_state[ISP_PRERAW0][ISP_FE_CH1]) == ISP_PRERAW_IDLE &&
			atomic_read(&vdev->pre_fe_state[ISP_PRERAW1][ISP_FE_CH0]) == ISP_PRERAW_IDLE &&
			atomic_read(&vdev->pre_fe_state[ISP_PRERAW1][ISP_FE_CH1]) == ISP_PRERAW_IDLE &&
			atomic_read(&vdev->pre_fe_state[ISP_PRERAW2][ISP_FE_CH0]) == ISP_PRERAW_IDLE &&
			atomic_read(&vdev->pre_fe_state[ISP_PRERAW2][ISP_FE_CH1]) == ISP_PRERAW_IDLE &&
	...
	const u8 hw_chn_num)
{
	...
	if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) { //RGB sensor
		...
		enum ISP_BLK_ID_T pre_be_dma = (hw_chn_num == ISP_BE_CH0) ? ISP_BLK_ID_DMA_CTL22 : ISP_BLK_ID_DMA_CTL23;
		struct isp_buffer *b = NULL;
		...
		u8 buf_chn = ctx->raw_chnstr_num[raw_num] + hw_chn_num;
		u32 dma = 0;
		...
		switch (raw_num) {
		case ISP_PRERAW0:
			dma = ISP_BLK_ID_DMA_CTL6 + hw_chn_num;
			break;
		case ISP_PRERAW1:
			dma = ISP_BLK_ID_DMA_CTL12 + hw_chn_num;
			break;
		case ISP_PRERAW2:
			dma = ISP_BLK_ID_DMA_CTL18 + hw_chn_num;
			break;
		default:
			break;
		}
	...
	const enum cvi_isp_pre_chn_num fe_chn_num)
{
	...
	if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) { //RGB sensor
		...
		if (raw_num == ISP_PRERAW0) {
			...
			pre_fe_dma = ((fe_chn_num == ISP_FE_CH0) ? ISP_BLK_ID_DMA_CTL12 : ISP_BLK_ID_DMA_CTL13);
		} else {
		...
		if (atomic_read(&vdev->isp_raw_dump_en[raw_num]) == 1) {//raw_dump flow
		   ...
		   ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) { //RGB+YUV sensor
		u8 buf_chn = ctx->raw_chnstr_num[raw_num] + fe_chn_num;
		u32 dma = 0;
		...
		switch (raw_num) {
		case ISP_PRERAW0:
			dma = ISP_BLK_ID_DMA_CTL6 + fe_chn_num;
			break;
		case ISP_PRERAW1:
			dma = ISP_BLK_ID_DMA_CTL12 + fe_chn_num;
			break;
		case ISP_PRERAW2:
			dma = ISP_BLK_ID_DMA_CTL18 + fe_chn_num;
			break;
		default:
			break;
		}
	...
	const enum cvi_isp_pre_chn_num chn_num)
{
	...
	if (chn_num == ISP_FE_CH0) {
		if (raw_num == ISP_PRERAW0)
			ispblk_dma_setaddr(ctx, ISP_BLK_ID_DMA_CTL10, isp_bufpool[raw_num].rgbmap_le[rgbmap_idx]);
		else if (raw_num == ISP_PRERAW1)
			ispblk_dma_setaddr(ctx, ISP_BLK_ID_DMA_CTL16, isp_bufpool[raw_num].rgbmap_le[rgbmap_idx]);
		else
			ispblk_dma_setaddr(ctx, ISP_BLK_ID_DMA_CTL20, isp_bufpool[raw_num].rgbmap_le[rgbmap_idx]);
	} else if (chn_num == ISP_FE_CH1) {
		if (raw_num == ISP_PRERAW0)
			ispblk_dma_setaddr(ctx, ISP_BLK_ID_DMA_CTL11, isp_bufpool[raw_num].rgbmap_se[rgbmap_idx]);
		else if (raw_num == ISP_PRERAW1)
			ispblk_dma_setaddr(ctx, ISP_BLK_ID_DMA_CTL17, isp_bufpool[raw_num].rgbmap_se[rgbmap_idx]);
	}
}
...
static long _vi_s_ctrl(struct cvi_vi_dev *vdev, struct vi_ext_control *p)
{
	...
	case VI_IOCTL_HDR:
	{
		...
		ctx->isp_pipe_cfg[ISP_PRERAW0].is_hdr_on = p->value;
	...
	case VI_IOCTL_USR_PIC_CFG:
	{
		...
		if ((cfg.crop.width < 32) || (cfg.crop.width > 4096)
			...
			vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].csibdg_width	= vdev->usr_fmt.width;
			vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].csibdg_height	= vdev->usr_fmt.height;
			vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].max_width		= vdev->usr_fmt.width;
			vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].max_height 	= vdev->usr_fmt.height;
	...
	case VI_IOCTL_USR_PIC_ONOFF:
	{
		...
		ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw =
			(vdev->isp_source == CVI_ISP_SOURCE_FE);

		vi_pr(VI_INFO, "vdev->isp_source=%d\n", vdev->isp_source);
		vi_pr(VI_INFO, "ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw=%d\n",
			ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw);
	...
	case VI_IOCTL_ENQ_BUF:
	{
		...
		qbuf = kzalloc(sizeof(struct cvi_isp_buf2), GFP_ATOMIC);
		if (qbuf == NULL) {
		...
		if (_is_all_online(ctx) &&
			cvi_isp_rdy_buf_empty2(vdev, ISP_PRERAW0) &&
			vdev->pre_fe_frm_num[ISP_PRERAW0][ISP_FE_CH0] > 0) {
			pre_trig = true;
		} else if (_is_fe_be_online(ctx)) { //fe->be->dram->post
			if (cvi_isp_rdy_buf_empty2(vdev, vdev->chn_id) &&
				vdev->postraw_frame_number[ISP_PRERAW0] > 0) {
...
int vi_cb(void *dev, enum ENUM_MODULES_ID caller, u32 cmd, void *arg)
{
	...
	case VI_CB_SET_VIVPSSMODE:
	{
		...
		for (dev_num = 0; dev_num < VI_MAX_DEV_NUM; dev_num++) {
			u8 is_vpss_online = (stVIVPSSMode.aenMode[dev_num] == VI_ONLINE_VPSS_ONLINE) ||
...
#ifdef VI_PROFILE
static void _vi_update_chn_real_frame_rate(VI_CHN_STATUS_S *pstViChnStatus)
{
...
static int _vi_event_handler_thread(void * arg)
{
	...
	while (1) {
#ifdef VI_PROFILE
		_vi_update_chn_real_frame_rate(&gViCtx->chnStatus[chn.s32ChnId]);
...
static void _vi_record_debug_info(struct isp_ctx *ctx)
{
	...
	if (_is_fe_be_online(ctx) && ctx->is_slice_buf_on &&
		...
		gOverflowInfo->vpss_info.dev_num = ISP_PRERAW0;
...
{
	struct isp_ctx *ctx = &vdev->ctx;
	u8 wait_fe = ISP_PRERAW0, count = 10;
...
static int _vi_err_handler_thread(void *arg)
{
	...
	while (1) {
		...
		if (vdev->vi_th[th_id].flag == 1)
			err_raw_num = ISP_PRERAW0;
		else if (vdev->vi_th[th_id].flag == 2)
			err_raw_num = ISP_PRERAW1;
...
static inline void _vi_wake_up_preraw_th(struct cvi_vi_dev *vdev, const enum cvi_isp_raw raw_num)
{
	...
	vdev->vi_th[E_VI_TH_PRERAW].flag = (raw_num == ISP_PRERAW0) ? 1 : 2;
	wake_up(&vdev->vi_th[E_VI_TH_PRERAW].wq);
}
	...
	const u8 chn_num)
{
	...
	if (_is_fe_be_online(ctx) && !ctx->is_slice_buf_on) { // fe->be->dram->post
		...
		type = (raw_num == ISP_PRERAW0) ? VI_EVENT_PRE0_EOF : VI_EVENT_PRE1_EOF;
...
static void _isp_postraw_shaw_done_handler(struct cvi_vi_dev *vdev)
{
	if (_is_fe_be_online(&vdev->ctx) && vdev->ctx.is_slice_buf_on) {
		...
		_vi_wake_up_preraw_th(vdev, ISP_PRERAW0);
	}
}
...
void vi_irq_handler(struct cvi_vi_dev *vdev)
{
	...
	if (top_sts_2.bits.FRAME_START_FE0 & 0x1) {
		...
		vi_record_sof_perf(vdev, ISP_PRERAW0, ISP_FE_CH0);

		if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw)
			++vdev->pre_fe_sof_cnt[ISP_PRERAW0][ISP_FE_CH0];

		if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_yuv_bypass_path) { //RGB sensor
			if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw) {
				_isp_sof_handler(vdev, ISP_PRERAW0);
				vi_event_queue(vdev, VI_EVENT_PRE0_SOF,
							vdev->pre_fe_sof_cnt[ISP_PRERAW0][ISP_FE_CH0]);
			}
		} else if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_offline_scaler) { //YUV sensor online mode
	...
	if (top_sts_2.bits.FRAME_START_FE0 & 0x2) {
		++vdev->pre_fe_sof_cnt[ISP_PRERAW0][ISP_FE_CH1];

		//_isp_sof_handler(ISP_PRERAW0);
	}

	/* pre_fe1 ch0 frame start */
	if (top_sts_2.bits.FRAME_START_FE1 & 0x1) {
		++vdev->pre_fe_sof_cnt[ISP_PRERAW1][ISP_FE_CH0];

		if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW1].is_yuv_bypass_path) { //RGB sensor
			_isp_sof_handler(vdev, ISP_PRERAW1);
			vi_event_queue(vdev, VI_EVENT_PRE1_SOF, vdev->pre_fe_sof_cnt[ISP_PRERAW1][ISP_FE_CH0]);
		} else if (!vdev->ctx.isp_pipe_cfg[ISP_PRERAW1].is_offline_scaler) { //YUV sensor online mode
			//ISP team no need sof event by yuv sensor
			//_post_hw_enque(vdev);
			tasklet_hi_schedule(&vdev->job_work);
		}
	...
	if (top_sts_2.bits.FRAME_START_FE1 & 0x2) {
		++vdev->pre_fe_sof_cnt[ISP_PRERAW1][ISP_FE_CH1];

		//_isp_sof_handler(ISP_PRERAW1);
	}

	/* pre_fe2 ch0 frame start */
	if (top_sts_2.bits.FRAME_START_FE2 & 0x1) {
		++vdev->pre_fe_sof_cnt[ISP_PRERAW2][ISP_FE_CH0];
	}

	/* pre_fe2 ch1 frame start */
	if (top_sts_2.bits.FRAME_START_FE1 & 0x2) {
		++vdev->pre_fe_sof_cnt[ISP_PRERAW2][ISP_FE_CH1];

		//_isp_sof_handler(ISP_PRERAW2);
	}
	...
	if (top_sts_1.bits.PQ_DONE_FE0 & 0x1) {
		if (ctx->isp_pipe_cfg[ISP_PRERAW0].is_hdr_on &&
		    ctx->isp_pipe_cfg[ISP_PRERAW0].is_stagger_vsync) {
			uintptr_t preraw0 = ctx->phys_regs[ISP_BLK_ID_PRE_RAW_FE0];
	...
	if (top_sts_1.bits.PQ_DONE_FE1 & 0x1) {
		if (ctx->isp_pipe_cfg[ISP_PRERAW1].is_hdr_on &&
		    ctx->isp_pipe_cfg[ISP_PRERAW1].is_stagger_vsync) {
			uintptr_t preraw1 = ctx->phys_regs[ISP_BLK_ID_PRE_RAW_FE1];

			ISP_WR_BITS(preraw1, REG_PRE_RAW_FE_T, PRE_RAW_FRAME_VLD, FE_PQ_VLD_CH1, 1);
		}
	}

	/* pre_fe0 ch0 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE0 & 0x1) {
		vi_record_fe_perf(vdev, ISP_PRERAW0, ISP_FE_CH0);

		++vdev->pre_fe_frm_num[ISP_PRERAW0][ISP_FE_CH0];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW0, ISP_FE_CH0);
	}

	/* pre_fe0 ch1 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE0 & 0x2) {
		++vdev->pre_fe_frm_num[ISP_PRERAW0][ISP_FE_CH1];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW0, ISP_FE_CH1);
	}

	/* pre_fe0 ch2 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE0 & 0x4) {
		++vdev->pre_fe_frm_num[ISP_PRERAW0][ISP_FE_CH2];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW0, ISP_FE_CH2);
	}

	/* pre_fe0 ch3 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE0 & 0x8) {
		++vdev->pre_fe_frm_num[ISP_PRERAW0][ISP_FE_CH3];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW0, ISP_FE_CH3);
	}

	/* pre_fe1 ch0 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE1 & 0x1) {
		++vdev->pre_fe_frm_num[ISP_PRERAW1][ISP_FE_CH0];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW1, ISP_FE_CH0);
	}

	/* pre_fe1 ch1 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE1 & 0x2) {
		++vdev->pre_fe_frm_num[ISP_PRERAW1][ISP_FE_CH1];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW1, ISP_FE_CH1);
	}

	/* pre_fe2 ch0 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE2 & 0x1) {
		++vdev->pre_fe_frm_num[ISP_PRERAW2][ISP_FE_CH0];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW2, ISP_FE_CH0);
	}

	/* pre_fe2 ch1 frm_done */
	if (top_sts_0.bits.FRAME_DONE_FE2 & 0x2) {
		++vdev->pre_fe_frm_num[ISP_PRERAW2][ISP_FE_CH1];

		_isp_pre_fe_done_handler(vdev, ISP_PRERAW2, ISP_FE_CH1);
	}

	/* pre_be ch0 frm done */
	if (top_sts_0.bits.FRAME_DONE_BE & 0x1) {
		vi_record_be_perf(vdev, ISP_PRERAW0, ISP_BE_CH0);

		_isp_pre_be_done_handler(vdev, ISP_BE_CH0);
	}

	/* pre_be ch1 frm done */
	if (top_sts_0.bits.FRAME_DONE_BE & 0x2) {
		_isp_pre_be_done_handler(vdev, ISP_BE_CH1);
	}

	/* post shadow up done */
	if (top_sts_0.bits.SHAW_DONE_POST) {
		if (_is_fe_be_online(ctx) && ctx->is_slice_buf_on) {
	...
	/* post frm done */
	if (top_sts_0.bits.FRAME_DONE_POST) {
		vi_record_post_end(vdev, ISP_PRERAW0);
