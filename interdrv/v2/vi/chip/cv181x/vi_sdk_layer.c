...
s32 vi_enable_dev(VI_DEV ViDev)
{
	...
	if ((ViDev < (VI_DEV)ISP_PRERAW0) || (ViDev >= (VI_DEV)ISP_PRERAW_MAX)) {
...
#endif
	vi_pr(VI_DBG, "enable=%d, total_dev_num=%d\n", gViCtx->isDevEnable[ViDev], gViCtx->total_dev_num);
...
s32 vi_set_pipe_frame_source(VI_PIPE ViPipe, const VI_PIPE_FRAME_SOURCE_E enSource)
{
	...
	gvdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw = (gvdev->isp_source == CVI_ISP_SOURCE_FE);

	vi_pr(VI_INFO, "isp_source=%d\n", gvdev->isp_source);
	vi_pr(VI_INFO, "ISP_PRERAW0.is_offline_preraw=%d\n",
			gvdev->ctx.isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw);

	return CVI_SUCCESS;
}

s32 vi_send_pipe_raw(VI_PIPE PipeId, const VIDEO_FRAME_INFO_S *pstVideoFrame)
{
	struct isp_ctx *ctx = &gvdev->ctx;
...
s32 vi_enable_chn(VI_CHN ViChn)
{
	...
	for (i = 0; i < gViCtx->total_chn_num; i++) {
		...
		if (ctx->isp_pipe_cfg[dev_num].is_offline_scaler) {
			u8 j = 0;
...
s32 vi_get_pipe_frame(VI_PIPE ViPipe, VIDEO_FRAME_INFO_S *pstFrameInfo, s32 s32MilliSec)
{
	...
	u32 u32BlkSize, dev_frm_w, dev_frm_h, frm_w, frm_h;
...
static s32 _vi_update_ldc_mesh(VPSS_CHN ViChn, const VI_LDC_ATTR_S *pstLDCAttr
	, ROTATION_E enRotation, u64 paddr)
{
	u64 paddr_old;
	struct cvi_gdc_mesh *pmesh = &g_vi_mesh[ViChn];
...
s32 vi_set_chn_rotation(VI_CHN ViChn, ROTATION_E enRotation)
{
...
s32 vi_set_chn_ldc_attr(VI_CHN ViChn, ROTATION_E enRotation,
	const VI_LDC_ATTR_S *pstLDCAttr, u64 mesh_addr)
{
