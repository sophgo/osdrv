	...
	const enum cvi_isp_raw raw_num)
{
	...
	if (!cfg->manr_enable) {
		...
		mm_00.bits.BYPASS = (ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw) ? 1 : 0;
	}
	...
	} else if (_is_fe_be_online(ctx) && ctx->is_rgbmap_sbm_on && cfg->rgbmap_w_bit > 4) {
		vi_pr(VI_WARN, "RGBMAP_w_bit(%d) need <= 4 under on rgbmap slice buf mode\n", cfg->rgbmap_w_bit);
		cfg->rgbmap_w_bit = cfg->rgbmap_h_bit = 4;
	...
	const enum cvi_isp_raw raw_num)
{
	...
	ispblk_dma_config(ctx, ISP_BLK_ID_DMA_CTL25, 0);
}
