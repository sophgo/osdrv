...
void ispblk_rawtop_config(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	rdmi_enable.bits.CH_NUM		= ctx->isp_pipe_cfg[raw_num].is_hdr_on ? 1 : !_is_all_online(ctx);
	patgen1.bits.PG_ENABLE		= ctx->isp_pipe_cfg[raw_num].is_hdr_on ? 0 : 1;
...
void ispblk_yuvtop_config(struct isp_ctx *ctx, const enum cvi_isp_raw raw_num)
{
	...
	if (_is_all_online(ctx) && !ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_scaler) {
...
void ispblk_isptop_config(struct isp_ctx *ctx)
{
	...
	for (raw_num = ISP_PRERAW0; raw_num < ISP_PRERAW_MAX; raw_num++) {
		...
		if (ctx->isp_pipe_cfg[raw_num].is_offline_preraw) { //RAW replay
			pre_fe_trig_by_hw[raw_num] = 0x0;
		} else if (!ctx->isp_pipe_cfg[raw_num].is_yuv_bypass_path) { //RGB sensor
			if (!ctx->isp_pipe_cfg[raw_num].is_hdr_on)
	...
	if ((ctx->isp_pipe_cfg[ISP_PRERAW0].is_offline_preraw == 0) &&
...
#else
	//In order for linearMode use guideWeight
	scene_ctrl.bits.HDR_ENABLE		= !_is_all_online(ctx);
#endif
