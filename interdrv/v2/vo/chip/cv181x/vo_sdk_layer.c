...
extern struct vb_jobs_t stVOjobs[VO_MAX_LAYER_NUM];
 ...
 * SDK layer APIs
 ****************************************************************************/
static s32 _vo_get_videolayer_bind(VO_LAYER_BIND_S *pstVoLayerBind)
{
	s32 i;

	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		if (gVoCtx->stLayerBind[i].s32VoLayer == pstVoLayerBind->s32VoLayer) {
			pstVoLayerBind->s32VoDev = gVoCtx->stLayerBind[i].s32VoDev;
			return CVI_SUCCESS;
		}
	}
	return CVI_FAILURE;
...
static s32 _vo_chk_videolayer_is_bind(VO_LAYER VoLayer)
{
	s32 i;

	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		if (gVoCtx->stLayerBind[i].s32VoLayer == VoLayer) {
			return CVI_SUCCESS;
		}
	}
	return CVI_FAILURE;
...
s32 vo_clear_chnbuf(VO_LAYER VoLayer, VO_CHN VoChn, bool bClrAll)
{
	...
	stVoLayerBind.s32VoLayer = VoLayer;
	ret = _vo_get_videolayer_bind(&stVoLayerBind);
	if (ret != CVI_SUCCESS) {
		CVI_TRACE_VO(CVI_DBG_ERR, "VoLayer(%d) isn't binded to any VoDev yet.\n", VoLayer);
		return CVI_ERR_VO_DEV_NOT_BINDED;
	}
	chn.s32DevId = stVoLayerBind.s32VoDev;
	jobs = base_get_jobs_by_chn(chn, CHN_TYPE_IN);
	MOD_CHECK_NULL_PTR(jobs);
...
s32 vo_get_panelstatus(VO_LAYER VoLayer, VO_CHN VoChn, u32 *is_init)
{
	u32 is_init_panel = 0;
	s32 ret = CVI_FAILURE;
	...
	memcpy(is_init, &is_init_panel, sizeof(u32));
...
s32 vo_get_pub_attr(VO_DEV VoDev, VO_PUB_ATTR_S *pstPubAttr)
{
	...
	//vo_get_intf_type(d->fd, (s32 *)&vo_sel);
...
s32 vo_enable(VO_DEV VoDev)
{
	...
	if (gVoCtx->stPubAttr[VoDev].enIntfType == 0) {
...
static s32 vo_hide_chn(VO_LAYER VoLayer, VO_CHN VoChn)
{
...
static s32 vo_pause_chn(VO_LAYER VoLayer, VO_CHN VoChn)
{
	s32 ret = CVI_FAILURE;
...
static s32 vo_resume_chn(VO_LAYER VoLayer, VO_CHN VoChn)
{
	s32 ret = CVI_FAILURE;
...
static s32 vo_show_chn(VO_LAYER VoLayer, VO_CHN VoChn)
{
...
static s32 vo_set_displaybuflen(VO_LAYER VoLayer, u32 u32BufLen)
{
	s32 ret = CVI_FAILURE;
...
static s32 vo_get_displaybuflen(VO_LAYER VoLayer, u32 *pu32BufLen)
{
	s32 ret = CVI_FAILURE;
...
s32 _vo_sdk_setfmt(s32 width, s32 height, u32 pxlfmt)
{
	s32 p = 0, i = 0;
	u8 align = 0;
	struct vo_video_format_mplane *mp;
...
static s32 vo_set_chnrotation(VO_LAYER VoLayer, VO_CHN VoChn, ROTATION_E enRotation)
{
	s32 ret = CVI_FAILURE;
	struct vo_rect area;
	struct sclr_disp_cfg *cfg;
...
static s32 vo_enablevideolayer(VO_LAYER VoLayer)
{
	s32 ret = CVI_FAILURE;
	...
	ret = _vo_chk_videolayer_is_bind(VoLayer);
	if (ret != CVI_SUCCESS) {
		return ret;
	} else {
		CVI_TRACE_VO(CVI_DBG_ERR, "VoLayer(%d) isn't binded to any VoDev yet.\n", VoLayer);
...
static s32 vo_disablevideolayer(VO_LAYER VoLayer)
{
	s32 VoChn = 0;
	s32 ret = CVI_FAILURE;
...
static s32 vo_bindvideolayer(VO_LAYER VoLayer, VO_DEV VoDev)
{
	s32 ret = CVI_FAILURE;
	...
	mutex_lock(&gVoCtx->stMutexBindLock);
	ret = _vo_chk_videolayer_is_bind(VoLayer, VoDev);
	if (ret != CVI_SUCCESS) {
		gVoCtx->stLayerBind[VoLayer].s32VoDev = VoDev;
		gVoCtx->stLayerBind[VoLayer].s32VoLayer = VoLayer;
		ret = CVI_SUCCESS;
	} else {
		CVI_TRACE_VO(CVI_DBG_ERR, "VoLayer(%d) has been binded to VoDev(%d)\n", VoLayer, VoDev);
		ret = CVI_ERR_VO_DEV_HAS_BINDED;
	}
	mutex_unlock(&gVoCtx->stMutexBindLock);

	return ret;
}

static s32 vo_get_videolayerattr(VO_LAYER VoLayer, VO_VIDEO_LAYER_ATTR_S *pstLayerAttr)
{
	s32 ret = CVI_FAILURE;
...
static s32 vo_set_videolayerattr(VO_LAYER VoLayer, const VO_VIDEO_LAYER_ATTR_S *pstLayerAttr)
{
	...
	if (vo_ctx.stPubAttr.enIntfType != VO_INTF_I80) {
		...
		u8 byte_cnt = (vo_ctx.stPubAttr.sti80Cfg.fmt == VO_I80_FORMAT_RGB666) ? 3 : 2;
...
long vo_sdk_ctrl(struct cvi_vo_dev *vdev, struct vo_ext_control *p)
{
	...
	case VO_SDK_BIND_VIDEOLAYER:
	{
		...
		rc = vo_bindvideolayer(cfg->VoLayer, cfg->VoDev);
