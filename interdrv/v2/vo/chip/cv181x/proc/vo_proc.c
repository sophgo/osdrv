...
static int _vo_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < VO_MAX_DEV_NUM; ++i) {
		memset(c, 0, sizeof(c));
		_intf_type_to_string(pvoCtx->stPubAttr.enIntfType, c, sizeof(c));

		memset(d, 0, sizeof(d));
		_intf_sync_to_string(pvoCtx->stPubAttr.enIntfSync, d, sizeof(d));
	...
	for (i = 0; i < VO_MAX_LAYER_NUM; ++i) {
		memset(c, 0, sizeof(c));
		_pix_fmt_to_string(pvoCtx->stLayerAttr.enPixFormat, c, sizeof(c));
