/*
 * Copyright (C) Sophon Co., Ltd. 2019-2020. All rights reserved.
 *
 * File Name: include/comm_aio.h
 * Description: basic audio in out definition
 */

#ifndef ___COMM_AIO_H__
#define ___COMM_AIO_H__
...
/* #define AUDIO_ALIGNED2   __attribute__((aligned(0x8)))*/
#define AUDIO_ALIGNED __aligned(0x8)

#define _MAX_AUDIO_FRAME_NUM    300       /*max count of audio frame in Buffer */
#define _AUD_MAX_VOICE_POINT_NUM    1280      /*max sample per frame for voice encode */
#define _AUD_MAX_AUDIO_POINT_NUM    2048     /*max sample per frame for all encoder(aacplus:2048)*/
#define _AUD_MAX_CHANNEL_NUM      3
#define _MAX_AUDIO_STREAM_LEN   (4 * 4096)
...
#define _MAX_AI_DEVICE_ID_NUM 3
#define _MAX_AO_DEVICE_ID_NUM 3
/* define macro */

#define CHECK_AI_DEVID_VALID(x) \
	((((x) > (_MAX_AI_DEVICE_ID_NUM-1))) ? 1:0)
#define CHECK_AO_DEVID_VALID(x) \
	((((x) > (_MAX_AO_DEVICE_ID_NUM-1))) ? 1:0)
...
typedef struct _AIO_ATTR_S {
	...
	u32 u32FrmNum;
	/* frame num in buf[2,_MAX_AUDIO_FRAME_NUM] */
	u32 u32PtNumPerFrm;
	/* point num per frame (80/160/240/320/480/1024/2048) */
	/*(ADPCM IMA should add 1 point, AMR only support 160) */
	u32 u32ChnCnt;	/* channel number on FS, valid value:1/2/4/8 */
	u32 u32ClkSel;	/* 0: AI and AO clock is separate*/
...
typedef struct _AUDIO_FRAME_S {
	...
	u32  u32Len;                      /*data length per channel in frame*/
	u32  u32PoolId[2];
} AUDIO_FRAME_S;

typedef struct _AEC_FRAME_S {
	AUDIO_FRAME_S   stRefFrame;    /* AEC reference audio frame */
	u8         bValid;        /* whether frame is valid */
	u8         bSysBind;       /* whether is sysbind */
} AEC_FRAME_S;
...
typedef struct _AUDIO_STREAM_S {
	/* u8 ATTRIBUTE *pStream;the virtual address of stream */
	/* u64 ATTRIBUTE u64PhyAddr; the physics address of stream */
	u8  *pStream;         /* the virtual address of stream */
	u64  u64PhyAddr;      /* the physics address of stream */
	u32 u32Len;          /* stream length, by bytes */
	u64 u64TimeStamp;    /* frame time stamp*/
	u32 u32Seq;          /* frame seq,if stream is not a valid frame,u32Seq is 0*/
} AUDIO_STREAM_S;


typedef struct _AO_CHN_STATE_S {
	u32                  u32ChnTotalNum;    /* total number of channel buffer */
	u32                  u32ChnFreeNum;     /* free number of channel buffer */
	u32                  u32ChnBusyNum;     /* busy number of channel buffer */
} AO_CHN_STATE_S;
...
typedef struct _AI_AEC_CONFIG_S {
	u16 para_aec_filter_len;
	u16 para_aes_std_thrd;                 /* the threshold of STD/DTD, [0, 35] */
	u16 para_aes_supp_coeff;           /* the residual echo suppression level in AES, [0, 24] */
} AI_AEC_CONFIG_S;

typedef struct _AI_AEC_CONFIG_S_RTOS {
	u16 para_aec_filter_len;
	u16 para_aes_std_thrd;                 /* the threshold of STD/DTD, [0, 35] */
	u16 para_aes_supp_coeff;           /* the residual echo suppression level in AES, [0, 24] */
} __attribute__((packed)) AUDIO_ALIGNED AI_AEC_CONFIG_S_RTOS;
...
typedef s32(*pFuncGainCallBack)(s32 s32SetGain);




//#define AUDIO_ALGO_SSP	0x11//defalut algorithm
//#define AUDIO_ALGO_SSP_NOTCH	0x12//customize algorithm with notch filter
#define AUDIO_ALGO_ONLINE_PARAM  0xC0  //customize parameter from /mnt/data/audvqe.cfg
typedef struct _AUDIO_DELAY_CONFIG_S {
	/* the initial filter length of linear AEC to support up for echo tail, [1, 13] */
	u16 para_aec_init_filter_len;
	/* the digital gain target, [1, 12] */
	u16 para_dg_target;
	/* the delay sample for ref signal, [1, 3000] */
	u16 para_delay_sample;
} AUDIO_DELAY_CONFIG_S;


typedef struct _AUDIO_DELAY_CONFIG_S_RTOS {
	/* the initial filter length of linear AEC to support up for echo tail, [1, 13] */
	u16 para_aec_init_filter_len;
	/* the digital gain target, [1, 12] */
	u16 para_dg_target;
...
typedef struct _AUDIO_AGC_CONFIG_S_RTOS {
	/* the max boost gain for AGC release processing, [0, 3] */
	/* para_obj.para_agc_max_gain = 1; */
	s8 para_agc_max_gain;
	/* the gain level of target high of AGC, [0, 36] */
	...
	/* para_obj.para_agc_vad_enable = 1; */
	u8 para_agc_vad_ena;
} __attribute__((packed)) AUDIO_ALIGNED AUDIO_AGC_CONFIG_S_RTOS;
...
typedef struct _AUDIO_SPK_EQ_CONFIG_S {
	u16 para_spk_eq_nband;
	u16 para_spk_eq_freq[5];
	u16 para_spk_eq_gain[5];
	u16 para_spk_eq_qfactor[5];
...
typedef struct _AUDIO_ANR_CONFIG_S {
	...
	u16 para_nr_snr_coeff;
	/* the coefficient of NR noise tracking, [0, 14] */
	/* para_obj.para_nr_noise_coeff = 2; */
	//s8 para_nr_noise_coeff;
	u16 para_nr_init_sile_time;
} AUDIO_ANR_CONFIG_S;

typedef struct _AUDIO_ANR_CONFIG_S_RTOS {
	/* the coefficient of NR priori SNR tracking, [0, 20] */
	/* para_obj.para_nr_snr_coeff = 15; */
	u16 para_nr_snr_coeff;
	/* the coefficient of NR noise tracking, [0, 14] */
	...
	//s8 para_nr_noise_coeff;
	u16 para_nr_init_sile_time;
} __attribute__((packed)) AUDIO_ALIGNED AUDIO_ANR_CONFIG_S_RTOS;

typedef struct _AI_TALKVQE_CONFIG_S {
	u16	para_client_config;
	u32	u32OpenMask;
	s32	s32WorkSampleRate;
	...
	s32 s32RevMask;//turn this flag to default 0x11
	s32 para_notch_freq;//user can ignore this flag
	s8 customize[MAX_AUDIO_VQE_CUSTOMIZE_NAME];
} AI_TALKVQE_CONFIG_S;

typedef struct _AI_TALKVQE_CONFIG_S_RTOS {
	u16	para_client_config;
	u32	u32OpenMask;
	s32	s32WorkSampleRate;
	/* Sample Rate: 8KHz/16KHz. default: 8KHz*/
	...
	s32 para_notch_freq;//user can ignore this flag
	s8 customize[MAX_AUDIO_VQE_CUSTOMIZE_NAME];
} __attribute__((packed)) AUDIO_ALIGNED AI_TALKVQE_CONFIG_S_RTOS;
...
typedef struct _AUDIO_SAVE_FILE_INFO_S {
	u8	bCfg;
	s8	aFilePath[MAX_AUDIO_FILE_PATH_LEN];
	s8	aFileName[MAX_AUDIO_FILE_NAME_LEN];
	u32		u32FileSize;  /*in KB*/
...
typedef struct _ST_AUDIO_SPEEDPLAY_USR_CONFIG {
	int sampleRate;
	int channels;
	float speed;
	float pitch;
	float rate;
	float volume;
} ST_AO_SPEEDPLAY_CONFIG;

extern ST_AudioUnitTestCfg  stAudTestCfg;
/* at least one parameter is illegal ,eg, an illegal enumeration value  */
#define _ERR_AIO_ILLEGAL_PARAM     0xAA000001
/* using a NULL point */
#define _ERR_AIO_NULL_PTR          0xAA000002
/* operation is not supported by NOW */
#define _ERR_AIO_NOT_PERM          0xAA000003
/* vqe  err */
#define _ERR_AIO_REGISTER_ERR      0xAA000004

/* invalid device ID */
#define _ERR_AI_INVALID_DEVID     0xA0000005
/* invalid channel ID */
#define _ERR_AI_INVALID_CHNID     0xA0000006
/* at least one parameter is illegal ,eg, an illegal enumeration value  */
#define _ERR_AI_ILLEGAL_PARAM     0xA0000001
/* using a NULL point */
#define _ERR_AI_NULL_PTR          0xA0000002
/* try to enable or initialize system,device or channel, before configing attribute */
#define _ERR_AI_NOT_CONFIG        0xA0000007
/* operation is not supported by NOW */
#define _ERR_AI_NOT_SUPPORT       0xA0000008
/* operation is not permitted ,eg, try to change stati attribute */
#define _ERR_AI_NOT_PERM         0xA0000003
/* the devide is not enabled  */
#define _ERR_AI_NOT_ENABLED       0xA0000009
/* failure caused by malloc memory */
#define _ERR_AI_NOMEM             0xA000000A
/* failure caused by malloc buffer */
#define _ERR_AI_NOBUF             0xA000000B
/* no data in buffer */
#define _ERR_AI_BUF_EMPTY         0xA000000C
/* no buffer for new data */
#define _ERR_AI_BUF_FULL          0xA000000D
/* system is not ready,had not initialized or loaded*/
#define _ERR_AI_SYS_NOTREADY      0xA000000E

#define _ERR_AI_BUSY              0xA000000F
/* vqe  err */
#define _ERR_AI_VQE_ERR       0xA0000010
#define _ERR_AI_VQE_BUF_FULL       0xA0000011
#define _ERR_AI_VQE_FILE_UNEXIST       0xA0000012
/* invalid device ID */
#define _ERR_AO_INVALID_DEVID     0xA1000001
/* invalid channel ID */
#define _ERR_AO_INVALID_CHNID     0xA1000002
/* at least one parameter is illegal ,eg, an illegal enumeration value  */
#define _ERR_AO_ILLEGAL_PARAM     0xA1000003
/* using a NULL point */
#define _ERR_AO_NULL_PTR          0xA1000004
/* try to enable or initialize system,device or channel, before configing attribute */
#define _ERR_AO_NOT_CONFIG        0xA1000005
/* operation is not supported by NOW */
#define _ERR_AO_NOT_SUPPORT       0xA1000006
/* operation is not permitted ,eg, try to change stati attribute */
#define _ERR_AO_NOT_PERM          0xA1000007
/* the devide is not enabled  */
#define _ERR_AO_NOT_ENABLED       0xA1000008
/* failure caused by malloc memory */
#define _ERR_AO_NOMEM             0xA1000009
/* failure caused by malloc buffer */
#define _ERR_AO_NOBUF             0xA100000A
/* no data in buffer */
#define _ERR_AO_BUF_EMPTY         0xA100000B
/* no buffer for new data */
#define _ERR_AO_BUF_FULL          0xA100000C
/* system is not ready,had not initialized or loaded*/
#define _ERR_AO_SYS_NOTREADY      0xA100000D

#define _ERR_AO_BUSY              0xA100000E
/* vqe  err */
#define _ERR_AO_VQE_ERR       0xA100000F
...
#endif /* End of #ifndef ___COMM_AI_H__ */
