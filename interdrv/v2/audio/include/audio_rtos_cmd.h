 ...
 *
 * File Name: include/audio_rtos_cmd.h
 * Description:
 */

#ifndef __AUDIO_RTOS_CMD_H__
#define __AUDIO_RTOS_CMD_H__
...
#define AUDIO_SSP_SINGLE_CHUNK_SIZE_BYTES (160*2) //1 chunk = 160 samples
#define AUDIO_SSP_SINGLE_CHUNK_SAMPLES	(160)
#define AUDIO_MAX_BUFFER_SIZE	((320)*(AUDIO_SSP_CHUNK_NUMBERS))
...
//data struct for AUDIO_RTOS_CMD_SSP_INIT
typedef struct _ssp_rots_init {
	//AI_TALKVQE_CONFIG_S stAinVqeConfig;
	s32 s32RevMask;
	uint32_t CbPhyAddr;
	ST_SSP_BUFTBL	buffertbl[AUDIO_SSP_CHUNK_NUMBERS];
	char *pmic_in_vir;
	char *pref_in_vir;
	char *poutput_vir;
	char *CbVirAddr;
...
typedef struct _ssp_mic_vqe_addr {
	...
	AI_TALKVQE_CONFIG_S_RTOS *pAinVqeCfg;
} ST_SSP_MIC_VQE_ADDR;
...
typedef struct _ssp_mic_buf_table_addr {
	...
	uint64_t	ref_in_phy;
	uint64_t	output_phy;
	ST_SSP_BUFTBL	*pbuffertbl;//buffertbl[AUDIO_SSP_CHUNK_NUMBERS];
	char *pmic_in_vir;
	char *pref_in_vir;
	char *poutput_vir;
...
typedef struct _st_aduio_mailbox {
	uint64_t u64RevMask;
	uint64_t AinVqeCfgPhy;
	uint64_t buffertblPhy;
	uint64_t indicatorPhy;
...
//data struct for AUDIO_RTOS_CMD_SSP_INIT
//data struct for AUDIO_RTOS_CMD_SSP_PROCESS
...
//data struct for AUDIO_RTOS_CMD_SSP_SPK_INIT
typedef struct _ssp_spk_rtos_init {
	AO_VQE_CONFIG_S stAoutVqeConfig;
	ST_SPK_SSP_BUFTBL spkbuffertbl[AUDIO_SSP_CHUNK_NUMBERS];
} ST_SPK_SSP_RTOS_INIT;

//data struct for AUDIO_RTOS_CMD_SSP_SPK_PROCESS
...
#define AUDIO_RTOS_MAGIC_WORD_UNIT_TEST_MODE 0x38
#define AUDIO_RTOS_MAGIC_WORD_KERNEL_BIND_MODE 0x99
#define AUDIO_RTOS_TRIGGER_THRESHOLD	3
#define CVAUDIO_AUDIO_ION_SIZE_REQUIRE 0x1000
#ifndef AUDIO_BYTES_ALIGNMENT
#define AUDIO_BYTES_ALIGNMENT  64
#endif
#ifndef AUDIO_ALIGN
#define AUDIO_ALIGN(x, a)      (((x) + ((a)-1)) & ~((a)-1))
#endif
#define AUDIO_BUF_TBL_UNOCCUPIED 0
#define AUDIO_BUF_TBL_INPUT	1
#define AUDIO_BUF_TBL_AFTER_SSP 2

/*define the audio SSP CMD---------------start*/
#define AUDIO_RTOS_CMD_SSP_INIT 0x01
#define AUDIO_RTOS_CMD_SSP_DEINIT 0x02
#define AUDIO_RTOS_CMD_SSP_PROCESS 0x03
#define AUDIO_RTOS_CMD_SSP_SPK_INIT 0x04
#define AUDIO_RTOS_CMD_SSP_SPK_DEINIT 0x05
#define AUDIO_RTOS_CMD_SSP_SPK_PROCESS 0x06
#define AUDIO_RTOS_CMD_SSP_DEBUG 0x07
#define AUDIO_RTOS_CMD_SSP_UNIT_TEST 0x08
#define AUDIO_RTOS_CMD_SSP_UNIT_TEST_BLOCK_MODE_INIT	0x09
#define AUDIO_RTOS_CMD_SSP_UNIT_TEST_BLOCK_MODE_GET	0x0A

#define AUDIO_RTOS_CMD_SSP_MAX	0x0B


#define AUDIO_CMD_STATUS_RECEIVE 0x10
#define AUDIO_CMD_STATUS_OPERATE 0x20
#define AUDIO_CMD_STATUS_FINISHED 0x40
#define AUDIO_CMD_STATUS_ERROR    0x80
