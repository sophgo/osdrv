...
s32 cvi_ive_go(struct cvi_ive_device *ndev, IVE_TOP_C *ive_top_c,
		   bool bInstant, int done_mask, int optype, int dev_id);
...
s32 cvi_ive_dump_reg_state(bool enable);
...
s32 cvi_ive_reset(struct cvi_ive_device *ndev, int select, int dev_id);

s32 cvi_ive_test(struct cvi_ive_device *ndev, char *addr, u16 *w,
		     u16 *h);

s32 cvi_ive_cmdq(struct cvi_ive_device *ndev, int dev_id);
...
s32 cvi_ive_add(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc1,
		    IVE_SRC_IMAGE_S *pstSrc2, IVE_DST_IMAGE_S *pstDst,
		    IVE_ADD_CTRL_S *pstAddCtrl, bool bInstant, int dev_id);

s32 cvi_ive_sub(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc1,
		    IVE_SRC_IMAGE_S *pstSrc2, IVE_DST_IMAGE_S *pstDst,
		    IVE_SUB_CTRL_S *pstSubCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_dilate(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc,
		       ...
		       IVE_DILATE_CTRL_S *pstDilateCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_sobel(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc,
		      ...
		      IVE_SOBEL_CTRL_S *pstSobelCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_hist(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc,
		     IVE_DST_MEM_INFO_S *pstDst, bool bInstant, int dev_id);

s32 cvi_ive_map(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc,
		    u16 *pstMap, IVE_DST_IMAGE_S *pstDst,
		    IVE_MAP_CTRL_S *pstMapCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_integ(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc,
		      ...
		      IVE_INTEG_CTRL_S *pstIntegCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_grad_fg(struct cvi_ive_device *ndev,
		       ...
		       IVE_GRAD_FG_CTRL_S *pstGradFgCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_resize(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc,
		       IVE_DST_IMAGE_S *pstDst,
		       IVE_RESIZE_CTRL_S *pstResizeCtrl, bool bInstant, int dev_id);

s32 cvi_ive_imgIn_to_odma(struct cvi_ive_device *ndev,
			    IVE_SRC_IMAGE_S *pstSrc, IVE_DST_IMAGE_S *pstDst,
			    IVE_FILTER_CTRL_S *pstFltCtrl, bool bInstant, int dev_id);
...
s32 cvi_ive_EqualizeHist(struct cvi_ive_device *ndev,
			     IVE_SRC_IMAGE_S *pstSrc, IVE_DST_IMAGE_S *pstDst,
...
s32 CVI_MPI_IVE_CCL(IVE_HANDLE *pIveHandle, IVE_IMAGE_S *pstSrcDst,
			IVE_DST_MEM_INFO_S *pstBlob, IVE_CCL_CTRL_S *pstCclCtrl,
			bool bInstant);

s32 CVI_MPI_IVE_CannyEdge(IVE_IMAGE_S *pstEdge, IVE_MEM_INFO_S *pstStack);

s32 CVI_MPI_IVE_LKOpticalFlowPyr(
...
cvi_ive_frame_diff_motion(struct cvi_ive_device *ndev, IVE_SRC_IMAGE_S *pstSrc1,
			IVE_SRC_IMAGE_S *pstSrc2, IVE_DST_IMAGE_S *pstDst,
			IVE_FRAME_DIFF_MOTION_CTRL_S *ctrl, bool bInstant, int dev_id);
...
s32 CVI_MPI_IVE_STCorner(IVE_SRC_IMAGE_S *pstCandiCorner,
			     IVE_DST_MEM_INFO_S *pstCorner,
			     IVE_ST_CORNER_CTRL_S *pstStCornerCtrl);

s32 CVI_MPI_IVE_ANN_MLP_LoadModel(const CVI_CHAR *pchFileName,
				      IVE_ANN_MLP_MODEL_S *pstAnnMlpModel);

void CVI_MPI_IVE_ANN_MLP_UnloadModel(IVE_ANN_MLP_MODEL_S *pstAnnMlpModel);

s32 CVI_MPI_IVE_ANN_MLP_Predict(IVE_HANDLE *pIveHandle,
				    IVE_SRC_DATA_S *pstSrc,
				    IVE_LOOK_UP_TABLE_S *pstActivFuncTab,
				    IVE_ANN_MLP_MODEL_S *pstAnnMlpModel,
				    IVE_DST_DATA_S *pstDst, bool bInstant);

s32 CVI_MPI_IVE_Query(IVE_HANDLE IveHandle, bool *pbFinish,
			  bool bBlock);
