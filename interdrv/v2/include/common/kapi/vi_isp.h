...
struct sop_isp_sts_mem {
	...
	struct sop_vip_memblock gms;
	struct sop_vip_memblock ae_le;
	struct sop_vip_memblock ae_se;
	struct sop_vip_memblock awb;
	struct sop_vip_memblock awb_post;
	struct sop_vip_memblock dci;
	struct sop_vip_memblock hist_edge_v;
	struct sop_vip_memblock mmap;
};
...
struct sop_isp_usr_pic_cfg {
	struct sop_isp_mbus_framefmt fmt;
	struct sop_isp_rect crop;
};
...
struct sop_vip_isp_yuv_param {
	__u8   raw_num;
	__u32  yuv_bypass_path;
};

struct sop_isp_mmap_grid_size {
	__u8  raw_num;
	__u8  grid_size;
};
...
struct sop_vip_isp_awb_sts {
	__u8  raw_num;
	__u8  is_se;
	__u8  buf_idx;
};

struct sop_vi_dma_buf_info {
	__u64  paddr;
	__u32  size;
};
