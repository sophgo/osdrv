...
static inline s32 CHECK_GDC_FORMAT(VIDEO_FRAME_INFO_S imgIn, VIDEO_FRAME_INFO_S imgOut)
{
...
int gdc_handle_to_procIdx(struct cvi_ldc_job *job)
{
	...
	for (idx = 0; idx < GDC_PROC_JOB_INFO_NUM; ++idx) {
		...
		if (gdc_proc_ctx->stJobInfo[idx].eState == GDC_JOB_WORKING) {
			CVI_TRACE_LDC(CVI_DBG_WARN, "  [%d] hHandle=0x%llx, job=0x%llx\n", idx,
			      ...
			      struct cvi_ldc_job *job, struct gdc_task *tsk)
{
	...
	u32 bufWrapDepth = tsk->stTask.bufWrapDepth;
...
static void _ldc_op_done_cb(MOD_ID_E enModId, void *pParam, VB_BLK blk)
{
...
static void cvi_ldc_handle_hw_cb(struct cvi_ldc_vdev *wdev, struct cvi_ldc_job *job,
				 struct gdc_task *tsk, u8 is_timeout)
{
	...
	if (is_internal) {
		...
		isLastTask = (u8)tsk->stTask.au64privateData[1];
		...
		blk_in = vb_phys_addr2handle(tsk->stTask.stImgIn.stVFrame.u64PhyAddr[0]);
		blk_out = vb_phys_addr2handle(tsk->stTask.stImgOut.stVFrame.u64PhyAddr[0]);
		...
		if (tsk->stTask.au64privateData[0] &&
			...
			base_ion_free(tsk->stTask.au64privateData[0]);

		// release slice buffer
		if (tsk->stTask.bufWrapPhyAddr)
			base_ion_free(tsk->stTask.bufWrapPhyAddr);
	}
}
...
static int _notify_vpp_sb_mode(struct gdc_task *tsk)
{
	...
	ret = bind_get_dst(&stSrcChn, &stBindDest);
	if (ret) {
...
void gdc_job_worker(struct kthread_work *work)
{
	struct cvi_ldc_vdev *wdev =
		container_of(work, struct cvi_ldc_vdev, work);
	unsigned long flags;
	struct cvi_ldc_job *job, *tmp_job;
		...
		list_for_each_entry_safe(tsk, tmp_tsk, &job->task_list, node) {
			if (tsk->state == GDC_TASK_STATE_IDLE) {
				if (tsk->stTask.stBufWrap.bEnable) {
					...
					timeout = msecs_to_jiffies(LDC_HW_TIMEOUT_MS);
				...
				CVI_TRACE_LDC(CVI_DBG_WARN, "ldc hw busy, not done tsk yet\n");
				continue;
			}
			...
			if (ret == -ETIME) {
				CVI_TRACE_LDC(CVI_DBG_WARN, "ldc hw irq not respond\n");
...
int cvi_gdc_init(struct cvi_ldc_vdev *wdev)
{
	...
	if (ret)
		CVI_TRACE_LDC(CVI_DBG_WARN, "gdc thread priority update failed: %d\n", ret);

	if (IS_ERR(wdev->thread)) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "failed to create gdc kthread\n");
		return -1;
	}
...
s32 gdc_begin_job(struct cvi_ldc_vdev *wdev, struct gdc_handle_data *data)
{
	...
	job = kzalloc(sizeof(struct cvi_ldc_job), GFP_ATOMIC);
			  ...
			  struct gdc_task_attr *attr)
{
	...
	if (attr->enRotation == ROTATION_180) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "do not support rotation 180\n");
...
static s32 _calc_buf_wrp_depth(struct gdc_task *tsk, const struct ldc_buf_wrap_cfg *cfg, u32 *buf_wrp_depth)
{
	u32 min_buf_wrp_depth = 2;
	u32 min_buf_size, depth;
	u32 u32Width = tsk->stTask.stImgOut.stVFrame.u32Width;
	u32 u32BufLine = cfg->stBufWrap.u32BufLine;
	u32 u32WrapBufferSize = cfg->stBufWrap.u32WrapBufferSize;
...
{
	s32 s32Ret = CVI_FAILURE;
	struct cvi_ldc_job *job, *tmp, *wrp_job;
	struct gdc_task *tsk, *tmp2;
	const struct gdc_task_attr *wrp_tsk_attr;
