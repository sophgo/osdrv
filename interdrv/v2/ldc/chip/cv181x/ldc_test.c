...
void ldc_dump_register(void)
{
	...
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_FORMAT=0x%08x\n", _reg_read(ldc_base + LDC_FORMAT));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_RAS_MODE=0x%08x\n", _reg_read(ldc_base + LDC_RAS_MODE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_RAS_XSIZE=0x%08x\n", _reg_read(ldc_base + LDC_RAS_XSIZE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_RAS_YSIZE=0x%08x\n", _reg_read(ldc_base + LDC_RAS_YSIZE));

	val = _reg_read(ldc_base + LDC_MAP_BASE);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_MAP_BASE=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    addr=0x%08x\n", val << LDC_BASE_ADDR_SHIFT);

	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_MAP_BYPASS=0x%08x\n", _reg_read(ldc_base + LDC_MAP_BYPASS));

	val = _reg_read(ldc_base + LDC_SRC_BASE_Y);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_BASE_Y=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    addr=0x%08x\n", val << LDC_BASE_ADDR_SHIFT);

	val = _reg_read(ldc_base + LDC_SRC_BASE_C);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_BASE_C=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    addr=0x%08x\n", val << LDC_BASE_ADDR_SHIFT);

	val = _reg_read(ldc_base + LDC_SRC_XSIZE);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_XSIZE=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    src_xsize=%d\n",
		(val & LDC_REG_SRC_XSIZE_MASK) >> LDC_REG_SRC_XSIZE_OFFSET);

	val = _reg_read(ldc_base + LDC_SRC_YSIZE);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_YSIZE=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    src_ysize=%d\n",
		(val & LDC_REG_SRC_YSIZE_MASK) >> LDC_REG_SRC_YSIZE_OFFSET);

	val = _reg_read(ldc_base + LDC_SRC_XSTR);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_XSTR=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    src_xstr=%d\n",
		(val & LDC_REG_SRC_XSTR_MASK) >> LDC_REG_SRC_XSTR_OFFSET);

	val = _reg_read(ldc_base + LDC_SRC_XEND);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_XEND=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    src_xend=%d\n",
		(val & LDC_REG_SRC_XEND_MASK) >> LDC_REG_SRC_XEND_OFFSET);

	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SRC_BG=0x%08x\n", _reg_read(ldc_base + LDC_SRC_BG));

	val = _reg_read(ldc_base + LDC_DST_BASE_Y);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_DST_BASE_Y=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    addr=0x%08x\n", val << LDC_BASE_ADDR_SHIFT);

	val = _reg_read(ldc_base + LDC_DST_BASE_C);
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_DST_BASE_C=0x%08x\n", val);
	CVI_TRACE_LDC(CVI_DBG_INFO, "    addr=0x%08x\n", val << LDC_BASE_ADDR_SHIFT);

	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_DST_MODE=0x%08x\n", _reg_read(ldc_base + LDC_DST_MODE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_IRQEN=0x%08x\n", _reg_read(ldc_base + LDC_IRQEN));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_START=0x%08x\n", _reg_read(ldc_base + LDC_START));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_IRQSTAT=0x%08x\n", _reg_read(ldc_base + LDC_IRQSTAT));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_IRQCLR=0x%08x\n", _reg_read(ldc_base + LDC_IRQCLR));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_MODE=0x%08x\n", _reg_read(ldc_base + LDC_SB_MODE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_NB=0x%08x\n", _reg_read(ldc_base + LDC_SB_NB));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_SIZE=0x%08x\n", _reg_read(ldc_base + LDC_SB_SIZE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_SET_STR=0x%08x\n", _reg_read(ldc_base + LDC_SB_SET_STR));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_SW_WPTR=0x%08x\n", _reg_read(ldc_base + LDC_SB_SW_WPTR));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_SW_CLR=0x%08x\n", _reg_read(ldc_base + LDC_SB_SW_CLR));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_FULL=0x%08x\n", _reg_read(ldc_base + LDC_SB_FULL));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_EMPTY=0x%08x\n", _reg_read(ldc_base + LDC_SB_EMPTY));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_WPTR_RO=0x%08x\n", _reg_read(ldc_base + LDC_SB_WPTR_RO));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_DPTR_RO=0x%08x\n", _reg_read(ldc_base + LDC_SB_DPTR_RO));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_SB_STRIDE=0x%08x\n", _reg_read(ldc_base + LDC_SB_STRIDE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_LDC_DIR=0x%08x\n", _reg_read(ldc_base + LDC_LDC_DIR));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_CMDQ_IRQ_EN=0x%08x\n", _reg_read(ldc_base + LDC_REG_CMDQ_IRQ_EN));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_FORCE_IN_RANGE=0x%08x\n", _reg_read(ldc_base + LDC_LDC_FORCE_IN_RANGE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_OUT_RANGE=0x%08x\n", _reg_read(ldc_base + LDC_LDC_OUT_RANGE));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_OUT_RANGE_DST_X=0x%08x\n", _reg_read(ldc_base + LDC_LDC_OUT_RANGE_DST_X));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_OUT_RANGE_DST_Y=0x%08x\n", _reg_read(ldc_base + LDC_LDC_OUT_RANGE_DST_Y));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_OUT_RANGE_SRC_X=0x%08x\n", _reg_read(ldc_base + LDC_LDC_OUT_RANGE_SRC_X));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_OUT_RANGE_SRC_Y=0x%08x\n", _reg_read(ldc_base + LDC_LDC_OUT_RANGE_SRC_Y));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_DST_TI_CNT_X=0x%08x\n", _reg_read(ldc_base + LDC_LDC_DST_TI_CNT_X));
	CVI_TRACE_LDC(CVI_DBG_INFO, "LDC_DST_TI_CNT_Y=0x%08x\n", _reg_read(ldc_base + LDC_LDC_DST_TI_CNT_Y));
}
...
void ldc_test_irq_handler(uint32_t intr_status)
{
	if (ldc_tst_enable_cmdq) {
		...
		if (status) {
			...
			CVI_TRACE_LDC(CVI_DBG_INFO, "status(%#x), cmdq-status(0x%x)\n", intr_status, status);

			if ((status & 0x04) && cmdq_is_sw_restart(REG_LDC_CMDQ_BASE)) {
				CVI_TRACE_LDC(CVI_DBG_INFO, "cmdq-sw restart\n", __func__);
			...
			CVI_TRACE_LDC(CVI_DBG_INFO, "status(%#x), cmdq-status not found\n", intr_status);
		}
	} else {
		ldc_intr_clr(intr_status);
		++ldc_tst_mark;
		CVI_TRACE_LDC(CVI_DBG_INFO, "status(%#x)\n", intr_status);
	}
}
...
static int ldc_check_irq_handler(unsigned int timeout)
{
	...
	for (i = 0; i < timeout; i += step) {
		if (ldc_tst_mark > 0) {
			ldc_tst_mark = 0;
			CVI_TRACE_LDC(CVI_DBG_INFO, "check tick:%d\n", i);
			return 0;
		}
		udelay(step);
	}
...
static int ldc_test1(void)
{
	...
	if (ion_fd < 0) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "ion allocate fail\n");
		return -1;
	}

	dmabuf = dma_buf_get(ion_fd);
	if (!dmabuf) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "dma_buf_get fail\n");
		return -1;
	}
	...
	if (ret < 0) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "dma_buf_begin_cpu_access fail.\n");
		return ret;
	}
	...
	CVI_TRACE_LDC(CVI_DBG_INFO, "src buf address Y(%#x)\n", cfg.src_y_base);
	CVI_TRACE_LDC(CVI_DBG_INFO, "dst buf address Y(%#x)\n", cfg.dst_y_base);
	...
	if (status) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "ldc intr status(%d) doesn't cleared\n", status);
		ret = -1;
	}
	...
	if (ldc_check_irq_handler(60) != 0) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "ldc end intr doesn't happens after 60 ms\n");
		ret = -1;
	} else {
		CVI_TRACE_LDC(CVI_DBG_INFO, "ldc done\n");
	}

	status = ldc_intr_status();
	CVI_TRACE_LDC(CVI_DBG_INFO, "ldc intr status(%d) after kicked\n", status);

	for (i = 0; i < 64 * 64; i++) {
		if (src[i] != dst[i]) {
			CVI_TRACE_LDC(CVI_DBG_ERR, "  [%d] src=%d, dst=%d, error !\n", i, src[i], dst[i]);
			ret = -1;
			break;
		}
	}

	CVI_TRACE_LDC(CVI_DBG_INFO, "%s\n", ret ? "fail":"pass");
...
static ssize_t ldc_test_proc_write(struct file *file, const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (kstrtouint_from_user(user_buf, count, 0, &input_param)) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "input parameter incorrect\n");
		return count;
	}

	//reset related info
	CVI_TRACE_LDC(CVI_DBG_ERR, "input_param=%d\n", input_param);
...
int32_t ldc_test_proc_init(void)
{
	...
	if (proc_create(PROC_NAME, 0644, NULL, &ldc_test_proc_ops) == NULL) {
		CVI_TRACE_LDC(CVI_DBG_ERR, "sclr_test: : proc_init() failed\n");
		ret = -1;
	}
