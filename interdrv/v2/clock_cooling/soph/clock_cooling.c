...
/**
 * cv186x_cooling_get_max_state - callback function to get the max cooling state.
 ...
 */
static int cv186x_cooling_get_max_state(struct thermal_cooling_device *cdev,
					unsigned long *state)
{
...
/**
 * cv186x_cooling_get_cur_state - function to get the current cooling state.
 * @cdev: thermal cooling device pointer.
 ...
 * Return: 0 (success)
 */
static int cv186x_cooling_get_cur_state(struct thermal_cooling_device *cdev,
					unsigned long *state)
{
	struct cv186x_cooling_device *cvcdev = cdev->devdata;
...
/**
 * cv186x_cooling_set_cur_state - function to set the current cooling state.
 * @cdev: thermal cooling device pointer.
 * @state: set this variable to the current cooling state.
 *
 * Callback for the thermal cooling device to change the cv186x cooling
...
static struct thermal_cooling_device_ops const cv186x_cooling_ops = {
	.get_max_state = cv186x_cooling_get_max_state,
	.get_cur_state = cv186x_cooling_get_cur_state,
	.set_cur_state = cv186x_cooling_set_cur_state,
};

static struct cv186x_cooling_device *
cv186x_cooling_device_register(struct device *dev)
{
	struct thermal_cooling_device *cdev;
	struct cv186x_cooling_device *cvcdev = NULL;
...
static void cv186x_cooling_device_unregister(struct cv186x_cooling_device
						 *cvcdev)
{
	thermal_cooling_device_unregister(cvcdev->cdev);
}

static int cv186x_cooling_probe(struct platform_device *pdev)
{
	struct cv186x_cooling_device *cvcdev;

	cvcdev = cv186x_cooling_device_register(&pdev->dev);
	if (IS_ERR(cvcdev)) {
...
MODULE_DEVICE_TABLE(of, cv186x_cooling_match);

static struct platform_driver cv186x_cooling_driver = {
	.driver = {
		.name = "cv186x-cooling",
		.of_match_table = of_match_ptr(cv186x_cooling_match),
	},
	.probe = cv186x_cooling_probe,
	.remove = cv186x_cooling_remove,
};

module_platform_driver(cv186x_cooling_driver);

MODULE_AUTHOR("fisher.cheng@cvitek.com");
MODULE_DESCRIPTION("cv186x cooling driver");
MODULE_LICENSE("GPL");
