			...
			for (j = 0; j < bindNodes[i].dsts.u32Num; ++j) {
				...
				if (nextBindNode != NULL) {
					...
					seq_printf(m, "%-10s%-10d%-10d%-10s%-10d%-10d%-10s%-10d%-10d\n",
						sys_get_modname(first->enModId), first->s32DevId, first->s32ChnId,
						sys_get_modname(second->enModId), second->s32DevId, second->s32ChnId,
						sys_get_modname(third->enModId), third->s32DevId, third->s32ChnId);
					}
