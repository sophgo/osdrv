...
static osal_spinlock vi_sys_lock;
...
osal_module_export(vi_sys_register_cmm_cb);
...
osal_module_export(vi_sys_cmm_cb_i2c);
...
osal_module_export(vi_sys_cmm_cb_ssp);
...
void vi_sys_set_base_addr(void *base)
{
	...
	if (base)
		osal_spin_lock_init(&vi_sys_lock);
	else
		osal_spin_lock_destroy(&vi_sys_lock);
}
osal_module_export(vi_sys_set_base_addr);
...
void vi_sys_toggle_reset(union vi_sys_reset mask)
{
	...
	osal_spin_lock_irqsave(&vi_sys_lock, &flags);
	...
	value.raw &= ~mask.raw;
	vi_sys_set_reset(value);
	osal_spin_unlock_irqrestore(&vi_sys_lock, &flags);
}
osal_module_export(vi_sys_toggle_reset);
...
osal_module_export(vi_sys_toggle_reset_apb);
...
osal_module_export(vi_sys_get_intr_status);
...
osal_module_export(vi_set_intr_mask);
...
osal_module_export(vi_sys_set_offline);
