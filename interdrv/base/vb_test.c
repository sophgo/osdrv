...
static int32_t _vb_get_block_test(void)
{
	...
	if (ret) {
		pr_err("vb_get_config fail, ret:%d\n", ret);
	...
	for (i = 0; i < cfg.comm_pool_cnt; ++i) {
		...
		if (blk == VB_INVALID_HANDLE) {
			pr_err("vb_get_block_with_id fail\n");
			goto GET_BLOCK_TEST_FAIL;
		}

		pool = vb_handle2PoolId(blk);
		if (pool != (uint32_t)i) {
			pr_err("Error:pool id:%d, expected pool id should be:%d\n", pool, i);
			goto GET_BLOCK_TEST_FAIL;
		}

		ret = vb_inquireUserCnt(blk, &usr_cnt);
		if (ret || usr_cnt != 1) {
			pr_err("vb_inquireUserCnt fail, ret:%d, usr_cnt:%d\n", ret, usr_cnt);
			goto GET_BLOCK_TEST_FAIL;
		}

		phy_addr = vb_handle2PhysAddr(blk);
		if (phy_addr == 0) {
			pr_err("vb_handle2PhysAddr fail\n");
			goto GET_BLOCK_TEST_FAIL;
		}
		tmp_blk = vb_physAddr2Handle(phy_addr);
		if (tmp_blk != blk) {
			pr_err("Error: should be the same blk\n");
			goto GET_BLOCK_TEST_FAIL;
		}

		pVirAddr = vb_handle2VirtAddr(blk);
		if (!pVirAddr) {
			pr_err("vb_handle2VirtAddr fail\n");
			goto GET_BLOCK_TEST_FAIL;
		}
		...
		if (memcmp(pTestBuf, pVirAddr, buf_len) != 0) {
			pr_err("memcpy test fail\n");
			vfree(pTestBuf);
			goto GET_BLOCK_TEST_FAIL;
		}
		vfree(pTestBuf);
		pr_info("memcpy(%d bytes)  %d - %d\n", buf_len, duration0, duration1);

		ret = vb_release_block(blk);
		if (ret) {
			pr_err("vb_release_block fail\n");
			goto GET_BLOCK_TEST_FAIL;
		}
		ret = vb_inquireUserCnt(blk, &usr_cnt);
		if (ret || usr_cnt != 0) {
	...
	pr_info("vb get block test SUCCESS!\n");
	return 0;
...
static int32_t _vb_create_pool_test(void)
{
	...
	if (ret) {
		pr_err("vb_create_pool fail, ret:%d\n", ret);
	...
	if (ret) {
		pr_err("vb_destroy_pool fail, pool id:%d\n", pool);
		goto GREATE_POOL_TEST_FAIL;
	}
	pr_info("vb create pool test SUCCESS!\n");
	return 0;
...
static int32_t _vb_create_blk_test(void)
{
	...
	if (blk == VB_INVALID_HANDLE) {
		pr_err("vb get static blk fail\n");
		goto GREATE_BLK_TEST_FAIL;
	}

	vb = (struct vb_s *)blk;
	if (vb->vb_pool != VB_STATIC_POOLID) {
		pr_err("Error: pool id:%d, should be VB_STATIC_POOLID\n", vb->vb_pool);
		goto GREATE_BLK_TEST_FAIL;
	}
	...
	tmp_blk = vb_physAddr2Handle(phy_addr);
	if (tmp_blk != blk) {
		pr_err("Error: Unexpected blk-phy_addr pair\n");
		goto GREATE_BLK_TEST_FAIL;
	}
	...
	if (tmp_blk != VB_INVALID_HANDLE) {
		pr_err("Error: vb_physAddr2Handle should fail after release static blk\n");
		goto GREATE_BLK_TEST_FAIL;
	}
	...
	blk = vb_create_block(phy_addr, ion_v, 0, true);
	if (blk == VB_INVALID_HANDLE) {
		pr_err("vb_create_block fail\n");
		goto GREATE_BLK_TEST_FAIL;
	}
	...
	tmp_blk = vb_physAddr2Handle(phy_addr);
	if (tmp_blk != VB_INVALID_HANDLE) {
		pr_err("Error: vb_physAddr2Handle should fail after release ext blk\n");
		goto GREATE_BLK_TEST_FAIL;
	}
	sys_ion_free(phy_addr);
	pr_info("vb create blk test SUCCESS!\n");
	return 0;
...
static int32_t _vb_qbuf_test(void)
{
	...
	if (ret == CVI_SUCCESS) {
		pr_err("vb_qbuf should fail due to workq full\n");
		goto QBUF_TEST_FAIL;
	}
	...
	for (i = 0; i < 2; ++i) {
		...
		if (workq_blk != blk[i]) {
			pr_err("vb_dqbuf fail, blk idx(%d)\n", i);
			goto QBUF_TEST_FAIL;
		}
		...
		if (ret) {
			pr_err("base_get_chn_buffer fail, blk idx(%d)\n", i);
			goto QBUF_TEST_FAIL;
		}
		if (doneq_blk != blk[i]) {
			pr_err("get unexpected blk from doneq, blk idx(%d)\n", i);
			goto QBUF_TEST_FAIL;
		}
		vb_release_block(blk[i]);
		...
		if (vb->usr_cnt.counter != 0) {
			pr_err("usr_cnt should be zero after release blk, blk idx(%d)\n", i);
			goto QBUF_TEST_FAIL;
		}
	}

	base_mod_jobs_exit(chn, CHN_TYPE_OUT);
	pr_info("vb qbuf test SUCCESS!\n");
	return 0;
...
static int32_t _vb_jobs_test(void)
{
	...
	if (base_mod_jobs_waitq_empty(chn, CHN_TYPE_IN)) {
		pr_err("waitq should not be empty\n");
		goto JOBS_TEST_FAIL;
	}

	tmp_blk = base_mod_jobs_waitq_pop(chn, CHN_TYPE_IN);
	if (tmp_blk != blk) {
		pr_err("base_mod_jobs_waitq_pop fail\n");
		goto JOBS_TEST_FAIL;
	}
	vb_release_block(blk);
	if (!base_mod_jobs_waitq_empty(chn, CHN_TYPE_IN)) {
		pr_err("waitq should be empty\n");
		goto JOBS_TEST_FAIL;
	}
	...
	if (base_mod_jobs_workq_empty(chn, CHN_TYPE_IN)) {
		pr_err("workq should not be empty\n");
		goto JOBS_TEST_FAIL;
	}
	if (!base_mod_jobs_waitq_empty(chn, CHN_TYPE_IN)) {
		pr_err("waitq should be empty\n");
	...
	if (tmp_blk != blk) {
		pr_err("base_mod_jobs_workq_pop fail\n");
		goto JOBS_TEST_FAIL;
	}
	vb_release_block(blk);
	if (!base_mod_jobs_workq_empty(chn, CHN_TYPE_IN)) {
		pr_err("workq should be empty\n");
		goto JOBS_TEST_FAIL;
	}

	vb_release_block(blk);
	base_mod_jobs_exit(chn, CHN_TYPE_IN);
	pr_info("vb jobs test SUCCESS!\n");
	return 0;
...
static int32_t _vb_acquire_block_test(void)
{
	...
	for (loop = 0; loop < 2; ++loop) {
		...
		for (i = 0; i < blk_cnt; ++i) {
			...
			if (blk[i] == VB_INVALID_HANDLE) {
				pr_err("vb_get_block_with_id fail, idx(%d)\n", i);
				goto ACQUIRE_BLK_TEST_FAIL;
			}
		}
		...
		if (tmp_blk != VB_INVALID_HANDLE) {
			pr_err("vb_get_block_with_id should be failed due to no free blk\n");
			goto ACQUIRE_BLK_TEST_FAIL;
		}
		...
		if (loop == 1) {
			if (test_cnt != old_test_cnt) {
				pr_err("_vb_acquire_block_test fail, cnt(%d) old_cnt(%d) loop(%d)\n",
	...
	pr_info("vb acquire block test SUCCESS!\n");
	return 0;
...
int32_t vb_unit_test(int32_t op)
{
	...
	switch (op) {
	case 1: {
		pr_info("vb get block test\n");
		ret = _vb_get_block_test();
		break;
	}
	case 2: {
		pr_info("vb create pool test\n");
		ret = _vb_create_pool_test();
		break;
	}
	case 3: {
		pr_info("vb create blk test\n");
		ret = _vb_create_blk_test();
		break;
	}
	case 4: {
		pr_info("vb qbuf test\n");
		ret = _vb_qbuf_test();
		break;
	}
	case 5: {
		pr_info("vb jobs test\n");
		ret = _vb_jobs_test();
		break;
	}
	case 6: {
		pr_info("vb acquire blk test\n");
