...
int base_set_mod_ctx(struct mod_ctx_s *ctx_s)
{
	if (!ctx_s || !ctx_s->ctx_info) {
		pr_err("set mod ctx error: data is NULL\n");
		return -1;
	}
				...
				uint8_t waitq_depth, uint8_t workq_depth, uint8_t doneq_depth)
{
	...
	if (jobs == NULL) {
		pr_err("mod(%s) job init fail, Null parameter\n", sys_get_modname(chn.enModId));
		return;
	}

	if (jobs->inited) {
		pr_err("mod(%s) job init fail, already inited\n", sys_get_modname(chn.enModId));
		return;
	}
...
void base_mod_jobs_exit(MMF_CHN_S chn, enum CHN_TYPE_E chn_type)
{
	...
	if (jobs == NULL) {
		pr_err("mod(%s) job exit fail, Null parameter\n", sys_get_modname(chn.enModId));
		return;
	}

	if (!jobs->inited) {
		pr_err("mod(%s) job exit fail, not inited yet\n", sys_get_modname(chn.enModId));
		return;
	}
...
int32_t vb_qbuf(MMF_CHN_S chn, enum CHN_TYPE_E chn_type, VB_BLK blk)
{
	...
	if (chn_type == CHN_TYPE_OUT) {
		...
		if (FIFO_FULL(&jobs->workq)) {
			...
			pr_debug("%s workq is full. drop new one.\n"
				     , sys_get_modname(chn.enModId));
			return -ENOBUFS;
		}
		...
		if (base_qbuf_cb[chn.enModId]) {
			...
			if (ret != CVI_SUCCESS) {
				atomic_fetch_sub(1, &vb->usr_cnt);
				pr_err("%s qbuf_cb fail.", sys_get_modname(chn.enModId));
				return ret;
			}
		}
		mutex_lock(&jobs->lock);
		...
		if (FIFO_FULL(&jobs->waitq)) {
			mutex_unlock(&jobs->lock);
			atomic_fetch_sub(1, &vb->usr_cnt);
			pr_debug("%s waitq is full. drop new one.\n"
				     , sys_get_modname(chn.enModId));
			return -ENOBUFS;
		}
