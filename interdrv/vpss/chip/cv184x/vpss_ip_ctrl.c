	...
	const struct sclr_size *size)
{
	...
	struct sclr_gop_fb_cfg fb_cfg;
	u8 bpp = 0, ow_idx, i;
	u32 fb_num_tmp, pix_num;
	...
	if (rgn_cfg->odec.enable) { // odec enable
		...
		for (ow_idx = 0; ow_idx < rgn_cfg->num_of_rgn; ++ow_idx) {
			...
			if (ow_cfg->fmt == SCL_GOP_FMT_FONT) {
				ow_cfg->mem_size.w = ALIGN((ow_cfg->img_size.w + 7) / 8, GOP_ALIGNMENT);
				gop_cfg->font_fg_color = 0xffff;
				gop_cfg->font_bg_color = 0x0;
				gop_cfg->font_fg_inv_color = 0xf000;
				gop_cfg->font_bg_inv_color = 0x0;

				for (i = 0; i < 2; i++) {
					if (rgn_cfg->invert_param[i].rgn_idx == ow_idx) {
						if (rgn_cfg->invert_param[i].invert_color.inv_col_en) {
							fb_num_tmp = ow_cfg->img_size.w /
								rgn_cfg->invert_param[i].invert_color.
								inv_col_area.width;
							fb_cfg.fb_ctrl.b.enable = 1;
							fb_cfg.fb_ctrl.b.attach_ow = ow_idx;
							gop_cfg->fb_ctrl.b.hi_thr =
								rgn_cfg->invert_param[i].invert_color.lum_thresh >> 2;
							gop_cfg->fb_ctrl.b.lo_thr =
								(gop_cfg->fb_ctrl.b.hi_thr * 8 + 5) / 10;
							gop_cfg->fb_ctrl.b.lo_thr_inv =
								rgn_cfg->invert_param[i].invert_color.chg_mod;
							fb_cfg.fb_ctrl.b.fb_num = fb_num_tmp - 1;
							fb_cfg.fb_ctrl.b.width = ow_cfg->img_size.w / fb_num_tmp - 1;
							pix_num = (fb_cfg.fb_ctrl.b.width + 1) * ow_cfg->img_size.h;
							fb_cfg.fb_ctrl.b.sample_rate = (pix_num >> 3) >> 5;
							fb_cfg.fb_ctrl.b.pix_thr =
								(((pix_num >> 3) >> fb_cfg.fb_ctrl.b.sample_rate) *
								65 + 50) / 100;
							sclr_gop_fb_set_cfg(inst, layer, i, &fb_cfg);
						}
					}
				}
			}
...
static void _sc_ext_set_sb(u8 dev_idx, const struct odma_sb_cfg *sb_cfg)
{
	struct sclr_odma_sb_cfg odma_sb_cfg;
	struct sclr_top_sb_cfg top_cfg;

	odma_sb_cfg.sb_cfg.raw = 0;

	//sb_mode: 0: disable, 1: free-run mode, 2: frame-base mode
	if (sb_cfg->sb_mode == 0) {
		odma_sb_cfg.sb_cfg.b.sb_mode = 0;
		sclr_odma_set_sb(dev_idx, &odma_sb_cfg);
		return;
	}

	if (dev_idx != VPSS_V0) {
		TRACE_VPSS(DBG_ERR, "Only vpss-%d support sbm\n", VPSS_V0);
		return;
	}

	// top
	top_cfg.ctrl.raw = 0;
	if (sb_cfg->sb_wr_ctrl_idx)
		top_cfg.ctrl.b.sb_wr_ctrl1 = 1;
	else
		top_cfg.ctrl.b.sb_wr_ctrl0 = 1;

	sclr_top_set_sb(dev_idx, &top_cfg);

	// odma
	odma_sb_cfg.sb_cfg.b.sb_mode = sb_cfg->sb_mode;
	odma_sb_cfg.sb_cfg.b.sb_size = sb_cfg->sb_size;
	odma_sb_cfg.sb_cfg.b.sb_nb = sb_cfg->sb_nb - 1;
	odma_sb_cfg.sb_cfg.b.sb_full_nb = sb_cfg->sb_full_nb - 2;
	sclr_odma_set_sb(dev_idx, &odma_sb_cfg);
}
...
int vpss_stitch_run(u8 dev_idx, struct vpss_cmdq_buf *cmdq_buf, struct vpss_stitch_cfg *cfg)
{
	...
	ctrl_cfg = (struct sclr_ctrl_cfg *)osal_vzalloc(sizeof(struct sclr_ctrl_cfg) * stitch_num);
	if (!ctrl_cfg) {
	...
	for (i = 0; i < stitch_num; i++) {
		window = &cfg->chn_cfg[i].window;

		if ((window->rect_in.left < window->rect_out.left) ||
			(window->rect_in.top < window->rect_out.top) ||
			((window->rect_out.left + window->rect_out.width) <
		...
		ctrl_cfg[i].input = SCL_INPUT_MEM;
		ctrl_cfg[i].src_fmt = user_fmt_to_hw(cfg->chn_cfg[i].pixelformat);
		ctrl_cfg[i].src_csc = get_img_csc(cfg->chn_cfg[i].pixelformat);
		ctrl_cfg[i].src.w = cfg->chn_cfg[i].src_size.width;
		ctrl_cfg[i].src.h = cfg->chn_cfg[i].src_size.height;
		ctrl_cfg[i].src_crop.x = cfg->chn_cfg[i].rect_crop.left;
		ctrl_cfg[i].src_crop.y = cfg->chn_cfg[i].rect_crop.top;
		ctrl_cfg[i].src_crop.w = cfg->chn_cfg[i].rect_crop.width;
		ctrl_cfg[i].src_crop.h = cfg->chn_cfg[i].rect_crop.height;
		ctrl_cfg[i].pitch_y = cfg->chn_cfg[i].bytesperline[0];
		ctrl_cfg[i].pitch_c = cfg->chn_cfg[i].bytesperline[1];
		ctrl_cfg[i].src_addr0 = cfg->chn_cfg[i].addr[0];
		ctrl_cfg[i].src_addr1 = cfg->chn_cfg[i].addr[1];
		ctrl_cfg[i].src_addr2 = cfg->chn_cfg[i].addr[2];

		ctrl_cfg[i].dst.inst = dev_idx;
		ctrl_cfg[i].dst.flip_mode = window->flip;
		ctrl_cfg[i].dst.fmt = user_fmt_to_hw(cfg->dst_cfg.pixelformat);
		ctrl_cfg[i].dst.csc = get_sc_csc(cfg->dst_cfg.pixelformat);
		ctrl_cfg[i].dst.addr0 = cfg->dst_cfg.addr[0];
		ctrl_cfg[i].dst.addr1 = cfg->dst_cfg.addr[1];
		ctrl_cfg[i].dst.addr2 = cfg->dst_cfg.addr[2];

		if (i == 0) {
			ctrl_cfg[i].border_cfg.cfg.b.enable = 1;
			ctrl_cfg[i].border_cfg.cfg.b.bd_color_r = cfg->dst_cfg.color[0];
			ctrl_cfg[i].border_cfg.cfg.b.bd_color_g = cfg->dst_cfg.color[1];
			ctrl_cfg[i].border_cfg.cfg.b.bd_color_b = cfg->dst_cfg.color[2];
			ctrl_cfg[i].border_cfg.start.x = window->rect_in.left;
			ctrl_cfg[i].border_cfg.start.y = window->rect_in.top;
			ctrl_cfg[i].dst.offset.x = 0;
			ctrl_cfg[i].dst.offset.y = 0;
			ctrl_cfg[i].dst.window.w = cfg->dst_cfg.dst_size.width;
			ctrl_cfg[i].dst.window.h = cfg->dst_cfg.dst_size.height;
			ctrl_cfg[i].dst.frame.w = window->rect_in.width;
			ctrl_cfg[i].dst.frame.h = window->rect_in.height;

			//background: vpp border[0]
			if ((window->rect_in.left != window->rect_out.left) ||
				(window->rect_in.top != window->rect_out.top) ||
				(window->rect_in.width != window->rect_out.width) ||
				(window->rect_in.height != window->rect_out.height)) {
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.enable = 1;
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.bd_color_r = window->bgcolor[0];
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.bd_color_g = window->bgcolor[1];
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.bd_color_b = window->bgcolor[2];
				ctrl_cfg[i].border_vpp_cfg[0].inside_start.x = window->rect_in.left;
				ctrl_cfg[i].border_vpp_cfg[0].inside_start.y = window->rect_in.top;
				ctrl_cfg[i].border_vpp_cfg[0].inside_end.x =
					window->rect_in.left + window->rect_in.width;
				ctrl_cfg[i].border_vpp_cfg[0].inside_end.y =
					window->rect_in.top + window->rect_in.height;
				ctrl_cfg[i].border_vpp_cfg[0].outside_start.x = window->rect_out.left;
				ctrl_cfg[i].border_vpp_cfg[0].outside_start.y = window->rect_out.top;
				ctrl_cfg[i].border_vpp_cfg[0].outside_end.x =
					window->rect_out.left + window->rect_out.width;
				ctrl_cfg[i].border_vpp_cfg[0].outside_end.y =
					window->rect_out.top + window->rect_out.height;
			}

			//border vpp border[1]
			if (window->top_width || window->left_width ||
				window->right_width || window->bottom_width) {
				ctrl_cfg[i].border_vpp_cfg[1].cfg.b.enable = 1;
				ctrl_cfg[i].border_vpp_cfg[1].cfg.b.bd_color_r = window->border_color[0];
				ctrl_cfg[i].border_vpp_cfg[1].cfg.b.bd_color_g = window->border_color[1];
				ctrl_cfg[i].border_vpp_cfg[1].cfg.b.bd_color_b = window->border_color[2];
				ctrl_cfg[i].border_vpp_cfg[1].inside_start.x =
					window->rect_out.left + window->left_width;
				ctrl_cfg[i].border_vpp_cfg[1].inside_start.y =
					window->rect_out.top + window->top_width;
				ctrl_cfg[i].border_vpp_cfg[1].inside_end.x =
					window->rect_out.left + window->rect_out.width - window->right_width;
				ctrl_cfg[i].border_vpp_cfg[1].inside_end.y =
					window->rect_out.top + window->rect_out.height - window->bottom_width;
				ctrl_cfg[i].border_vpp_cfg[1].outside_start.x = window->rect_out.left;
				ctrl_cfg[i].border_vpp_cfg[1].outside_start.y = window->rect_out.top;
				ctrl_cfg[i].border_vpp_cfg[1].outside_end.x =
					window->rect_out.left + window->rect_out.width;
				ctrl_cfg[i].border_vpp_cfg[1].outside_end.y =
					window->rect_out.top + window->rect_out.height;
			}
		} else {
			ctrl_cfg[i].border_cfg.cfg.b.enable = 1;
			ctrl_cfg[i].border_cfg.cfg.b.bd_color_r = window->bgcolor[0];
			ctrl_cfg[i].border_cfg.cfg.b.bd_color_g = window->bgcolor[1];
			ctrl_cfg[i].border_cfg.cfg.b.bd_color_b = window->bgcolor[2];
			ctrl_cfg[i].border_cfg.start.x = window->rect_in.left - window->rect_out.left;
			ctrl_cfg[i].border_cfg.start.y = window->rect_in.top - window->rect_out.top;
			ctrl_cfg[i].dst.offset.x = window->rect_out.left;
			ctrl_cfg[i].dst.offset.y = window->rect_out.top;
			ctrl_cfg[i].dst.window.w = window->rect_out.width;
			ctrl_cfg[i].dst.window.h = window->rect_out.height;
			ctrl_cfg[i].dst.frame.w = window->rect_in.width;
			ctrl_cfg[i].dst.frame.h = window->rect_in.height;
			...
			if (window->top_width || window->left_width ||
				window->right_width || window->bottom_width) {
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.enable = 1;
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.bd_color_r = window->border_color[0];
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.bd_color_g = window->border_color[1];
				ctrl_cfg[i].border_vpp_cfg[0].cfg.b.bd_color_b = window->border_color[2];
				ctrl_cfg[i].border_vpp_cfg[0].inside_start.x = window->left_width;
				ctrl_cfg[i].border_vpp_cfg[0].inside_start.y = window->top_width;
				ctrl_cfg[i].border_vpp_cfg[0].inside_end.x =
					window->rect_out.width - window->right_width;
				ctrl_cfg[i].border_vpp_cfg[0].inside_end.y =
					window->rect_out.height - window->bottom_width;
				ctrl_cfg[i].border_vpp_cfg[0].outside_start.x = 0;
				ctrl_cfg[i].border_vpp_cfg[0].outside_start.y = 0;
				ctrl_cfg[i].border_vpp_cfg[0].outside_end.x = window->rect_out.width;
				ctrl_cfg[i].border_vpp_cfg[0].outside_end.y = window->rect_out.height;
			}
		}

		ctrl_cfg[i].dst.pitch_y = cfg->dst_cfg.bytesperline[0];
		ctrl_cfg[i].dst.pitch_c = cfg->dst_cfg.bytesperline[1];
	}

	mask.raw = 0;
	mask.b.cmdq = true;
	sclr_set_intr_mask(dev_idx, mask);
	//TODO:recover intr

	sclr_engine_cmdq(dev_idx, ctrl_cfg, stitch_num, cmdq_buf->cmdq_phy_addr,
						cmdq_buf->cmdq_vir_addr, cmdq_buf->cmdq_buf_size);
	osal_vfree(ctrl_cfg);
	return 0;
}
...
void sc_update(u8 dev_idx, const struct vpss_sc_cfg *chn_cfg)
{
	...
	/*slice buffer*/
	_sc_ext_set_sb(dev_idx, &chn_cfg->sb_cfg);
	TRACE_VPSS(DBG_DEBUG, "%10s(%4d)%10s(%4d)%10s(%4d)%10s(%4d)%20s(%4d)\n",
		"sb_mode", chn_cfg->sb_cfg.sb_mode, "sb_size", chn_cfg->sb_cfg.sb_size,
		"sb_nb", chn_cfg->sb_cfg.sb_nb, "sb_full_nb", chn_cfg->sb_cfg.sb_full_nb,
		"sb_wr_ctrl_idx", chn_cfg->sb_cfg.sb_wr_ctrl_idx);

	/*gop*/
	layers = RGN_MAX_LAYER_VPSS;
	if (dev_idx > 1)
		layers = 1;
...
{
	sclr_set_base_addr(base_addr);
}

void vpss_ip_init(u8 dev_idx, bool is_resume)
{
	sclr_ctrl_init(dev_idx, is_resume);
}

void vpss_ip_reset(u8 dev_idx, bool is_sbm, bool toggle)
{
	sclr_img_reset(dev_idx);

	if (is_sbm)
		sclr_odma_clear_sb(dev_idx);
...
void vpss_interrupter_clear(u8 dev_idx, struct vpss_interrupter_status *status)
{
	...
	intr_status = sclr_intr_status(dev_idx);
	sclr_intr_clr(dev_idx, intr_status);

	TRACE_VPSS(DBG_NOTICE, "vpss(%d), status(0x%x)\n", dev_idx, intr_status.raw);
	...
	if (intr_status.b.cmdq_end == 1) {
		...
		sclr_clr_cmdq(dev_idx);
	}
}

void img_update(u8 dev_idx, bool is_master, const struct vpss_img_in_cfg *grp_cfg)
{
	...
	cfg->dup2fancy_enable = false; //grp_cfg->upsample, not support
	...
	if (!is_master) {
		...
		cfg->mem.start_y = 0;
		//work-around
		sclr_img_set_src_mem(dev_idx);
	}


	TRACE_VPSS(DBG_DEBUG, "update img(%d) online_from_isp=%d, fmt=%d, csc=%d\n",
		dev_idx, grp_cfg->online_from_isp, cfg->fmt, cfg->csc);
...
void top_update(u8 dev_idx, bool is_share, bool sc_enable)
{
	...
	sclr_top_set_cfg(dev_idx, sc_enable);
}
...
void img_start(u8 dev_idx, u8 chn_num)
{
	...
	TRACE_VPSS(DBG_NOTICE, "---img(%d) start---\n", dev_idx);
}
...
u32 vpss_get_checksum(u8 dev_idx)
{
	...
	sclr_core_get_checksum_status(dev_idx, &sc_status);
	return sc_status.axi_write_data;
}
...
void vpss_get_csc_mtrx(enum vpss_csc csc, struct vpss_csc_matrix *csc_matrix)
{
	...
	mtrx = sclr_get_csc_mtrx((enum sclr_csc)csc);

	memcpy(csc_matrix, mtrx, sizeof(*csc_matrix));
}
...
void vpss_clr_gop_addr(u8 inst, u8 layer, u8 ow_inst)
{
	sclr_gop_ow_clr_addr(inst, layer, ow_inst);
}
