...
static int vo_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "%10s%10s%10s%10s%10s%10s%10s%20s%20s\n",
	...
	for (i = 0; i < VO_MAX_VIDEO_LAYER_NUM; ++i) {
		...
		seq_printf(m, "%8s%2d%10d%10d%10d%10d%10d%10d%20llu%20llu\n",
				"#",
				i,
	...
	for (i = 0; i < VO_MAX_VIDEO_LAYER_NUM; ++i) {
		...
		seq_printf(m, "%8s%2d%10d%10d%10d%10d%10d\n",
				"#",
				i,
	...
	for (i = VO_MAX_VIDEO_LAYER_NUM; i < VO_MAX_LAYER_NUM; ++i) {
		...
		seq_printf(m, "%8s%2d%10d%10d%10d\n",
				"#",
				i,
	...
	// chn basic info
	seq_puts(m, "\n-------------------------------CHN BASIC INFO 1---------------------------\n");
	seq_printf(m, "%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s%10s\n",
	...
	for (i = 0; i < VO_MAX_VIDEO_LAYER_NUM; ++i) {
		for (j = 0; j < VO_MAX_CHN_NUM; ++j) {
			...
			seq_printf(m, "%8s%2d%8s%2d%10s%10d%10d%10d%10d%10d%10d%10d%10s%10d\n",
				"#",
				i,
				"#",
				j,
	...
	for (i = 0; i < VO_MAX_VIDEO_LAYER_NUM; ++i) {
		for (j = 0; j < VO_MAX_CHN_NUM; ++j) {
			if (!vo_ctx_p->layer_ctx[i].chn_ctx[j].is_chn_enable)
				continue;
			seq_printf(m, "%8s%2d%8s%2d%10s%10s%10d%10d%20d%20llu%20llu\n",
				"#",
				i,
				"#",
				j,
