...
#   echo 1 > test
# linux端(user space for cust and system)
#   ./test
# 
# ----------------------------------------------------
# custom core comm 内核态api测试
# insmod cvi_ipcm_test.ko
# 参考代码：osdrv/interdrv/v2/ipcm/test/linux/cust_kernel_test.c
#
# 说明：
# 1. alios端发送port id>=PORT_CUST_KER_ST的客制化msg会先送到kernel 处理，根据kernel handler返回值判断是否进一步给用户层处理；
# 2. alios端发送port id范围为PORT_CUST_ST到PORT_CUST_KER_ST之间的处理逻辑不变；
# 3. linux kernel可以支持发送port id>=PORT_CUST_KER_ST的msg
#
# 测试步骤：
