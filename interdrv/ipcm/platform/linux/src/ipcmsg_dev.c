...
static struct ipcmsg_signal_ctl stSigCtl;
static atomic_t s_msg_id = ATOMIC_INIT(0);
static struct mutex data_lock[IPCM_DATA_SPIN_MAX];

void ipcmsg_signal_send(int s32Id, CVI_IPCMSG_MESSAGE_S *pstMsg)
{
	unsigned int i;
	...
	for (i = 0; i < MAX_SIG_MSG_NUM; ++i) {
		...
		stSigCtl.stSigData[i].pstMsg->pBody = (void *)kzalloc(pstMsg->u32BodyLen, GFP_KERNEL);
...
static long dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case IPCMSG_IOC_ADD_SERVICE:
	{
		struct ipcmsg_add_service_cfg add_service;
	...
	case IPCMSG_IOC_CONNECT:
	{
		struct ipcmsg_connect_cfg connect_cfg;
	...
	case IPCMSG_IOC_SEND_ONLY:
	{
		struct ipcmsg_send_only_cfg send_only_cfg;
	...
	case IPCMSG_IOC_SEND_ASYNC:
	{
		struct ipcmsg_send_cfg send_cfg;
		CVI_IPCMSG_MESSAGE_S stMsg;
	...
	case IPCMSG_IOC_GET_ID:
	{
		int s32Id = atomic_fetch_add(1, &s_msg_id);
	...
	case IPCMSG_IOC_SIG_DATA:
	{
		struct ipcmsg_signal_cfg sig_cfg;
		unsigned int i;
