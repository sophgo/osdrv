/**
 * @file ipcm_dev.c
 * @brief IPCM device driver implementation for Linux
 */
...
// Global mailbox interrupt number
int mailbox_irq;
...
static int ipcm_dev_probe(struct platform_device *pdev)
{
	...
	ret = ipcm_register_dev();
	if (ret < 0) {
		ipcm_err("ipcm_register_dev error ret:%d\n", ret);
		return ret;
	}
	...
	ret = ipcm_cust_register_dev();
	if (ret) {
		ipcm_err("ipcm_cust_register_dev fail.\n");
	}
