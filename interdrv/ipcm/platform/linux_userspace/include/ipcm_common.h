/**
 * @brief Common definitions and types for IPCM
 *
 * Contains shared constants, data structures and type definitions used across IPCM
 */
...
/**
 * @brief RTOS boot status indicators
 */
typedef enum _RTOS_BOOT_STATUS_E {
    RTOS_SYS_BOOT_STAT = 0,  // System boot status
    RTOS_PANIC,              // System panic state
 ...
 * 12 ~ 147: for custom core comm msg
 * others: reserved
 *
 */
...
#define IPCM_CUST_PORT_MAX 128
#define IPCM_CUST_KER_PORT_ST 64
...
// 64 for userspace, 64 for kernel and userspace
#define IPCM_CUST_PORT_IDX (IPCM_MSG_PORT_IDX + IPCM_MSG_PORT_MAX)
...
typedef enum _MsgType {
	MSG_TYPE_SHM = 0,      // Message parameter is shared memory address
	MSG_TYPE_RAW_PARAM,    // Message parameter is raw value
} MsgType;
...
/**
 * @brief Message pointer structure
 */
typedef struct _MsgPtr {
	u32 data_pos : 20;         // Data position offset
	u32 remaining_rd_len : 12; // Remaining read length
} MsgPtr;

/**
 * @brief Message parameter union
 */
typedef union _MsgParam {
	MsgPtr msg_ptr;        // Message pointer for shared memory
	unsigned int param;    // Raw parameter value
} MsgParam;
 ...
 * PORT_CUST: custom api
 * if alios send msg, msg port_id >= IPCM_CUST_KER_PORT_ST, and linux kernel has been registered handler,
 * linux kernel handler will recv msg
 */
typedef enum _PortType {
	PORT_SYSTEM = 0,  // System port
	PORT_VIRTTTY,     // Virtual TTY port
	PORT_SHAREFS,     // Shared filesystem port
	PORT_MSG,         // Message port
	PORT_CUST,        // Custom port
	PORT_BUTT,
} PortType;

typedef struct _IPCM_FLUSH_PARAM {
	u32 data_pos; // offset relative to pool manager
	u32 len;
} IPCM_FLUSH_PARAM;

// ipcm pool
typedef void *POOLMGRHANDLE;
