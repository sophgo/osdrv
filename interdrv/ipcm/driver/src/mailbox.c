/**
 * @file mailbox.c
 * @brief Mailbox communication implementation for IPCM
 *
 * Implements hardware mailbox interface for inter-processor communication.
 * Features include:
...
IPCMPA_IRQ_RETURN_TYPE prvQueueISR(int irq, void *dev_id)
{
	...
	if (set_val) {
		for (i = 0; i < MAILBOX_MAX_NUM; i++) {
			...
			if (valid_val) {
				...
				/* mailbox buffer context is send from linux or rtos*/
				if (ipcmpa_get_mb_valid(&tmsg) == 1) {
...
s32 mailbox_init(mailbox_handle handle, void *data)
{
	...
	reg_base = (unsigned long)ipcmpa_ioremap(MAILBOX_REG_BASE, 0x1000);
...
/**
 * @brief Clean up mailbox system
 *
 * Releases mailbox resources:
 * - Disables interrupts
...
{
	ipcmpa_iounmap((void *)(unsigned long)reg_base);
	_mb_handle = NULL;
	ipcmpa_free_irq();
	return 0;
}
...
/**
 * @brief Get count of invalid messages
