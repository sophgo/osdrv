...
{
	int i;
	TRACE_CIF(DBG_DEBUG, "devno = %d, input_mode = %s, mac_clk = %s\n",
		attr->devno,
		_to_string_input_mode(attr->input_mode),
		_to_string_mac_clk(attr->mac_clk));
	TRACE_CIF(DBG_DEBUG, "mclk%d = %s\n",
		attr->mclk.cam, _to_string_mclk(attr->mclk.freq));
	TRACE_CIF(DBG_DEBUG, "width = %d, height = %d\n",
		attr->img_size.width, attr->img_size.height);
	switch (attr->input_mode) {
	case INPUT_MODE_MIPI: {

		TRACE_CIF(DBG_DEBUG, "raw_data_type = %s\n",
			_to_string_raw_data_type(attr->mipi_attr.raw_data_type));

		for (i = 0; i < MIPI_LANE_NUM + 1; i++) {
			TRACE_CIF(DBG_DEBUG, "lane_id[%d] = %d, pn_swap = %s\n", i,
				attr->mipi_attr.lane_id[i],
				attr->mipi_attr.pn_swap[i] ? "True" : "False");
		}
		TRACE_CIF(DBG_DEBUG, "wdr_mode = %s\n",
			_to_string_mipi_wdr_mode(attr->mipi_attr.wdr_mode));
		for (i = 0; i < WDR_VC_NUM; i++) {
			TRACE_CIF(DBG_DEBUG, "data_type[%d] = 0x%x\n",
				i, attr->mipi_attr.data_type[i]);
		}
	}
	break;
	...
	case INPUT_MODE_HISPI: {
		...
		TRACE_CIF(DBG_DEBUG, "wdr_mode = %s\n",
			_to_string_wdr_mode(attr->lvds_attr.wdr_mode));
		TRACE_CIF(DBG_DEBUG, "sync_mode = %s\n",
			_to_string_lvds_sync_mode(attr->lvds_attr.sync_mode));
		TRACE_CIF(DBG_DEBUG, "raw_data_type = %s\n",
			_to_string_raw_data_type(attr->lvds_attr.raw_data_type));
		TRACE_CIF(DBG_DEBUG, "data_endian = %s\n",
			_to_string_bit_endian(attr->lvds_attr.data_endian));
		TRACE_CIF(DBG_DEBUG, "sync_code_endian = %s\n",
			_to_string_bit_endian(attr->lvds_attr.sync_code_endian));
		for (i = 0; i < MIPI_LANE_NUM + 1; i++) {
			TRACE_CIF(DBG_DEBUG, "lane_id[%d] = %d, pn_swap = %s ", i,
				attr->lvds_attr.lane_id[i],
				attr->lvds_attr.pn_swap[i] ? "True" : "False");
		}
		TRACE_CIF(DBG_DEBUG, "sync code = {\n");
		for (i = 0; i < MIPI_LANE_NUM; i++) {
			TRACE_CIF(DBG_DEBUG, "\t{\n");
			for (j = 0; j < WDR_VC_NUM+1; j++) {
				TRACE_CIF(DBG_DEBUG,
					"\t\t{ %3x, %3x, %3x, %3x },\n",
					attr->lvds_attr.sync_code[i][j][0],
					attr->lvds_attr.sync_code[i][j][1],
					attr->lvds_attr.sync_code[i][j][2],
					attr->lvds_attr.sync_code[i][j][3]);
			}
			TRACE_CIF(DBG_DEBUG, "\t},\n");
		}
		TRACE_CIF(DBG_DEBUG, "}\n");
		TRACE_CIF(DBG_DEBUG, "vsync_type = %s\n",
			_to_string_lvds_vsync_type(attr->lvds_attr.vsync_type.sync_type));
		TRACE_CIF(DBG_DEBUG, "fid = %s\n",
			_to_string_lvds_fid_type(attr->lvds_attr.fid_type.fid));
				    ...
				    struct param_sublvds *sublvds)
{
	...
	struct link *link = container_of(ctx, struct link, cif_ctx);
...
#ifndef FPGA_TESTING
	if (vi == TTL_VI_SRC_VI0) {
		TRACE_CIF(DBG_ERR, "BT_DEMUX can not use VI0\n");
		return -EINVAL;
	} else if (vi == TTL_VI_SRC_VI1) {
		PINMUX_CONFIG(VIVO_CLK, VI1_CLK);
	} else {
...
#ifndef FPGA_TESTING
	if (vi == TTL_VI_SRC_VI0) {
		TRACE_CIF(DBG_ERR, "BT_656_DDR can not use VI0\n");
		return -EINVAL;
	} else if (vi == TTL_VI_SRC_VI1) {
		PINMUX_CONFIG(VIVO_CLK, VI1_CLK);
	} else {
		...
		enum rx_mac_clk_e mac_clk)
{
	...
	TRACE_CIF(DBG_DEBUG, "ratio %d, set rate %dM\n", tmp, (tmp + 1) * cdev->max_mac_clk / 32);
	osal_udelay(5);
	}
		...
		struct combo_dev_attr_s *attr)
{
	...
	if (attr->devno >= CIF_MAX_CSI_NUM) {
		TRACE_CIF(DBG_DEBUG, "invalid devno %d\n", attr->devno);
		return -EINVAL;
	}
	...
	if (attr->img_size.width == 0 || attr->img_size.height == 0 ||
		...
		TRACE_CIF(DBG_DEBUG, "invalid image size %dx%d, max_size %dx%d\n",
			attr->img_size.width, attr->img_size.height,
	...
	if ((attr->img_size.start_x + attr->img_size.active_w > attr->img_size.max_width) ||
		...
		TRACE_CIF(DBG_DEBUG, "invalid image size x:y:w:h(%d:%d:%d:%d)\n",
...
int cif_set_dev_attr(struct cif_dev *dev, struct combo_dev_attr_s *attr)
{
	...
	if (rc < 0) {
		TRACE_CIF(DBG_ERR, "check attr fail %d\n", rc);
		return rc;
	}
	...
	if (rc < 0) {
		TRACE_CIF(DBG_ERR, "set attr fail %d\n", rc);
		return rc;
	}
...
int _cif_enable_snsr_clk(struct cif_dev *cdev, uint32_t devno, uint8_t on)
{
	...
	if (snsr_mclk[0] > CAMPLL_FREQ_NONE && snsr_mclk[0] < CAMPLL_FREQ_NUM) {
		if (on) {
			if (!cdev->clk_cam[0].is_on) {
				TRACE_CIF(DBG_INFO, "start set snsr_mclk[0]\n");
				clk_div = mmio_read_32(CIF_MCLK_GEN_BASE + CIF_MCLK_GROUP0_PLL);
				...
				mmio_write_32(CIF_MCLK_GEN_BASE + CIF_MCLK_GROUP0_PLL, clk_div);
	...
	if (snsr_mclk[1] > CAMPLL_FREQ_NONE && snsr_mclk[1] < CAMPLL_FREQ_NUM) {
		TRACE_CIF(DBG_INFO, "start set snsr_mclk[1]\n");
		if (on) {
			if (!cdev->clk_cam[1].is_on) {
				TRACE_CIF(DBG_INFO, "start set snsr_mclk[1]\n");
				clk_div = mmio_read_32(CIF_MCLK_GEN_BASE + CIF_MCLK_GROUP1_PLL);
				...
				write_value_range(&clk_div, 4, 4, 0);
				mmio_write_32(CIF_MCLK_GEN_BASE + CIF_MCLK_GROUP1_PLL, clk_div);
	...
	if (snsr_mclk[2] > CAMPLL_FREQ_NONE && snsr_mclk[2] < CAMPLL_FREQ_NUM) {
		TRACE_CIF(DBG_INFO, "start set snsr_mclk[2]\n");
		if (on) {
			if (!cdev->clk_cam[2].is_on) {
				TRACE_CIF(DBG_INFO, "start set snsr_mclk[2]\n");
				clk_div = mmio_read_32(CIF_MCLK_GEN_BASE + CIF_MCLK_GROUP2_PLL);
				...
				write_value_range(&clk_div, 4, 4, 0);
				mmio_write_32(CIF_MCLK_GEN_BASE + CIF_MCLK_GROUP2_PLL, clk_div);
...
int cif_reset_snsr_gpio(struct cif_dev *dev, sns_rst_config *sns_rst_config, uint8_t on)
{
	...
	TRACE_CIF(DBG_INFO, "config rst GPIO port[%d] pin [%d]\n",
				sns_rst_config->gpio_port, sns_rst_config->gpio_pin);

	ret = csi_gpio_init(&link->snsr_rst_gpio, sns_rst_config->gpio_port);
	if (ret != CSI_OK) {
		printf("sensor reset gpio init failed\r\n");
		return -1;
	}
	ret = csi_gpio_dir(&link->snsr_rst_gpio, 1 << sns_rst_config->gpio_pin, GPIO_DIRECTION_OUTPUT);
	if (ret != CSI_OK) {
		TRACE_CIF(DBG_INFO, "csi_gpio_dir failed\r\n");
		return -1;
	}
	if (on)
		csi_gpio_write(&link->snsr_rst_gpio, 1 << sns_rst_config->gpio_pin, reset);
	else
		csi_gpio_write(&link->snsr_rst_gpio, 1 << sns_rst_config->gpio_pin, !reset);
...
static void cif_show_dev_attr(struct combo_dev_attr_s *attr)
{
	...
	printf("%8s%10s%10s%10s%10s%12s%16s\n",
...
static void cif_show_mipi_sts(struct link *link)
{
...
static void cif_show_phy_sts(struct link *link)
{
