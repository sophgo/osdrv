...
u32 halbb_get_fw_ra_rpt(struct bb_info *bb, u16 len, u8 *c2h)
{
	...
	if (ra_rpt_i->is_mu) {
		...
		if (ra_rpt_i->rpt_md_sel == BB_HE_MODE)
			...
			rt_i->is_actrl = false;
		/* 8852C Aruba AP STBC IOT WA: Dynamic turn off STBC in WD when HE 1ss per=100 */
		if ((rt_i->en_stbc == true) && (ra_rpt_i->retry_ratio == 100)
			&& (bb->ic_type & BB_RTL8852C) && (ra_rpt_i->rpt_md_sel == BB_HE_MODE)
			&& ((ra_rpt_i->rpt_mcs_nss) >> 4 & 0x3) == 0)
			rt_i->en_stbc= false;
