			  ...
			  u32 input_num, char *output, u32 out_len)
{
	...
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
	 ...
	 BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		  ", rx_evm[1ss:%ddB/2ss_max:%ddB/2ss_min:%ddB]",
				  ...
				  char *output, u32 out_len)
{
	...
	if (hid->exist) {
		...
		for (i = 0; i < hid->cnt; i++) {
			...
			BTC_CNSL(out_len, *used, output + *used, out_len - *used,
				 ", Dev-0x%x[v-id:0x%x/%s/%s/slot:%d-%d]",
				 hid->links[i].h,
				 hid->links[i].vendor_id,
			  ...
			  u32 input_num, char *output, u32 out_len)
{
...
#if 0
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 ", wl_ch[%02x%02x_%02x%02x_%02x%02x_%02x%02x_%02x%02x]",
		 wl->ch_map[0], wl->ch_map[1], wl->ch_map[2], wl->ch_map[3],
		 wl->ch_map[4], wl->ch_map[5], wl->ch_map[6], wl->ch_map[7],
		 wl->ch_map[8], wl->ch_map[9]);
#endif

	if (bt_linfo->status.map.ble_connect) {
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 ", LE[%02x%02x_%02x_%02x%02x](cnt:%d/conflict_ch:%d)",
			 afh_le[0], afh_le[1], afh_le[2], afh_le[3], afh_le[4],
			 cx->cnt_bt[BTC_BCNT_AFH_LE_UPDATE],
			 cx->cnt_bt[BTC_BCNT_AFH_LE_CONFLICT]);
#if 0
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 ", wl_ch_LE[%02x%02x_%02x_%02x%02x]",
			 wl->ch_map_le[0], wl->ch_map_le[1], wl->ch_map_le[2],
			 wl->ch_map_le[3], wl->ch_map_le[4]);
#endif
	}
	...
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 "role-sw:%d, afh-sw:%d, inq:%d, pag:%d, igno_wl:%d",
	...
	if (!bt->scan_info_update) {
		...
		rpt_map &= ~RPT_EN_BT_SCAN_INFO;
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		         "(BG:%d-%d", bt->scan_info[CXSCAN_BG].win,
		         bt->scan_info[CXSCAN_BG].intvl);
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		         "/INIT:%d-%d", bt->scan_info[CXSCAN_INIT].win,
		         bt->scan_info[CXSCAN_INIT].intvl);
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		         "/LE:%d-%d)", bt->scan_info[CXSCAN_LE].win,
			    ...
			    char *output, u32 out_len)
{
	...
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 "leak_n:%d,/ext_ctrl:%d/null_role:0x%x/opt:0x%x]",
...
#ifdef BTC_FDDT_TRAIN_SUPPORT
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 "leak_ap:%d, fddt_train:%d", dm->leak_ap, dm->fddt_train);
#else
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 "leak_ap:%d, fddt_train:No-Support", dm->leak_ap);
#endif
...
#endif
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 ", bcn[all:%d/ok:%d/in_bt:%d/in_bt_ok:%d]",
			...
			char *output, u32 out_len)
{
	...
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 ...
		 "fw_rpt[len_err:0x%x/ver_err:0x%x/except_cnt:%d]",
		 "[ERROR]",
	...
	BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 ", bt_flood_cnt:%d", dm->cnt_dm[BTC_DCNT_BT_SLOT_FLOOD]);
}
		     ...
		     u32 input_num, char *output, u32 out_len)
{
	...
	if (input_num < 2 || input_num > 10) {
		...
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 " dbg = 4: bt flood-threshold setup <para1:8bits>\n");
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 " dbg = 5: dm error map decode\n");
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 " dbg = 6: fw exception map decode\n");
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 " dbg = 7: fw report length mismatch map decode\n");
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 " dbg = 8: fw report version mismatch map decode\n");
		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
			 " dbg = 9: fddt no-run reason map decode\n");
		return;
	}
	...
	case 7: /* fw report length mismatch */
		if (!pfwinfo->len_mismch) {
			BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 		 " no fw report length mismatch!!\n");
			return;
		}

		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 	 " fw report length mismatch includes:\n");
	...
	case 8: /* fw report version mismatch */
		if (!pfwinfo->fver_mismch) {
			BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 		 " no fw report version mismatch!!\n");
			return;
		}

		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 	 " fw report verison mismatch includes:\n");
		for (i = 0; i < BTC_RPT_TYPE_MAX; i++) {
	...
	case 9: /* fddt no-run reason */
		if (!dm->fddt_info.nrsn_map) {
			BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 		 " no fddt no-run reason!!\n");
			return;
		}

		BTC_CNSL(out_len, *used, output + *used, out_len - *used,
		 	 " fddt no-run reason includes:\n");
