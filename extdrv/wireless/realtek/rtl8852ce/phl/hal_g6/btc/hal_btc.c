			 ...
			 u8 reason)
{
	...
	for (i = 0; i < ntfy_num; i++) {
		...
		if (btc->hal->chip_id == CHIP_WIFI6_8851B &&
		    ...
		    wl_rinfo->active_role[role_idx].band == BAND_ON_5G &&
		    wl_rinfo->active_role[role_idx].ch >= 100)
			is_5g_hi_channel = 1;
	}
	...
	if (is_sta_change) {
		...
		if (wl->status.map.busy && wl->rssi_level <= 2 &&
		    is_5g_hi_channel)
