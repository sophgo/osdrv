...
static u8 *rm_gen_bcn_detail_elem(struct rrm_obj *prm,
	_adapter *padapter, u8 *pframe, 
	struct wlan_network *pnetwork, unsigned int *fr_len)
{
	...
	for (j = 0; j < prm->q.opt.bcn.opt_id_num; j++) {
		...
		case bcn_req_req:
			...
			break;
		case bcn_req_rep_detail:
			FSM_INFO(prm, "bcn_req_rep_detail\n");
			break;
		case bcn_req_ap_ch_rep:
			FSM_INFO(prm, "bcn_req_ap_ch_rep\n");
			break;
		default:
			FSM_INFO(prm, "OPT %d TODO\n",prm->q.opt.bcn.opt_id[j]);
			break;
		}
	}
done:
...
static u8 *rm_gen_bcn_rep_ie (struct rrm_obj *prm,
	u8 *pframe, struct wlan_network *pnetwork, unsigned int *fr_len)
{
	...
	*/
	...
	pframe = rm_gen_bcn_detail_elem(prm, padapter, pframe,
		pnetwork, &my_len);
	/*
	* update my length
	* content length does NOT include ID and LEN
	...
	while (1) {
		...
		*/
		...
		if (rm_bcn_req_cond_mach(prm, pnetwork) == _FALSE) {
			FSM_DBG(prm, "condition mismatch ch %u ssid %s bssid "MAC_FMT"\n",
				pch_set[i].hw_value, pbss->Ssid.Ssid,
				MAC_ARG(pbss->MacAddress));
			FSM_DBG(prm, "condition %u:%u\n",
				prm->q.opt.bcn.rep_cond.cond,
				prm->q.opt.bcn.rep_cond.threshold);
			goto next;
		}
		...
		FSM_DBG(prm, "ch %u %s bssid "MAC_FMT"\n",
			pch_set[i].hw_value, pbss->Ssid.Ssid,
			MAC_ARG(pbss->MacAddress));
...
fail:
	...
	FSM_INFO(prm, "%d matched %s\n",
		matched_network, prm->q.opt.bcn.ssid.Ssid);
...
/* For EXTERNAL application to abort all rrm section on the same sta (expose)
 * psta : specific psta
 * psta : null means all sta
 */
int rtw_rrm_abort_sta(struct rrm_priv *prmpriv, struct sta_info *psta)
{
	_queue *queue = &prmpriv->rm_queue;
	struct rrm_obj *prm;
	...
	_rtw_spinlock_bh(&queue->lock);
	list_for_each_entry(prm, &queue->queue, list) {
		if (!psta) /* abort all rm */
			rtw_fsm_abort_obj(prm);
		else if (obj2sta(prm) == psta)
			rtw_fsm_abort_obj(prm);
	}
	_rtw_spinunlock_bh(&queue->lock);
