...
#ifdef CONFIG_PLATFORM_SOPHGO
#include <linux/gpio.h>
extern int cvi_sdio_rescan(void);
extern int cvi_get_wifi_pwr_on_gpio(void);
static int gpio_power_on;

static int sophgo_wifi_power_on(void)
{
	gpio_power_on = cvi_get_wifi_pwr_on_gpio();
	if (gpio_power_on < 0) {
		printk(KERN_ERR "power on gpio request error!\n", __func__);
		return -1;
	}
	printk(KERN_INFO "[sophgo] Power on AIC8800.\n");

	gpio_direction_output(gpio_power_on, 0);
	mdelay(500);
...
static void sophgo_wifi_power_off(void)
{
	if (gpio_power_on >= 0)
	{
		if (gpio_direction_output(gpio_power_on, 0))
		{
			printk(KERN_ERR "%s: WL_PW_ON didn't output low\n", __func__);
			return ;
		}
	}
	printk(KERN_INFO "[sophgo]Wifi sdio power off\n");
}
#endif
